...
> 🆕🆕🆕 **RECAP TERBARU (18 Jun 2026, SESI v.103b — NATIVE-FEEL MOBILE + FIX IZIN GOOGLE PLAY + EKSPOR AUTO-SAVE).** Branch `feature/vue-migration`, HEAD `a46920e` (25 file). **BELUM rebuild AAB / resubmit Play / deploy web** (nunggu kyai — butuh keystore + uji device). Diverifikasi preview DOM@375px + `npm run build` exit 0. (1) **FIX PENOLAKAN GOOGLE PLAY** "Penggunaan izin foto & video tidak valid": hapus `READ_MEDIA_IMAGES`/`READ_MEDIA_VIDEO` dari `vue-app/android/app/src/main/AndroidManifest.xml` — SEMUA upload gambar (post `PostsView`, bukti transfer `PembayaranView`, foto profil, logo) lewat `<input type=file>` WebView = file chooser/photo picker SISTEM, TAK butuh izin (Filesystem manifest kosong, tak ada plugin kamera). CAMERA & storage TIDAK disentuh. ⚠️ legacy `android/` root masih punya READ_MEDIA (IMAGES+DOCUMENTS) tapi BUKAN build yg dikirim. (2) **Back button Android → NATIVE back-stack** (`App.vue`) — back = mundur ke layar sebelumnya via `window.history.state.back` di mana pun; keluar app HANYA di Beranda/`/login` (tekan 2×). GANTI pola "tab-app" v.103 lama (tab utama selalu lempar ke Beranda = keluhan kyai "selalu balik ke beranda"). Sub-state halaman tetap via event `android-back`. Native-only (uji di HP). (3) **Modal POS kepotong sisi-kiri di HP = FIX** (`ModalPOS.vue`: `.modal min-width:0`+`overflow-x:hidden`, grid track `minmax(0,…)`, `.inp min-width:0`) — flex-item tak lagi melar > viewport; verified preview (left=16,right=359 @375). (4) **Toolbar header → 1-baris scroll-samping** (pola `flex flex-nowrap md:flex-wrap overflow-x-auto md:overflow-visible hide-scrollbar [&>*]:shrink-0 md:[&>*]:shrink`): BukuInduk, NaikKelas (search full-width + tombol jadi strip terpisah), AbsensiGuru (+samakan link "Atur Libur"→pola tombol), Bisyaroh (tab strip), **RekapPrestasi action bar** (Cetak/PDF/Excel/Sheet/Template/Impor/SIMPAN — dulu wrap jadi 3 baris berantakan). (5) **Tabel Input Bulanan QIRAATI → kartu mobile** (`RekapPrestasiView`, `v-if=isMobile` table/cards, handler edit getEdit/setJuz/computedTotal/canEditPrestasi SAMA) → konsisten Rekap Diniyah. **2 jenis rekap (Qiraati & Diniyah) kini dua-duanya kartu di HP**, tabel di desktop. (6) **Target sentuh ≥44px**: tombol aksi standar `h-9 px-3 inline-flex … rounded-xl` → **`h-11 md:h-9`** (44px HP / 36px desktop) GLOBAL di SEMUA view (Guru/Santri/Rapor/TesKenaikan/RekapPrestasi/RiwayatSantri/BukuInduk/AbsensiGuru/NaikKelas/BackButton). (7) **Ekspor PDF/Excel di HP: auto-simpan ke `Documents/AmmuOnline` + TOAST lokasi** (`useNativeDownload.saveNative`) — BUANG dialog Share paksa tiap ekspor (kyai: "ribet"); Share hanya fallback bila cuma Cache yg berhasil. Native-only. ⚠️ folder **Download** sistem spesifik = belum (targetSdk 35 scoped-storage + Capacitor Filesystem 8 tak punya target Download → perlu MediaStore/plugin native = **opsi B**, ditunda sampai uji device). (8) Bump **v.103.0626 / versionCode 103** (10 titik display: package.json/Dashboard/Login/AppSidebar/Ribbon×2/Backstage/Ppdb/Bantuan/main.js Sentry + `build.gradle`). ⏳ **PENDING KYAI:** rebuild AAB (vc103) → uji HP (back button/upload/modal POS/ekspor — semua native-only) → resubmit Play (cek READ_MEDIA hilang) → deploy web. ⚠️ NB: recap **v.102** & **v.103 (rapikan dashboard /statistik→grafik + Laporan)** belum tercatat di PKB ini — ada di memory `project_dashboard_rapikan_v103` / `project_android_nativefeel` / `project_sesi_v102`.
> 🆕🆕🆕 **RECAP TERBARU (16 Jun 2026, SESI v.101 UI POLISH — DIAGNOSA AUTH ELECTRON + HAPUS PLACEHOLDER + UANG SAKU MA'HAD + PROFIL).** ✅ Web SUDAH DEPLOY (2×) + origin SUDAH PUSH. HEAD `3df8f03`. versionCode TETAP vc101 (web-only, TIDAK bump). (1) **Diagnosa console Electron** (gambar kyai): error massal `[subscribeColl] … Missing or insufficient permissions` + `Backup audit_log gagal` = GEJALA; AKAR = `securetoken.googleapis.com 400` (refresh token Auth MATI, kemungkinan sisa Anonymous di-disable v.100f) → `request.auth` null → semua rule `signedIn()` ditolak. UI "seolah login" dari localStorage. FIX = admin **logout + login 1×** (BUKAN fix kode); bandel → Clear site data. Lihat [[project_auth_domain_ammu]]. (2) **Form PSB: SEMUA placeholder dikosongkan** (12 field) — `PsbFormView.vue`. (3) **Menu "Uang Saku Saya" santri → HANYA ma'had/mukim**: sesi santri kini bawa `is_mukim` (auth.js), gate `santriMukimOnly` di useMenus (santri lama perlu logout+login agar terisi). (4) **ProfilGuru "Jabatan - Lembaga"**: sembunyikan " - lembaga" bila lembaga kosong (tak lagi "Jabatan - -"). (5) **Opsi pendidikan "D4 / S1" → "S1"** (ProfilGuru + ProfilSantri). (6) **Hapus placeholder teks field form SELURUH app** (150×, 33 file) — DIPERTAHANKAN: search/filter ("Cari" + "🔍 Cari"), typeahead "Ketik nama…", login, prop UiInput, `:placeholder` dinamis. Commit `5d5026f` + `3df8f03`. Build 2 app exit 0, EOL per-file terjaga. Detail PALING BAWAH ("SESI v.101 UI POLISH").
> 🆕🆕 **RECAP (15 Jun 2026, SESI v.101 LANJUTAN — AUDIT MENYELURUH + SENTRY FIX + PENSIUN LEGACY + SEO/AEO + FIREBASE 12 + BUMP vc101).** ✅ Web SUDAH DEPLOY + origin SUDAH PUSH. ⏳ AAB vc101 tinggal `npm run build:aab` (kyai, butuh keystore). HEAD `8b9d680`. (1) **Audit 6-poin** (app/UI/framework/SEO/dark-mode/Sentry) — laporan + fix. (2) **Sentry app Vue diperbaiki** (`d3b9cbb`): AKAR "Sentry hanya site legacy" = `initSentry()` dulu ambil DSN HANYA dari Firestore `settings/general.sentryDsn` yg KOSONG → app live tak pernah lapor; kini DSN dari `VITE_SENTRY_DSN` > default project, Firestore jadi override (smoke-test live: `[main.js] Sentry initialized`). (3) **Legacy dipensiun** (`6b9befd`): target `hosting:legacy` → folder `legacy-redirect/` (redirect + SW self-destruct bersihkan PWA cache); site TIDAK di-disable (authDomain `portal-mambaul-ulum.firebaseapp.com` melayani `/__/auth/*` OAuth). Deploy SEKALI: `firebase deploy --only hosting:legacy`. (4) **SEO/AEO** (`f00414f`+`123563b`): OG+Twitter+canonical+JSON-LD `EducationalOrganization` (alamat/telp/email/sosial — situs resmi **almanshur.id**, **@almanshurtv**) + robots.txt + sitemap.xml (landing+PSB); **Rich Results Test LOLOS** (Organization valid). (5) **Firebase JS 10.14.1→12.14.0** (`ab94b28`): NOL perubahan source, smoke-test live LOLOS (boot/init/App Check/Sentry/Firestore-read/auth-lookup, nol console error). (6) **Cleanup repo**: untrack `vue-app-psb/dist`+`public/psb`, ignore scratch, hapus KB duplikat underscore. (7) **Dark mode: TAK ada gap** — 4 sistem tema (Tailwind `dark:`, CSS-var token, scoped `:global(.dark)`, ribbon.css) SEMUA dark-aware. (8) **Bump vc100→vc101** (`8b9d680`, semua titik §5). Detail PALING BAWAH ("SESI v.101 LANJUTAN — AUDIT"). 
> 🆕🆕 **RECAP (15 Jun 2026, SESI v.101 — RAPOR PPPH REDESIGN + IDENTITAS QIRAATI TANPA NISN).** ✅ SUDAH DEPLOY (web `ammuonline.web.app`, 3× iterasi oleh kyai). (1) **Identitas rapor Qiraati** (TPQ/Pra PTPT/PTPT/PPPH): kolom **NISN DIHAPUS** → kiri Nama Santri·No. Induk·Kelas, kanan Semester·Tahun Ajaran (**Diniyah/sekolah formal TETAP pakai NISN**). (2) **Rapor PPPH** sesuai contoh kyai: header kolom **No · Level Kitab · Tgl Khotam**; urutan grup **DIBALIK** → **Hafalan Hadits** (Pencapaian, Ketepatan Matan, Pemahaman Sanad) lalu **Hafalan Al-Qur'an** (Tahfizh, Fashohah, **Tartil**); aspek ke-3 Al-Qur'an = **Tartil** (bukan Tajwid) — ikut diubah di **Tes Kenaikan** + feed nilai; **Pencapaian** = kolom baru **MANUAL** (di luar Tes Kenaikan). (3) **Lebar kolom PPPH** diperbaiki (No 8·Level Kitab ~28·Tgl 19·6 nilai @18·Predikat 22 = 185mm) + padding **asimetris** (atas/bawah tebal, kiri/kanan tipis) → teks tak kepotong tengah kata; baris dilonggarkan (minCellHeight 14). (4) Jarak judul 'SURAT KETERANGAN' ↔ identitas dilebarkan. (5) Skema PPPH default disinkronkan 3 titik (RaporView buildSchema + PengaturanView + LembagaDetailView) + `ppphStale` diperkuat (override tanpa `pencapaian` → rebuild default). (6) Bump **display v.101.0626** (AppSidebar + Ribbon fallback + package.json root); ⚠️ **versionCode AAB BELUM naik** (deploy web-only) → rilis AAB berikut tetap min vc101. Detail PALING BAWAH ("SESI v.101").
...
## 5. SKEMA VERSI
Format `v.<versionCode>.<MMYY>` (MM=bulan, YY=2 digit tahun). Saat ini **v.101.0626 / versionCode 101** — bump SEMUA titik §5 SELESAI 15 Jun (`8b9d680`), **SIAP rebuild AAB** (`npm run build:aab`, kyai — butuh keystore). AAB live terakhir di Play Console = **vc100** (14 Jun, `e18595c`); **vc101 BELUM di-build/upload**. (Riwayat: vc 100 sempat di-set lalu DITURUNKAN ke 99 pada 11 Jun karena belum ter-upload; vc100 dipakai 14 Jun → vc101 sekarang.) (Riwayat: vc 100 sempat di-set lalu DITURUNKAN ke 99 pada 11 Jun karena belum ter-upload; kini vc100 benar-benar dipakai → rilis berikut minimal **vc 101**.) Bump SEMUA tempat saat rilis AAB baru (Play Console tolak versionCode sama):
...
4. ✅ **SELESAI (commit `9a337a1`, v.100d) — Rekap Prestasi (input bulanan) FILTER pemisah Qiraati / Sekolah utk guru DUAL.**
   ...
   - HASIL: toggle segmented **Semua / Qiraati / Sekolah** di `RekapPrestasiView` (`v-if="isDualGuru"`,
     tampil di SEMUA mode utk guru dual). `filterTipe` ('all'|'qiraati'|'sekolah') di `filteredSantri`:
     qiraati=`ownNgaji` (editable), sekolah=`ownSekolah && !ownNgaji` (read-only, tak dobel), all=campur (perilaku lama).
     `isDualGuru` via `deteksiTipeGuru` (guruScope). Ekspor PDF/Excel/Sheet/Cetak/Template ikut OTOMATIS
     (semua pakai `filteredSantri`) → terpisah per kategori. Vue source murni, TANPA bump versionCode.
     Verify build:electron exit 0. **Deploy web kyai (`firebase:deploy`); AAB/Electron opsional.**

### STATUS DEPLOY (sesi v.100/b/c/d/e) — ✅ SUDAH DEPLOY semua (per 14 Jun 2026)
- v.100/b/c/d/e: web `firebase:deploy` + rules + functions SUDAH dijalankan kyai. v.100e murni Vue source (tanpa bump vc/rules/functions). AAB/Electron = opsional (belum tentu di-rebuild).

---

## SESI v.100d + v.100e — 4 BUG FIX + AUDIT LOG + KARTU IKON KENAIKAN + REVIEW KEAMANAN (14 Jun 2026, Claude Code) — RECAP PALING BARU, BACA INI DULU

> Branch `feature/vue-migration`. HEAD = `ff43298`. ✅ **SEMUA SUDAH DEPLOY (web)** sesi ini. v.100e = Vue source murni (tanpa bump vc/rules/functions). Verify `build:electron` exit 0 tiap commit.

### A. v.100d (5 commit, ADA di git SEBELUM sesi ini — belum tercatat di recap lama)
- `8de637b` **Riwayat Prestasi BULANAN**: snapshot awal/akhir/total per bulan → koleksi `riwayat_prestasi` (rules sudah ada). = AGENDA #1 SELESAI.
- `032aad3`+`db08444`+`412225c`+`f27b129` **Tes Kenaikan REDESIGN** (= AGENDA #3): penilaian aspek per lembaga skala 0–90 + Pra PTPT ikut tes (`032aad3`); logika naik diekstrak ke `utils/promosiKenaikan.js` sumber tunggal (`db08444`); **Lulus = auto-naik** via modal (konfirmasi tujuan + pilih guru; konsep "siap naik" DIHAPUS) (`412225c`); scoping Ajukan = ngaji-only + filter Lembaga/Jenis/Cari (`f27b129`).
  - ⚠️ **PENDING (AGEN BERIKUTNYA):** Fase 3 (nilai tes auto-isi rapor, prefill editable) · Fase 4 (ubah skema rapor: PTPT tukar Istimror↔Kelancaran + rename→Tahfizh; PPPH desain ulang 2 grup) · Fase 5 (statistik tes + cetak PDF daftar). Detail lengkap di memory `project_tes_kenaikan_redesign` (lokasi edit Fase 4 = builder INLINE di `RaporView.vue`, BUKAN `utils/raporSchema.js`).

### B. v.100e — 4 bug fix (commit `ce392e8`)
1. **Rename guru menyebar ke santri** (`useGuruForm.save` → `propagateGuruRename`): santri simpan guru sbg STRING nama (bukan id); saat nama guru berubah, perbarui referensi di santri yang cocok (`guru_pagi`/`guru_sore`/`guru`/`guru_sekolah[]`). Data LAMA yg terlanjur basi = kyai re-pilih guru manual sekali (rename sudah lewat, nama lama tak terpetakan otomatis).
2. **Header periode profil santri** (`CapaianPrestasiView`): baca `settings.txtPeriode` (diisi admin di Pengaturan "Periode Aktif"), bukan field hantu `periodeAktif` + fallback hardcoded "Dzulqo'dah 1447".
3. **Dedup "Juz JUZ n"**: helper `juzNum()` di `utils/format.js` (SUMBER TUNGGAL) → dipasang di CapaianPrestasi/ProfilSantri/SantriView/StatistikView/NaikKelasView(2)/TesKenaikanView. Akar: kenaikan PTPT simpan `juz='JUZ n'`, tampilan tambah prefix 'Juz' lagi.
4. **Tgl ceremonial PTPT/PPPH/TPQ-PK = MANUAL**: `promosiKenaikan.js` hapus auto-stamp `block.ceremonial=today`. Diisi lewat editor kartu (NaikKelasView `setCeremonial`) saat acara digelar.

### C. v.100e — audit log + kartu ikon (commit `ff43298`)
- **Audit Log "tidak update"** (`MasterDataView`): 2 penulis beda format timestamp — `firestoreSafe.backupSebelumHapus` (string ISO + field `aksi`/`user_nama`) vs `auditLog.writeAuditLog` (Firestore Timestamp + `action`/`operator`/`detail`-objek). Sort lama `tb - ta` jadi NaN utk entri ISO → backup hapus terbaru terlempar dari 50 teratas. Fix: `auditTsMs()` normalkan ke epoch ms + `auditUser()`/`auditDetail()` baca KEDUA skema (kolom User/Detail tak lagi kosong/[object Object]).
- **Filter lembaga Kenaikan → KARTU IKON** (`NaikKelasView`): Form Kenaikan grid tombol → kartu ikon (ikon+label, `active:scale-95` enak di Android, tetap warna identitas via `lembagaColor`). Riwayat: dropdown `<select>` DIGANTI = toggle Qiraati/Sekolah + kartu ikon (sama persis dgn Form). Helper `lembagaIcon()` + state `riwayatKategori`/`riwayatLembagaOptions`. `riwayatLembaga` kosong = wajib pilih (perilaku lama).

### D. ⚠️ REVIEW KEAMANAN (14 Jun 2026) — TEMUAN, RULES BELUM DIUBAH (perlu keputusan kyai)
- 🔴 **CRITICAL:** (1) `settings` read PUBLIK + simpan `adminPassword` (default 1234, dipakai `services/auth.js`) → siapa pun bisa baca sandi admin via Firestore langsung (config ada di bundle JS) → login super_admin. (2) `santri`/`guru` read PUBLIK + field `password` plaintext (`useSantriForm`/`useGuruForm`, default 1234) → PII anak+wali (NIK/WA/alamat) + semua sandi terbuka ke internet.
- 🟠 **HIGH:** write/delete = `signedIn()` = anonymous = siapa pun (semua user via Firebase Anonymous). RBAC HANYA di sisi app (bisa dilewati via SDK/REST langsung pakai config publik). Penyerang anonim bisa hapus/ubah santri/guru/keuangan/rapor.
- ✅ Sudah baik: read keuangan `signedIn` (v.98), `notif_queue` read:false (token FCM), `audit_log` update/delete:false (append-only), Storage delete butuh auth, default-deny di kedua rules.
- **Rekomendasi (urut):** (1) keluarkan kredensial dari dokumen public-read — jangan simpan `adminPassword`/`password` plaintext; pakai Firebase Auth + koleksi `read:false` / hash; (2) tutup read `santri`/`guru`/`settings` → `signedIn()` (⚠️ TES login lazy-migration dulu — mungkin masih butuh read publik; ada draft `firestore.rules.stage2-proposed`); (3) roadmap custom claims (role di token) → enforce role utk write/delete. **JANGAN ubah rules tanpa tes — bisa memutus login.**

---

## SESI v.100d LANJUTAN — RECOVERY SESI TERPOTONG + FIX EKSPOR PDF RAPOR (14 Jun 2026, Claude Code) — RECAP PALING BARU

> Branch `feature/vue-migration`. HEAD = `60bede7` (`f7e93fd` → `60bede7`). ✅ **SUDAH DEPLOY (web)** oleh kyai. Verify `build:electron` exit 0. Pure Vue source + rules, TANPA bump vc.

### KOREKSI TOPOLOGI DEPLOY (PENTING — perbaiki anggapan lama di HANDOVER)
- **LIVE** = **Vue** `ammuonline.web.app` (firebase target `main` → `public/vue`, build dari `vue-app/`) + **PSB** `ammuonline-psb` (`public/psb`).
- **Legacy** `public/index.html` (target `legacy` → `portal-mambaul-ulum.web.app`) = **SUDAH TIDAK DIPAKAI**. JANGAN diagnosa/edit di sana (gampang menyesatkan — mis. 70× `_raporState`). Semua screenshot & bug report kyai = dari **Vue**.

### A. Izin & Sakit mandiri guru (commit `f7e93fd`) — recovery dari sesi terpotong limit
- Dikerjakan sesi sebelumnya tapi BELUM ke-commit (terpotong limit setelah Fase 5 `45caa05`); di-recover & commit sesi ini.
- `composables/useIzinGuru.js` (BARU): guru `ajukan` (jenis izin/sakit, tgl mulai–selesai guard ≤60 hari, shifts[]) → Kepala/PJ/admin `setujui`/`tolak`; pengaju bisa `batal` selagi status 'diajukan'. `setujui` tulis `absensi_shift_guru` per tanggal×shift (id `shift_{guru_id}_{tgl}_{shift}`, status izin/sakit, source 'pengajuan_guru'); LEWATI hari yg sudah hadir/masuk/terlambat (return {written, skipped}). Scoping: admin=semua, Kepala=`lembagaScopeMatches`.
- `views/PersonalView.vue`: kartu "Izin & Sakit" (pengajuan saya + Batalkan) + kartu "Persetujuan" (approver, badge antrian) + modal Ajukan. Juga ganti label "saya"→"Anda"/netral (sesuai gaya).
- `firestore.rules`: koleksi `izin_guru` (read/create/update/delete = `signedIn()`; scoping per-role di app).

### B. Fix ekspor PDF rapor (commit `60bede7`) — 2 bug, KEDUANYA nyata di source (rebuild saja tak cukup)
- Gejala: toast **"Gagal ekspor PDF: raporState is not defined"** (kyai tes PPPH). Konfirmasi via artefak ter-deploy `public/vue/assets/RaporView-*.js` (ada `raporState` free-var) vs source.
- **Bug 1 (crash):** `raporPdf.js` `drawSignBlocks` (L391) TAK punya param `raporState`, tapi pakai `raporState.tanggal` (L396, fitur "tanggal terbit manual" v.100c #3) + call site L1082 tak mengoper → ReferenceError. `drawSignBlocks` dipanggil utk SEMUA rapor → ekspor PDF semua lembaga gagal sejak v.100c (PPPH kebetulan yg dites). Fix: tambah param `raporState = {}` + oper `raporState` di call site L1082.
- **Bug 2 (nilai kosong, ketutup Bug 1):** kelima generator (`generateTpqPdf`/`PraPtpt`/`Ptpt`/`Diniyah`/`Ppph`) baca `raporState.data`, tapi `RaporView.buildRaporStateFor` isi `data_nilai`. Fix: tambah `data: dn` di return `buildRaporStateFor` (`data_nilai` tetap utk path simpan rapor doc).
- Verify: `build:electron` exit 0; chunk `RaporView-*.js` baru = **0** akses `raporState.` free-var (tinggal `raporState:` sbg object key — sah).

### C. ⏳ PENDING kyai (DATA, BUKAN KODE) — kartu kenaikan PTPT Juz 5/10/15/20/25/30 hilang
- Gejala: kartu PTPT (`/naik-kelas`) tampil Kelas 1 = Juz 1–4, Kelas 2 = Juz 6–9, dst (juz kelipatan-5 hilang). Masih ada pasca-deploy.
- AKAR: **bukan** bug render (modal kartu pakai `v-for="it in k.items"` PENUH, tanpa slicing/special-case ceremonial). Default `DEFAULT_KARTU_KENAIKAN_PTPT` (kenaikan.js) BENAR = Juz 1–5/6–10/…/26–30 sejak commit pertama. Tapi `getKartuKenaikanSchema` (kenaikan.js:211) PRIORITASKAN **override** `settings.kartuKenaikanSchema.PTPT` yang BASI (4 juz/kelas; kemungkinan artefak legacy / pernah di-Simpan saat default lama). Store baca merge `settings/general`+`settings/web`.
- FIX (in-app, kyai): **Kenaikan Kelas → tab Pengaturan (ikon gear, admin) → pilih lembaga PTPT → "Reset Default" → lalu "Simpan Schema"** (urutan: Reset DULU baru Simpan). `saveSchema` tulis ke `settings/general`+`settings/web`. KOP "PTPT MAMBAUL ULUM" aman (key terpisah `kartuKenaikanKop`/`kopKartuKenaikan`). NB: modal kartu per-santri (Ekspor PDF/Simpan Kartu) BUKAN tempat edit struktur.
- ⚠️ Schema tiap lembaga BEDA — JANGAN reset borongan; cukup PTPT (atau lembaga lain yg kartunya juga kurang item). Firebase `db` TIDAK di-expose ke window → tak bisa fix via console snippet.

### Alur rilis sesi ini (sudah dijalankan kyai)
- `npm run firebase:deploy` (build vue → `public/vue` + psb → `public/psb`, deploy web) + `firebase deploy --only firestore:rules` (koleksi `izin_guru`). Hard reload device (bust cache).

---

## SESI v.100d LANJUTAN-2 — TES KENAIKAN FASE 3/4/5 SELESAI + 2 FIX KARTU PTPT (14 Jun 2026, Claude Code) — RECAP PALING BARU, BACA INI

> Branch `feature/vue-migration`. HEAD = `63a92a8`. Verify `build:electron` exit 0 tiap commit. Vue source murni, TANPA bump vc. ⚠️ Lihat STATUS DEPLOY di bawah — sebagian commit BELUM tentu ter-deploy.

### A. TES KENAIKAN — FASE 3/4/5 SELESAI → Item 3 AGENDA TUNTAS TOTAL (Fase 1–5 + scoping/filter)
- **Fase 4 (skema rapor) — `237cc9c`:** **PTPT** Kualitas Hafalan jadi `[Tahfizh, Istimror]` (tukar urutan istimror↔kelancaran + label 'Kelancaran'→'Tahfizh'; **id `kelancaran`/`hafalan_kelancaran` TETAP** → data lama aman) di `RaporView.vue buildSchema` + `raporSchema.js`. **PPPH** desain ulang layout `perKitab` → 2 grup berheader: **Hafalan Al-Qur'an** [tahfizh, fashohah, tajwid] + **Hafalan Hadits** [ketepatan_matan, pemahaman_sanad]; header grup 2 baris di preview (`ppphHeaderRow1`/`ppphGroupedFields`) + PDF (`raporPdf.js generatePpphPdf` colSpan/rowSpan). `predikatPpph` generik (rata-rata `fieldsNilai`). Staleness `ptptStale`/`ppphStale` (RaporView `_schemaRaw`) → override RAPOR lama auto-rebuild ke layout baru. ⚠️ minor belum: editor skema RAPOR di PengaturanView/LembagaDetailView masih seed lama (rendered rapor sudah benar via buildSchema).
- **Fase 3 (nilai tes auto→rapor) — `508587a`:** `utils/tesRaporFeed.js` (BARU) — `buildTesRaporFeed(tes, nilai)` map nilai tes → key `data_nilai` koleksi **`rapor_semester`** (doc `rapor_{id}_{lembaga}_{periodKey}`), di posisi yang **DISELESAIKAN** (kelas_asal/juz_asal, bukan target). Key: PTPT `kj__juz_<n>__`, PPPH `ppph__lvl_<n>__`, Pra PTPT `pra__lvl_<n>__kh_<ROM>__`, TPQ `jilid__<row>__` / `imtas__` / `khotaman__`. `currentRaporPeriode()` = TA `Y-(Y+1)` + semester (bln<6=Genap). `TesKenaikanView.submitLulus` → setDoc **merge** ke rapor periode berjalan (prefill editable; best-effort, tak menggagalkan Lulus). Aspek→field id: PTPT `tahfizh`→`kelancaran`; Pra PTPT `tahfizh_juz30`→`tahfizh_juz_30`; TPQ jilid HANYA `doa_harian`+`surat_pendek`. TANPA rules baru (`rapor_semester` sudah `signedIn` + butuh `santri_id`).
- **Fase 5 (statistik + cetak PDF) — `45caa05`:** `TesKenaikanView` tab **Rekap** (penguji) — `statsRekap` (total/menunggu/lulus/belum/tolak + %kelulusan, hormati filter Lembaga/Jenis/Cari) + tabel per lembaga; cetak **PDF saja**: "Daftar Peserta Tes" (antrianView) + "Rekap Hasil" (riwayatView) via `buildListPdf` + KOP settings.

### B. FIX KARTU KENAIKAN PTPT (preview & override basi) — 2 commit
- **`aedfe7a`:** `NaikKelasView` `schema` kartu diubah dari **ref snapshot** (diisi sekali di `openKartu`) → **computed** `getKartuKenaikanSchema(kartuLembaga.value, settingsStore.settings)`. Preview kini reaktif (PDF `eksporKartuPdf` pakai `schema.value` yang SAMA).
- **`63a92a8`:** `utils/kenaikan.js getKartuKenaikanSchema` — GUARD: override PTPT dianggap valid HANYA bila tiap kelas ≥5 juz & total ≥30 (`_isValidPtptOverride`); selainnya **abaikan override, pakai `DEFAULT_KARTU_KENAIKAN_PTPT`** (5 juz/kelas, 30 juz). Membenahi preview + PDF kartu + `resolveKenaikanSchemaPath` sekaligus, tak perlu reset manual.

### C. ⏳ BELUM TUNTAS (AKTIF) — kartu PTPT preview MASIH 4 juz di app LIVE kyai
- Status terakhir: kyai sudah **Reset Default → Simpan Schema** (editor menampilkan draft 5 juz) + **deploy** (`firebase:deploy` MEM-BUILD vue, `deploy-minified.cjs` L58 `npm run build --prefix vue-app`) + **hard reload** — TAPI kartu santri PTPT **masih 4 juz**.
- LOGIKA KUNCI: editor menampilkan **`schemaDraft`** (draft lokal hasil Reset) — TAK membuktikan data tersimpan. Kartu baca `getKartuKenaikanSchema('PTPT')`. **Kalau guard `63a92a8` benar-benar aktif, kartu PASTI 5 juz.** Masih 4 juz ⇒ **guard belum jalan di bundle yang dijalankan device** ⇒ tersangka utama **service worker PWA menyajikan bundle LAMA** (hard reload TIDAK meng-update SW yang "waiting"). Commit2 ini kemungkinan **belum benar-benar aktif** walau "sudah deploy".
- LANGKAH BERIKUTNYA (belum dikonfirmasi kyai): (1) Force SW update — DevTools → Application → Service Workers → centang "Update on reload" + **Unregister** + Storage → **Clear site data** + **tutup SEMUA tab** app lalu buka lagi. (2) Kalau masih 4 juz → inspeksi app live langsung via **Claude in Chrome** (firebase `db` TIDAK di-expose ke window, jadi tak bisa via console snippet biasa). (3) Pertimbangkan **bump SW_VERSION** tiap deploy (aturan §lama yg terlewat) supaya cache ke-bust otomatis.

### D. DISKUSI (belum dikode)
- **Absensi guru — absen HP geofence** ("hanya bisa absen di area pondok"): konsep + mockup + daftar setting sudah dibahas (fokus anti-curang vs fingerprint: GPS+anti-mock + WiFi BSSID/QR dinamis + device-binding + jendela jam). Rekomendasi rilis bertahap: izin/sakit (✅ DONE) → geofence+device-binding → WiFi/QR. BELUM dibangun.
- **Item 2 AGENDA — Diskusi DB Firestore vs "SQL Connect"** (Firebase Data Connect / Cloud SQL): kyai minta PALING AKHIR setelah semua task lain. BELUM dibahas.

### E. GAYA (feedback kyai 14 Jun) — JANGAN pakai kata "Saya" di label/menu
- Kurang modern. Hindari "Kehadiran Saya", "Santri Kelas Saya", "Pengajuan Saya", "Data Saya", dll → pakai kata benda netral ("Kehadiran", "Izin & Sakit", "Data Pribadi"). Sudah diterapkan di PersonalView ("Kehadiran", "Untuk Anda"). Sisa label "Saya" lama tersebar di app = kandidat cleanup batch.

### STATUS DEPLOY (per akhir sesi)
...
## SESI v.101 LANJUTAN (15 Jun 2026) — AUDIT MENYELURUH + SENTRY + PENSIUN LEGACY + SEO/AEO + FIREBASE 12 + vc101

> Sesi Claude Code (Opus 4.8). Branch `feature/vue-migration`, HEAD `8b9d680`, SUDAH push origin. Web SUDAH deploy (Sentry + legacy redirect + SEO + rapor v.101 + Firebase 12). ⏳ AAB vc101 BELUM di-build (kyai: `npm run build:aab`).

### AUDIT 6-POIN (permintaan kyai) — temuan
1. **App/topologi:** Live = Vue `ammuonline.web.app` (public/vue) + PSB `ammuonline-psb` (public/psb) + legacy `portal-mambaul-ulum` (public/, di-SKIP deploy tapi MASIH hidup). RBAC matang. Sisa keamanan lama: guru/santri `read:true` + password plaintext (sudah diketahui, lihat [[project_security_findings]]).
2. **Tampilan modern:** design-token matang (main.css light+dark), ribbon Office desktop, mobile shell lengkap → skor tinggi.
3. **Framework:** Vue 3.5 / Vite 6 / Pinia 2 / Router 4 (hash mode) — modern. **Firebase 10 tertinggal → diupgrade ke 12.** Capacitor 8. Root `package.json` masih nyantol Capacitor ^6 + deskripsi "legacy HTML5" (artefak legacy, sumber kebenaran app = `vue-app/package.json`).
4. **UI/UX dark/responsif:** **TAK ada gap dark-mode.** App pakai 4 mekanisme tema, SEMUA dark-aware: Tailwind `dark:`, CSS-var token (`main.css` `.dark`), scoped `:global(.dark)`/`.dark-mode` (PostCard/ModalPOS), ribbon.css `rb-*` (shell Electron). Grep `dark:` awal over-count → false positive. Responsif 75/107 komponen + safe-area + bottom-nav + pull-to-refresh.
5. **SEO-AEO:** dulu NOL (hash routing + tanpa robots/sitemap/OG/JSON-LD) → diperbaiki (lihat commit).
6. **Sentry "hanya site legacy":** akar = (A) app Vue tak pernah lapor — DSN HANYA dari Firestore `settings/general.sentryDsn` yg KOSONG → `initSentry()` return; (B) legacy MASIH hidup + DSN **hardcoded** `public/index.html:14924` → trafik sisa (PWA penguji ter-cache, crawler) terus kirim event.

### COMMIT (urut, di atas `8f53aab`)
- `d3b9cbb` fix(sentry): DSN `VITE_SENTRY_DSN` > default project, Firestore override; timeout CDN 3→5s. DSN browser bukan rahasia (sama project legacy, dibedakan via `release`/`environment`).
- `6b9befd` chore(legacy): `firebase.json` target `legacy` `public` → `legacy-redirect/` (index.html bersih cache + unregister SW → redirect ammuonline; `sw.js` + `firebase-messaging-sw.js` self-destruct utk PWA penguji). Site TIDAK di-disable (jaga OAuth `/__/auth/*`). Inert utk pipeline normal (deploy hanya hosting:main,psb). Deploy SEKALI: `firebase deploy --only hosting:legacy`.
- `f00414f` feat(seo): OG + Twitter + canonical + JSON-LD `EducationalOrganization` + robots.txt + sitemap.xml (vue-app + vue-app-psb).
- `78063b7` chore(repo): untrack `vue-app-psb/dist`, ignore scratch tmp_recovery, hapus `PROJECT_KNOWLEDGE_BASE.md` (underscore, deprecated).
- `ea09521` + `b7752de` feat(v.101): rapor PPPH + identitas Qiraati (kerjaan kyai sesi lain, deployed-tapi-uncommitted → disimpan) + artefak public/psb.
- `ab94b28` chore(deps): **Firebase JS 10.14.1 → 12.14.0** (vue-app + PSB), NOL perubahan source (import modular stabil). Build 2 app exit 0. **Smoke-test live (Claude Preview) LOLOS:** boot/init/App Check token/Sentry/Firestore-read (branding)/auth-lookup `findUserByLogin` (login palsu → "user tidak ditemukan" rapi), NOL console error. Merge fast-forward ke `feature/vue-migration`.
- `123563b` feat(seo): JSON-LD lengkap — address (Jl. Kolonel Sugiono No.112, Panjunan, Waru, Sidoarjo, Jawa Timur 61256), telephone +62-31-8674713, contactPoint +62-818-0877-1000, email ppmupanjunan@gmail.com, url **almanshur.id**, sameAs IG/TikTok/YT **@almanshurtv**. Rich Results Test: Organization valid (LocalBusiness "non-kritis" = recommended fields, wajar).
- `84511c5` chore(repo): untrack `public/psb` (artefak deploy, regenerated Step 0d).
- `8b9d680` chore(release): **bump vc100 → vc101 / v.101.0626** — build.gradle (versionCode 101 + versionName), `vue-app/package.json`, `electron/package.json`, `main.js` Sentry release `@101.0626`, footer Bantuan/Dashboard/Login/PpdbAdmin. Build exit 0; footer terverifikasi render v.101.0626.

### PENDING / FOLLOW-UP
- **AAB vc101**: `npm run build:aab` (kyai) → bawa fix Sentry + rapor v.101 + Firebase 12 ke app Android terinstal. Upload Play Console (vc101 > vc100, diterima).
- **App Check Play Integrity native**: kini DIMUNGKINKAN (firebase 12) via `@capacitor-firebase/app-check` + `CustomProvider`; `firebase.js` BELUM diubah (follow-up native + tes device). App Check masih Monitoring nunggu penguji ke vc101.
- **`muassis.png`** root untracked tapi dirujuk 8 util (kop/rapor/struk) — keputusan kyai lokasi/commit.
- JSON-LD opsional: tambah `image`/`geo`/jam buka utk bersihkan warning LocalBusiness.

### MEMORY: `project_audit_v100h` (di-update sepanjang sesi) + MEMORY.md pointer.


---

## SESI v.101 UI POLISH (16 Jun 2026, Claude Code Opus 4.8) — DIAGNOSA AUTH + HAPUS PLACEHOLDER + UANG SAKU + PROFIL — RECAP TERBARU

> Branch `feature/vue-migration`, HEAD `3df8f03`, SUDAH push origin. Web SUDAH deploy 2× (ammuonline + psb). versionCode **TETAP vc101** (web-only, TIDAK bump). Permintaan kyai bertahap: baca PKB + periksa gambar console → 4 tweak → hapus placeholder app-wide.

### A. DIAGNOSA CONSOLE ELECTRON (gambar kyai) — TANPA fix kode
- **Gejala** (DevTools app Electron `app.asar/.../index.html`): App Check debug token (info, Monitoring), `securetoken.googleapis.com 400`, 10× `[subscribeColl] <coll> error: FirebaseError: Missing or insufficient permissions` (notif_prestasi, tes_kenaikan, riwayat_kenaikan, keuangan_*, supervisi_catatan, audit_log), `Backup audit_log gagal: permission-denied`. Sentry initialized OK.
- **Akar (1 sebab, bukan 10):** `securetoken 400` = endpoint refresh-token Firebase Auth menolak token (kemungkinan sisa Anonymous yg di-DISABLE v.100f / token kadaluarsa) → ID token tak bisa diperbarui → `request.auth` **NULL** → SEMUA rule `signedIn()` (read+write) gagal. 10 subscribeColl + backup audit_log = gejala turunan.
- **Kenapa UI tetap "login":** `sesiAktif` di-restore dari localStorage (`initAuth` → `restoreAdminSesiFromStorage` + `_loadFullSesi`), bukan dari status Auth asli → `isLoggedIn=true` walau sesi Firebase mati.
- **FIX:** admin **Logout → Login 1×** (sesi Firebase Auth asli baru `adminmu@ammu.local`). Bandel → DevTools Application → Clear site data → tutup semua window → login. Sesuai [[project_auth_domain_ammu]].

### B. 4 TWEAK (commit `5d5026f`)
1. **Form PSB — SEMUA placeholder dikosongkan** (12 field: nama, NIK, KK, tempat lahir, asal sekolah, alamat, TTL+No.HP ayah/ibu, WA wali, nama wali). `vue-app-psb/src/views/PsbFormView.vue` (LF-preserving, hapus baris attribute via skrip).
2. **Uang Saku → santri ma'had saja.** Sesi santri kini bawa `is_mukim: s.is_mukim === true` di `stores/auth.js` (login + loadSesiFromUser). `composables/useMenus.js`: menu "Uang Saku Saya" dapat flag `santriMukimOnly:true` + gate `if (m.santriMukimOnly && !auth.sesiAktif?.is_mukim) return false`. Halaman admin Uang Saku SUDAH filter `is_mukim` sebelumnya (`TabunganView.vue:861`). ⚠️ santri yg sedang login (sesi localStorage lama) perlu **logout+login** agar `is_mukim` terisi (sementara menu hilang utk santri mukim sampai login ulang — sembuh otomatis).
3. **ProfilGuru** `jabatanHeader`: kalau `lembaga` kosong → tampil jabatan saja (tak lagi "Jabatan - -").
4. **Opsi pendidikan "D4 / S1" → "S1"** di `PENDIDIKAN_OPTS` ProfilGuru + ProfilSantri. (PSB pakai skema BEDA: "Diploma (D1-D4)" + "Sarjana (S1)", TIDAK diubah.)

### C. HAPUS PLACEHOLDER FIELD FORM SELURUH APP (commit `3df8f03`)
- Permintaan kyai: "placeholder di tempat lain juga hapus". Konfirmasi scope → **field form saja** (search/login dipertahankan).
- **Removed 150** static `placeholder="..."` di **33 file** vue-app/src. **Kept 39** static + 11 `:placeholder` dinamis.
- **Aturan KEEP** (skrip PowerShell, regex `\s*(?<!:)placeholder="([^"]*)"` + MatchEvaluator, per-file EOL-preserving):
  - value mengandung "cari" (case-insens) → search/filter, **termasuk "🔍 Cari…"** (MultiSelectGuru* yg lolos filter "diawali Cari").
  - value diawali "Ketik nama" → typeahead (Bisyaroh/Tabungan/LembagaForm).
  - file `UiInput.vue` (prop pass-through) + `LoginView.vue` (login) di-SKIP total.
  - `:placeholder` dinamis (nominal default, rentang `0–MAX`, KOP default di RaporView/PengaturanKeuangan/MasterData/LembagaDetail/TesKenaikan) TAK disentuh.
- Verifikasi: vite build vue-app + psb **exit 0**; scan EOL → **NOL file MIXED** (LF/CRLF per-file utuh).

### FILE DIUBAH (ringkas)
- `vue-app-psb/src/views/PsbFormView.vue` (B1)
- `vue-app/src/stores/auth.js` (B2 is_mukim ×2) + `vue-app/src/composables/useMenus.js` (B2 gate)
- `vue-app/src/views/profil/ProfilGuru.vue` (B3+B4) + `ProfilSantri.vue` (B4)
- 33 file `vue-app/src/**/*.vue` (C — hapus placeholder)

### GOTCHA / CATATAN (utk sesi baru)
- **Bulk edit aman utk file campur EOL:** `[IO.File]::ReadAllText` → `[regex]::Replace` MatchEvaluator (return `$m.Value` utk KEEP, `''` utk hapus; `\s*` di depan pattern ikut serap newline+indent supaya tak ada baris kosong sisa) → `WriteAllText` UTF8-no-BOM. Regex TIDAK menyisipkan newline → EOL per-file otomatis terjaga. Verifikasi: scan CRLF-vs-loneLF, pastikan tak ada MIXED.
- Edit tool harness ternyata **preserve CRLF** otomatis (tak perlu normalisasi manual); tapi utk bulk lintas-file skrip lebih cepat dari puluhan Edit.
- `npm run firebase:deploy` (deploy-minified.cjs) build+deploy **2 site sekaligus** (hosting:main `public/vue` + hosting:psb `public/psb`) + firestore:rules (skip kalau tak berubah) → 1 perintah cukup utk perubahan vue-app MAUPUN PSB.
- versionCode TIDAK di-bump (web-only). Perubahan UI ini masuk app Android/Electron HANYA saat rebuild AAB vc101 (`npm run build:aab`) / Electron.

### MEMORY: tak ada file baru (diagnosa auth tercover [[project_auth_domain_ammu]]; perubahan kode tercatat di git).
