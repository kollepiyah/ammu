...
>
> ### ✅ FOLLOW-UP — SHIFT KERJA PEGAWAI utk DUAL-ROLE (11 Jun 2026, sesi Claude Code) — commit menyusul
> Temuan kyai: tipe `pegawai_guru` (dual-role) tak bisa pilih shift PEGAWAI (field `shift` = "Shift Qiraati"/mengajar; blok "Shift Kerja" hanya utk pegawai murni) → di absensi kolom Peg.Pagi & Peg.Sore selalu dua-duanya. Fix (Vue murni, tanpa bump vc): field baru `shift_pegawai` (`useGuruForm`), blok form "Shift Kerja (Pegawai)" `v-if tipe==='pegawai_guru'` (`GuruFormView`), gating kolom pegawai di `AbsensiGuruView.shiftsForGuru` (pegawai murni→`g.shift`, dual→`g.shift_pegawai`; default `pagi_sore` → data lama tak regresi). Verify `_run_vite.cmd` exit 0.
>
> ### ✅ AUDIT v.99 + FIX BAYAR-DI-MUKA + GUARD FAN-OUT WA (11 Jun 2026, sesi Claude Code) — commit menyusul
> Audit read-only menyeluruh atas paket v.99 menggantung (keamanan/RBAC/anti-bocor/integritas/push/parity/versi). **Verdict sehat:** anti-bocor v.98 utuh (GlobalSearch scoped, ProfilDetail `canView`), push K1/K2/K3/S1/S2 benar, bonus shift pegawai (single+bulk) + gating `shiftsForGuru` OK, lembaga canonical OK, versi konsisten vc99 (13 titik §5), Capacitor native (no `server.url`), `notif_queue` read=false. **Konfirmasi kyai:** #1 PII (rule `read:if true` publik) = backlog custom-claims (scoping in-app sudah benar, view-only di guru/Kepala); #3 reshuffle NIS = biarkan (akun belum dibagikan); #4 "edit guru reset password" = **FALSE ALARM** (sudah preserve `old.password` di `useGuruForm.js` L397 + auth pakai Firebase Auth). **2 FIX (approve kyai, functions+web, TANPA AAB & TANPA bump vc):** (A) **bayar-di-muka POS** (`status:'lunas'`) tak lagi memicu notif "Tagihan Baru" palsu — `functions/index.js onTagihanCreated` skip `status==='lunas'` + `useNotifications.getTagihan` filter lunas; (B) **guard fan-out WA lintas-keluarga** — `resolveTokensByTarget` cabang `type:'santri'` saring device ber-WA-sama hanya SE-KELUARGA (`nik_ayah`/`nama_ayah` cocok dgn santri target; flat + nested; tanpa data ayah → token sendiri saja) + helper `_normNama`. File: `functions/index.js`, `vue-app/src/composables/useNotifications.js`. Verify `_run_vite.cmd` exit 0 + `node --check` OK. Deploy: `firebase deploy --only functions` + `npm run firebase:deploy` (gabung paket v.99).
>
> ### ✅ BATCH 16 — AUTO-GENERATE NIG + KEGIATAN PESANTREN ke ribbon Electron (11 Jun 2026, sesi Claude Code) — commit menyusul
> **A. Auto-generate NIG** (analog NIS, kyai). Util baru `utils/nigGenerator.js` (reuse `normDob`/`ddmmyy` dari nisGenerator). Format **NNN+DDMMYY** (9 digit): NNN = urut dari **tanggal_tugas TERLAMA→terbaru**, seri → nama A–Z; DDMMYY = tanggal_tugas. Field `nig` (koleksi `guru`). Dua mode + manual:
> - **IMPOR guru = reshuffle**: pasca `confirmImportGuru` (`GuruView`), `getAll('guru')` → `planRegenerateNig` → `applyNigChanges` (tulis yg berubah + audit_log) → toast.
> - **Guru baru via form = append**: `useGuruForm.save` — guru baru tanpa NIG & ada tanggal_tugas → `nextNigForNew` (max NNN+1), TANPA reshuffle.
> - **Tombol manual + preview** di Audit Data (`MasterDataView`, card indigo): Pratinjau → tabel NIG lama→baru → Terapkan.
> - Guru tanpa tanggal_tugas DILEWATI. NIG = field, bukan kunci lintas-koleksi → aman. (Catatan: digenerate utk SEMUA doc koleksi `guru` yg punya tgl tugas — termasuk pegawai bila ada tgl tugas; bisa di-scope khusus guru bila kyai mau.)
> **B. Kegiatan Pesantren ke ribbon Electron** (kyai: "halaman kegiatan pesantren di Electron ribbon belum ada"). Sebelumnya hanya via Pengaturan Desktop (in-page) — sekarang tombol pita langsung: grup **Pendidikan → Laporan** (ikon megaphone, gate admin), to `/kegiatan-pesantren` (sudah ada di TAB_PATHS pendidikan). Verify `_run_vite.cmd` exit 0. TANPA bump versionCode.
>
> ### ✅ BATCH 15 — IMPOR GOOGLE FORM: scope guru + split guru pagi/sore (11 Jun 2026, sesi Claude Code) — commit menyusul
> Konteks: guru isi data bulanan via Google Form (kolom: Timestamp/Nama Lengkap Guru/Nama Lengkap Santri/JK/Sekolah/PTPT/Juz) — **tanpa NIS** & **nama santri sering tak lengkap**. Keputusan kyai: pendekatan "impor pintar (scope by guru/kelas)", TANPA ubah form.
> 1. **Rekap Bulanan scope-by-guru** (`RekapPrestasiView` `onImportRekap`): tambah alias header `Nama Lengkap Santri`; baca `Nama Lengkap Guru`+`Sekolah`(kelas)+`JK` sbg **PENYARING** (tidak ditulis). Pencocokan baru: NIS → nama persis → **bangun POOL ter-scope** (santri yg `guru_pagi`/`guru_sore`/`guru`-nya cocok guru baris itu (fuzzy ≥0.8) → dipersempit kelas_sekolah/kelas + JK) → `_matchNameInPool` (persis → **awalan/subset token** → fuzzy longgar min 0.72). Di lingkup kecil (~guru sekelas), nama tak lengkap ("Adiba") langsung resolve & anti-ambigu. Tanpa info guru → fallback fuzzy global lama. Badge preview: "Lingkup" (token) / "Mirip" (ejaan) di-highlight amber. **Yang DITULIS (utk lembaga PTPT): Juz → field `juz` ("JUZ N") + kolom `PTPT` → field `kelas`** (klarifikasi kyai: "isi tabel PTPT itu adalah Kelas") — keduanya tampil di preview Nilai Baru/Lama. **`Sekolah` & `JK` TIDAK ditulis** (penyaring scoping saja). ⚠️ Scoping SENGAJA tidak pakai PTPT/kelas (field itu yg di-update — kalau dipakai filter, santri yg naik kelas malah terlewat). Pakai guru + kelas_sekolah(Sekolah) + JK.
> 2. **Assign Guru split pagi/sore** (`KelasGuruView` `onImportAssign`): helper `_splitGuruPair` — sel **"Guru A/Guru B"** → pagi=A, sore=B; **1 nama tanpa "/"** → pagi & sore SAMA (kyai). Prioritas kolom kombinasi (`Guru`/`Guru Ngaji`/`Guru Pagi/Sore`) > kolom terpisah (Guru Pagi & Guru Sore); "A/B" yg nyasar di kolom Guru Pagi juga ke-split. Fuzzy nama santri + preview yg sudah ada tetap jalan.
> Verify `_run_vite.cmd` exit 0. TANPA bump versionCode. Catatan alur: **impor Assign Guru DULU** (biar guru↔santri terisi) baru impor Rekap (scope guru butuh data itu).
>
> ### ✅ BATCH 14 — AUTO-GENERATE NIS + FIX FITUR YATIM ELECTRON (11 Jun 2026, sesi Claude Code) — commit menyusul
> **A. Auto-generate NIS** (kyai: tadinya generate di Excel, mau pindah ke app). Format **NNNN+DDMMYY** (10 digit, mis. `0001120399`): NNNN = nomor urut GLOBAL dari **tgl lahir TERTUA→termuda**, seri tgl sama → **nama A–Z**; DDMMYY = tgl lahir santri itu. Util baru `utils/nisGenerator.js` (`normDob`/`ddmmyy`/`maxRankOf`/`nextNisForNew`/`planRegenerateNis`/`applyNisChanges`). Dua mode (keputusan kyai):
> - **IMPOR = reshuffle**: tiap selesai impor santri (`confirmImportSantri` di `SantriView`), SEMUA santri di-`getAll` → `planRegenerateNis` (re-sort & assign ulang) → `applyNisChanges` (tulis yang berubah saja + 1 entri `audit_log` ringkasan) → toast jumlah berubah/dilewati. Otomatis.
> - **PSB = append (TANPA reshuffle)**: saat konversi pendaftar→santri (`PpdbDetailView`), bila NIS kosong → `nextNisForNew` = (max NNNN +1) + DDMMYY-nya. Kyai: "kalau dari PSB tetap meneruskan, walau lebih tua".
> - **Tombol manual + preview** di Audit Data (`MasterDataView`): "Pratinjau" → kartu hitung (berubah/tetap/tanpa tgl lahir) + tabel NIS lama→baru + "Terapkan (N)". Reuse `applyNisChanges`.
> - Santri **tanpa tgl lahir DILEWATI** (NIS lama dibiarkan). Cakupan = **semua santri (aktif+non-aktif)** → rank stabil. **Aman:** NIS = field; pembayaran/tabungan/POS/notif refer `santri_id` (doc id) → riwayat tak terpengaruh. ⚠️ VA BMT turunan NIS bila `va_bmt` belum terbit — kyai aktifkan VA setelah data lengkap & NIS stabil (VA belum aktif sekarang).
> **B. Fix fitur yatim di Electron** (kyai: "audit data dan beberapa fitur gk terlihat di Electron"). Akar: di Electron tab-bar/header MasterData & beberapa view tak ada tombol pita. Audit menyeluruh (diff route vs ribbon): orphan admin nyata = **Audit Data** (`/master-data?tab=audit`) + **Absensi Guru** (`/absensi-guru`) — keduanya ada di menu web admin tapi tak ada tombol pita. Ditambah ke `useRibbonNav`: Audit Data (grup "Lembaga & Kenaikan", ikon shield, gate superadmin — route butuh super_admin), Absensi Guru (grup "Akademik", ikon clipboard, gate admin). Yang lain (`input-bulanan`/`kegiatan-pesantren`/`rekap-diniyah`/`impor-kalender`) ternyata **terjangkau in-page** dari halaman yang sudah ada di pita; `capaian-prestasi` = santri-facing (sudah ke-map tab Personal). Verify `_run_vite.cmd` exit 0. TANPA bump versionCode (nunggu tes kyai).
>
> ### ✅ BATCH 13 — TOMBOL AKSI di "Kandidat Nama Mirip" (11 Jun 2026, sesi Claude Code) — commit menyusul
> Permintaan kyai: fuzzy Excel "masih belum bisa full" → panel **Kandidat Nama Mirip** (Master Data → Audit Data → Kesehatan Data) yang tadinya report-only kini bisa **menggabung langsung**. Tiap kartu grup: **checkbox per record** (default tercentang) + tombol **"Gabung yang dicentang"**. Reuse `mergeGroupManual` (`v100_dedupe.js`): gabung ke record TERLENGKAP, sisanya dihapus + backup `audit_log` (recoverable), guard identitas dilewati SADAR. ⚠️ Pakai checkbox (bukan satu tombol "gabung grup") karena fuzzy bisa mengelompokkan **campur** — sebagian orang sama, sebagian beda (mis. "Daffa Ibnu Hafidz" 2012 vs "Moch. Daffa Ibnu Hafidz" 2016 = beda orang) → user lepas centang yang bukan orang sama. Konfirmasi danger menampilkan nama + NIS + tgl lahir tiap record; minimal 2 tercentang. Lookup record mentah via `_santriById`/`_guruById` (Map by id dari raw list). File: `views/MasterDataView.vue` (state `fuzzyChecked`/`fuzzyMerging` + `gabungFuzzy()` + UI). Verify `_run_vite.cmd` exit 0. TANPA bump versionCode / deploy (nunggu tes kyai di app).
>
...
> 7. **Impor Rekap Prestasi pakai PREVIEW dulu** (kyai: jangan langsung timpa, spt impor santri/guru). `onImportRekap` (`RekapPrestasiView`) kini FASE 1 = bangun `importRekapPreview` (tanpa tulis): per baris ditandai **Baru** (santri belum punya nilai prestasi) / **Update** (menimpa nilai lama) / **Lewati** (kolom prestasi kosong) / **Tak ditemukan**, + cara cocok (NIS/Nama/**Mirip**=fuzzy), + "Nilai Baru" vs "Nilai Lama". Modal preview (mirip impor Santri): kartu hitung + tabel + tombol **Konfirmasi (N)**. FASE 2 `confirmImportRekap` baru tulis `updateDoc` + `notif_prestasi` utk baris Baru/Update saja. (Impor **Assign Guru** masih apply langsung — bisa ditambah preview serupa bila kyai mau.)
> 8. **Impor santri: auto-deteksi nama lembaga ke bentuk KANONIK** (kyai: file tulis "PRA PTPT", app pakai "Pra PTPT"). Util baru `canonLembaga()` di `useLembaga.js` (+ `LEMBAGA_CANON_ALL` dari variants `LEMBAGA_GROUPS` = sumber tunggal): cocokkan via kunci ternormalisasi (buang spasi/kapital/tanda) → "PRA PTPT"/"pra-ptpt" → **Pra PTPT**, "tpq pagi" → **TPQ Pagi**, "ppph"/"p3h" → **PPPH**; alias sekolah **SMP/SMA → PKBM** (sub-tier dari kelas), SD → SDI. Tak dikenal → biarkan apa adanya. Dipakai di `onImportSantri` (`SantriView`) utk `lembaga` (Qiraati) & `lembaga_sekolah` (ganti `.toUpperCase()` lama yg bikin "PRA PTPT" ≠ kanonik). Hasil tampil di preview impor santri sebelum konfirmasi. (Impor **Guru** masih `.toUpperCase()` — bisa dipasang `canonLembaga` serupa bila kyai mau.)
> Verify: `_run_vite.cmd` **VITE_EXITCODE=0** (6×: fix 1–3, roll-out GSheet, audit refine, fuzzy impor, preview impor prestasi, canon lembaga impor santri). TANPA bump versionCode / deploy (nunggu hasil tes kyai di app).
>
