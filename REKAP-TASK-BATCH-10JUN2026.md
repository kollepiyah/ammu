...
> **STATUS:** ✅ **SEMUA BATCH 1–8 SELESAI.** Batch 1–6 deployed kyai. **Batch 7** commit `413cbf7` (T20 Google login PERLU VERIFIKASI DEVICE kyai pasca rebuild Electron). **T22+P5/P6/P7** commit `23d7c9a`. **Batch 8a (audit menu overhaul)** commit `5bca2ac`. **Batch 8 FINAL (10 Jun 2026, sesi Claude Code): re-audit regresi LOLOS + P9 + BUMP vc99→100 (13 titik §5)** — lihat blok "BATCH 8 FINAL" di bawah. **PENDING kyai = SHIP:** `firebase:deploy` + `build:aab` (vc100) + `electron:publish` + `git push` (rules & functions TIDAK berubah → tak perlu deploy rules/functions). Keputusan kyai: Batch 4 Bisyaroh/Syahriyah = Electron saja. Batch 6: schema sekolah = 1 tanggal naik/kelas + kelulusan; semua step.
>
