...
> **PENDING (jawab dulu, belum eksekusi): Ekspor → Google Sheet via Apps Script.** Keputusan kyai: **HYBRID** (Excel tetap + tambah opsi GSheet utk laporan pilihan) + tujuan = **layout rapi mirip PDF**. Butuh kyai deploy Apps Script Web App (akun Google kyai) → URL+token disimpan di settings. Rancangan menyusul.
>
