...
import { u as pt } from './useToast-BjwjYk11.js'
import { u as ft } from './useConfirm-DjSfx6QZ.js'
import { b as V, a as J, m as kt, t as vt } from './hijri-BQdwk8_X.js'
function ht() {
  const D = Y(),
    b = k([]),
    m = k(!0),
  ...
  function M() {
    try {
      v = ut(
        dt(L, 'kegiatan'),
        (n) => {
          ;((b.value = n.docs.map((r) => ({ id: r.id, ...r.data() }))), (m.value = !1))
        },
        (n) => {
          ;((g.value = n), (m.value = !1))
        }
  ...
  const N = x(() => {
    var r
    const n = (r = D.sesiAktif) == null ? void 0 : r.role
    return n
      ? b.value.filter((i) => {
          const p = i.audience || 'semua'
          return p === 'semua' ||
  ...
  async function $(n) {
    ...
    const r = n.id || `kg_${Date.now()}`,
      i = {
        ...
        dibuat_oleh: ((p = D.sesiAktif) == null ? void 0 : p.nama) || 'Admin',
        timestamp: new Date().toISOString()
      }
    return (await gt(z(L, 'kegiatan', r), i), i)
  }
  async function h(n) {
    await ct(z(L, 'kegiatan', n))
  }
  return (
    it(M),
    ot(E),
...
const yt = ['Legi', 'Pahing', 'Pon', 'Wage', 'Kliwon'],
  wt = new Date(1900, 0, 1),
  _t = 1
function Dt(D) {
  const b = new Date(D),
    m = Math.floor((b - wt) / (1e3 * 60 * 60 * 24)),
    g = (((_t + m) % 5) + 5) % 5
  return yt[g]
}
const jt = { class: 'p-4 md:p-6 max-w-6xl mx-auto space-y-4' },
  At = {
    class:
      'bg-white dark:bg-slate-800 rounded-2xl p-4 md:p-5 border border-slate-200 dark:border-slate-700 shadow-sm'
  },
  St = { class: 'flex items-center justify-between gap-3 flex-wrap' },
  ...
  Bt = { class: 'grid grid-cols-7 gap-1.5 mb-2' },
  ...
  Ht = { key: 0, class: 'w-1.5 h-1.5 bg-cyan-500 rounded-full mt-0.5 mx-auto' },
  Ut = {
    class:
      'bg-white dark:bg-slate-800 rounded-2xl p-4 md:p-5 border border-slate-200 dark:border-slate-700 shadow-sm'
  },
  It = { key: 0, class: 'text-xs text-slate-400 italic text-center py-4' },
  Lt = { key: 1, class: 'space-y-2' },
  Rt = ['onClick'],
  Vt = { class: 'flex items-start justify-between gap-2' },
  ...
  Yt = { class: 'text-base font-black text-slate-800 dark:text-white mb-4' },
  Gt = { class: 'space-y-3' },
  Wt = { class: 'grid grid-cols-2 gap-3' },
  Qt = {
    class:
  ...
  se = {
    ...
    setup(D) {
      const b = Y(),
        ...
        g = pt(),
        v = ft(),
        { kegiatanRelevan: M, simpanKegiatan: E, hapusKegiatan: N } = ht(),
        $ = x(() => {
          var s
          return (
            b.isAdmin || ((s = b.cekHakAkses) == null ? void 0 : s.call(b, 'post_profil_pesantren'))
          )
        }),
        h = x(() => {
          var s
          return parseInt(((s = m.settings) == null ? void 0 : s.kalibrasiHijri) || 0) || 0
        }),
        n = new Date(),
        r = k(n.getFullYear()),
        i = k(n.getMonth()),
        ...
        F = x(() => p[i.value]),
        G = x(() => new Date(r.value, i.value, 1).getDay()),
        W = x(() => new Date(r.value, i.value + 1, 0).getDate())
      function P(s) {
      ...
      const C = P(new Date()),
        H = x(() => {
          const s = `${r.value}-${String(i.value + 1).padStart(2, '0')}`
          return M.value
            .filter((t) => {
              const a = (t.tgl_mulai || '').substring(0, 7),
                o = (t.tgl_akhir || t.tgl_mulai || '').substring(0, 7)
              return a <= s && o >= s
            })
        ...
        Q = x(() => {
          const s = []
          for (let t = 1; t <= W.value; t++) {
            const a = new Date(r.value, i.value, t),
              o = P(a),
              f = kt(a, h.value),
              U = H.value.some((I) => o >= I.tgl_mulai && o <= (I.tgl_akhir || I.tgl_mulai))
            s.push({
              day: t,
              dateStr: o,
              isToday: o === C,
        ...
        X = x(() => {
          const s = new Date(r.value, i.value, 1),
            t = new Date(r.value, i.value + 1, 0)
          return `${J(s, h.value)} — ${J(t, h.value)}`
        })
      function Z() {
        i.value === 0 ? ((i.value = 11), r.value--) : i.value--
      }
      function tt() {
        i.value === 11 ? ((i.value = 0), r.value++) : i.value++
      }
      function et() {
        const s = new Date()
        ;((r.value = s.getFullYear()), (i.value = s.getMonth()))
      }
      const at = { semua: '👥 Semua', guru: '🧑‍🏫 Guru', santri: '🧑‍🎓 Santri' }
      function st(s) {
        return at[s || 'semua']
      }
      function lt(s, t) {
        const a = V(new Date(s))
        return !t || t === s ? a : `${a} s/d ${V(new Date(t))}`
      }
      function q(s) {
        const t = M.value.filter((f) => s >= f.tgl_mulai && s <= (f.tgl_akhir || f.tgl_mulai)),
          a = V(new Date(s)),
          o = J(new Date(s), h.value)
        if (t.length === 0) g.info(`${a} (${o}) — Tidak ada kegiatan.`)
        else {
          const f = t.map((U) => `• ${U.judul}`).join(`
`)
          alert(`${a}
${o}
...
${f}`)
      ...
      const y = k(!1),
        w = k(!1),
        l = mt({ id: '', judul: '', tgl_mulai: C, tgl_akhir: '', audience: 'semua', deskripsi: '' })
      function O(s = null) {
        ;(s
          ? ((l.id = s.id),
            (l.judul = s.judul || ''),
            (l.tgl_mulai = s.tgl_mulai || C),
      ...
      async function nt() {
        if (l.tgl_akhir && l.tgl_akhir < l.tgl_mulai) {
          g.warning('Tanggal akhir harus setelah tanggal mulai.')
          return
        }
        w.value = !0
        try {
          ;(await E({ ...l }), g.success(l.id ? 'Diperbarui' : 'Tersimpan'), (y.value = !1))
        } catch (s) {
          g.error('Error: ' + (s.message || s))
        } finally {
          w.value = !1
        }
      }
      async function rt() {
        if (
          ...
          w.value = !0
          try {
            ;(await N(l.id), g.success('Dihapus'), (y.value = !1))
          } catch (t) {
            g.error('Error: ' + (t.message || t))
          } finally {
            w.value = !1
          }
        }
          ...
          e('div', Tt, [
            e('div', Bt, [
              ...
              u(
                T,
                null,
                B(['MIN', 'SEN', 'SEL', 'RAB', 'KAM', 'JUM', 'SAB'], (a, o) =>
                  e(
                    'div',
                    {
                      key: o,
            ...
            e('div', Kt, [
              ...
              u(
                T,
                null,
                B(G.value, (a) => (d(), u('div', { key: `empty-${a}`, class: 'aspect-square' }))),
                128
              )),
              (d(!0),
              u(
                    ...
                    u(
                      ...
                      [
                        e(
                          ...
                          c(a.day) + ' ' + c(F.value.substring(0, 3)),
                          3
                        ),
                        e(
                          'p',
                        ...
                        e('p', Nt, c(a.pasaran), 1),
                        a.hasEvent ? (d(), u('div', Ht)) : j('', !0)
                      ],
                      10,
                      Et
                    )
          ...
          e('div', Ut, [
            t[13] ||
              (t[13] = e(
                ...
                [e('i', { class: 'fas fa-list-ul text-cyan-600 mr-2' }), _('Kegiatan Bulan Ini ')],
                -1
              )),
            H.value.length === 0
              ? (d(),
                u('div', It, [
                  ...(t[12] ||
                    (t[12] = [
                      ...
                      _(' Tidak ada kegiatan bulan ini. ', -1)
                    ]))
                ]))
              : (d(),
                u('div', Lt, [
                      ...
                      (a) => (
                        ...
                        u(
                          'div',
                          {
                            ...
                            onClick: (o) => q(a.tgl_mulai)
                          },
                          [
                            e('div', Vt, [
                              e('div', Jt, [
                                e('p', Ft, c(a.judul), 1),
                                e('p', Pt, c(lt(a.tgl_mulai, a.tgl_akhir)), 1),
                                e(
                                  'p',
                                  qt,
                                  c(st(a.audience)) +
                                    c(a.deskripsi ? ' · ' + a.deskripsi.substring(0, 80) : ''),
                                  1
                                )
                              ]),
                              $.value
          ...
          y.value
            ? (d(),
              u(
                ...
                {
                  key: 0,
                  onClick: t[7] || (t[7] = R((a) => (y.value = !1), ['self'])),
                  class:
                ...
                [
                  e('div', zt, [
                    e(
                      'form',
                      { onSubmit: R(nt, ['prevent']), class: 'p-5' },
                      [
                        e('h3', Yt, [
                          e(
                            'i',
                          ...
                          _(' ' + c(l.id ? 'Edit Kegiatan' : 'Tambah Kegiatan'), 1)
                        ]),
                        e('div', Gt, [
                          e('div', null, [
                            t[14] ||
