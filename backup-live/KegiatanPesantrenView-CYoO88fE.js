...
import { u as bt } from './useToast-BjwjYk11.js'
import { u as kt } from './useExcel-D_0sjauS.js'
...
const ft = { class: 'p-4 md:p-6 max-w-6xl mx-auto space-y-4' },
  ...
  $t = { class: 'flex items-center justify-between mb-3 flex-wrap gap-2' },
  jt = { key: 0, class: 'text-xs text-slate-400 italic text-center py-8' },
  Ct = { key: 1, class: 'space-y-2' },
  ...
  It = ['value'],
  Vt = ['value'],
  Rt = { class: 'text-[11px] text-slate-500 dark:text-slate-400 italic mb-3' },
  Ut = { key: 0, class: 'text-xs text-slate-400 italic text-center py-8' },
  Et = { key: 1, class: 'overflow-x-auto' },
  Pt = { class: 'w-full text-xs' },
  ...
  ce = {
    ...
    setup(de) {
      const { importFile: et } = kt(),
        ...
        g = tt(() => {
          var t
          const l = (t = at.settings) == null ? void 0 : t.master_kegiatan
          return Array.isArray(l) ? l : []
        }),
        G = tt(() =>
          z.value
            .filter((l) => l.aktif !== !1 && l.is_mukim === !0)
            .sort((l, t) => String(l.nama || '').localeCompare(String(t.nama || '')))
        ),
        j = b(!1),
        D = b(!1),
        n = gt({ idx: null, nama: '', jam: '', deskripsi: '' })
      function q(l = null, t = null) {
        ;(l
          ? ((n.idx = t),
            (n.nama = l.nama || ''),
            (n.jam = l.jam || ''),
            (n.deskripsi = l.deskripsi || ''))
      ...
      async function rt() {
        ...
        try {
          ...
          ;(n.idx !== null ? (l[n.idx] = t) : l.push(t),
            await V(M(_, 'settings', 'general'), { master_kegiatan: l }, { merge: !0 }),
            await V(M(_, 'settings', 'web'), { master_kegiatan: l }, { merge: !0 }),
            p.success(n.idx !== null ? 'Diperbarui' : 'Tersimpan'),
            (j.value = !1))
        } catch (l) {
          p.error('Error: ' + (l.message || l))
        } finally {
          D.value = !1
        }
      }
      async function lt(l) {
        if (confirm(`Hapus kegiatan "${g.value[l].nama}"?`))
          try {
            const t = [...g.value]
            ;(t.splice(l, 1),
              await V(M(_, 'settings', 'general'), { master_kegiatan: t }, { merge: !0 }),
              await V(M(_, 'settings', 'web'), { master_kegiatan: t }, { merge: !0 }),
              p.success('Dihapus'))
          } catch (t) {
      ...
      async function nt(l) {
        var a
        const t = (a = l.target.files) == null ? void 0 : a[0]
        if (t) {
          ;((E.value = t.name), (A.value = !0), (m.value = { ok: 0, error: 0, errors: [] }))
          try {
            const d = await et(t)
            if (!d.length) {
            ...
            for (const i of d)
              try {
                ...
                if (!T || (!L && !Y)) {
                  ;(c++, C.push(`Row ${y + c}: tanggal/santri kosong`))
                  continue
                }
                let B = String(T)
                const I = B.match(/^(\d{2})[-\/](\d{2})[-\/](\d{4})$/)
                I && (B = `${I[3]}-${I[2]}-${I[1]}`)
                let x = null
                if (
                  (L &&
                    (x = X.find((w) => String(w.fingerprint_id || w.fp_id || '') === String(L))),
                  !x &&
                    Y &&
                    (x = X.find(
                      (w) =>
                  ...
                  ;(c++, C.push(`Row: santri ${L || Y} tidak ditemukan`))
                  continue
                }
                if (x.is_mukim !== !0) {
                  ;(c++, C.push(`Row: ${x.nama} bukan mukim, skip`))
                  continue
                }
                ...
                const dt =
                    h
                      ...
                      .replace(/[^a-z0-9]+/g, '_')
                      .replace(/^_+|_+$/g, '') || 'keg',
                  Q = `keg_${x.id}_${B}_${dt}`
                ;(F.set(M(_, 'absensi_kegiatan', Q), {
                  id: Q,
                  santri_id: x.id,
                  santri_nama: x.nama,
                  tanggal: B,
                ...
                ;(c++, C.push(`Row error: ${T.message}`))
              }
            ;(y > 0 && (await F.commit()),
              (m.value = { ok: y, error: c, errors: C.slice(0, 5) }),
              y > 0
                ? p.success(`Impor selesai: ${y} OK, ${c} error`)
                : p.error(`Impor gagal: 0 OK, ${c} error. Cek format kolom.`))
          } catch (d) {
            p.error('Parse gagal: ' + (d.message || d))
          } finally {
            ;((A.value = !1), (l.target.value = ''))
          }
        }
      }
      return (l, t) => (
              ...
              f(st, (a) =>
                e(
                  ...
                  [
                    e(
                      'i',
                      { class: J(['fas', a.icon, 'text-base md:text-lg drop-shadow']) },
                      null,
                      2
                    ),
                    e('h3', wt, o(a.name), 1),
                          ...
                          e(
                            ...
                            [
                              ...
                              s(
                                k,
                                null,
                                f(H, (a, d) => e('option', { key: d, value: d }, o(a), 9, It)),
                                64
                              ))
                            ],
                            512
                          ),
                          [[Z, R.value, void 0, { number: !0 }]]
                        ),
                        K(
                          e(
                            'select',
                            {
                              'onUpdate:modelValue': t[2] || (t[2] = (a) => (U.value = a)),
                              class:
                                'text-xs px-3 py-1.5 border border-slate-300 dark:border-slate-600 rounded-lg bg-white dark:bg-slate-900 text-slate-800 dark:text-white'
                            },
                            [
                              ...
                              s(
                                ...
                                f(
                                  ...
                                  (a) => (r(), s('option', { key: a, value: a }, o(a), 9, Vt))
                                ),
                                128
                              ))
                            ],
                    ...
                    e('div', Gt, [
                      ...
                      e('div', qt, [
                        ...
                        e(
                          'button',
                          {
                            onClick: t[3] || (t[3] = (a) => l.$refs.fileInput.click()),
                            class:
                        ...
                        E.value
                          ...
                          : v('', !0),
                        !A.value && (m.value.ok > 0 || m.value.error > 0)
                          ? (r(),
                            s('div', Zt, [
                              e('div', Wt, [
          ...
          j.value
            ? (r(),
              s(
                ...
                {
                  key: 3,
                  onClick: t[8] || (t[8] = W((a) => (j.value = !1), ['self'])),
                  class:
                ...
                [
                  e('div', re, [
                    e(
                      'form',
                      { onSubmit: W(rt, ['prevent']), class: 'p-5' },
                      [
                        e('h3', le, [
                          e(
                            'i',
