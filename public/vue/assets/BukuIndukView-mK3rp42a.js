...
import { u as ie } from './useExcel-CfMuM0d2.js'
import { a as c, c as E } from './format-CpwY2Om2.js'
import { b as de } from './pdfBuilder-C97mFJ0V.js'
import './pdf-CfElz_X6.js'
const ue = { class: 'p-3 md:p-5 max-w-6xl mx-auto space-y-4' },
  pe = {
    key: 0,
    class:
  ...
  ke = { class: 'flex flex-col md:flex-row md:items-center md:justify-between gap-3' },
  ge = ['disabled'],
  xe = { class: 'text-xs text-slate-500 mt-0.5' },
  be = { class: 'flex gap-2 flex-wrap' },
  ce = { class: 'grid grid-cols-3 gap-2 md:gap-3 mt-3' },
  fe = { class: 'bg-emerald-50 border-l-4 border-emerald-500 p-3 rounded-xl' },
  ve = { class: 'text-base md:text-lg font-black text-emerald-800 mt-1' },
  we = { class: 'bg-rose-50 border-l-4 border-rose-500 p-3 rounded-xl' },
  he = { class: 'text-base md:text-lg font-black text-rose-800 mt-1' },
  ye = { class: 'bg-white dark:bg-slate-800 rounded-2xl shadow-2xl w-full max-w-md' },
  _e = { class: 'space-y-3' },
  Se = { class: 'grid grid-cols-2 gap-2' },
  ...
  Ae = { class: 'grid grid-cols-2 md:grid-cols-4 gap-2' },
  Te = ['value'],
  Ue = ['value'],
  ...
  Ie = { class: 'divide-y divide-slate-100 dark:divide-slate-700' },
  De = { class: 'text-[11px] text-slate-500 font-bold whitespace-nowrap block md:inline' },
  $e = { class: 'md:inline' },
  Ve = { class: 'text-sm font-bold text-slate-800 dark:text-white truncate' },
  Pe = { class: 'text-[10px] text-slate-500 mt-0.5' },
  Ee = {
    key: 0,
    class:
      'bg-slate-100 dark:bg-slate-700 text-slate-700 dark:text-slate-300 px-1.5 py-0.5 rounded font-bold'
  },
  Oe = { key: 1, class: 'ml-1 text-slate-400' },
  je = { class: 'mt-1 md:mt-0 md:text-right' },
  Fe = { key: 0, class: 'text-sm font-black text-emerald-700' },
  Ye = { key: 1, class: 'text-slate-300' },
  Ge = { class: 'md:text-right' },
  qe = { key: 0, class: 'text-sm font-black text-rose-700' },
  Re = { key: 1, class: 'text-slate-300' },
    ...
    setup(Je) {
      ...
      async function F() {
        ...
        if (!n.keterangan.trim()) {
          k.warning('Keterangan wajib diisi')
          return
        }
        if (!n.nominal || n.nominal <= 0) {
          k.warning('Nominal harus > 0')
          return
        }
        S.value = !0
        try {
          const a = `bi_${Date.now()}_${Math.random().toString(36).slice(2, 7)}`,
            l = {
              ...
              sumber: 'manual',
              operator:
                ((s = T.sesiAktif) == null ? void 0 : s.nama) ||
                ((e = T.sesiAktif) == null ? void 0 : e.guru) ||
                'Admin',
              createdAt: se()
            }
          ;(n.tipe === 'masuk' ? (l.masuk = l.nominal) : (l.keluar = l.nominal),
            await le(ne(re, 'keuangan_buku_induk', a), l),
            k.success('Transaksi tersimpan'),
            (_.value = !1))
        } catch (a) {
          k.error('Gagal: ' + (a.message || a))
        } finally {
          S.value = !1
        }
      }
      async function Y() {
        try {
          const s = (M == null ? void 0 : M.settings) || {},
            e = {
            ...
            a = (f.value || []).map((d, J) => ({
              no: J + 1,
              tanggal: d.tanggal ? E(d.tanggal) : '',
              keterangan: d.keterangan || d.deskripsi || '',
              tipe: d.tipe || '',
              masuk: d.masuk ? c(d.masuk) : '',
              keluar: d.keluar ? c(d.keluar) : ''
            })),
            l = x.value > 0 ? `${U[x.value - 1]} ${g.value}` : `Tahun ${g.value}`
          ;(await de({
            kind: 'umum',
            orientation: 'l',
            format: 'a4',
            ...
            k.success('PDF buku induk berhasil dibuat'))
        } catch (s) {
          k.error('Gagal cetak: ' + ((s == null ? void 0 : s.message) || s))
        }
      }
      const G = A(() => {
          const s = T.sesiAktif
      ...
      async function z() {
        if (!w.value) {
          w.value = !0
          try {
            const e = (f.value || C.value || []).map((l, d) => ({
                no: d + 1,
                tanggal: l.tanggal || '',
                no_struk: l.no_struk || '',
                keterangan: l.keterangan || l.deskripsi || '',
                kategori: l.kategori || '',
                tipe: l.tipe || (Number(l.masuk) > 0 ? 'Masuk' : 'Keluar'),
              ...
              o(
                ...
                [
                  t('div', me, [
                    ...
                    t('div', ce, [
                      t('div', fe, [
                        ...
                        t('p', ve, i(h(c)(v.value.pemasukan)), 1)
                      ]),
                      t('div', we, [
                        e[18] ||
                          (e[18] = t(
                        ...
                        t('p', he, i(h(c)(v.value.pengeluaran)), 1)
                      ]),
                      t(
                        'div',
                        {
                    ...
                    _.value
                      ? (r(),
                        o(
                          ...
                          {
                            key: 0,
                            onClick: e[8] || (e[8] = P((a) => (_.value = !1), ['self'])),
                            class:
                          ...
                          [
                            t('div', ye, [
                              t(
                                'form',
                                { onSubmit: P(F, ['prevent']), class: 'p-5' },
                                [
                                  e[26] ||
                                    (e[26] = t(
                                      'h3',
                    ...
                    t('div', Ae, [
                        ...
                        t(
                          ...
                          [
                            ...
                            o(
                              ...
                              $(
                                q.value,
                                (a) => (r(), o('option', { key: a, value: a }, i(a), 9, Te))
                              ),
                              128
                            ))
                          ],
                        ...
                        t(
                          ...
                          [
                            ...
                            o(
                              L,
                              null,
                              $(U, (a, l) => t('option', { key: a, value: l + 1 }, i(a), 9, Ue)),
                              64
                            ))
                          ],
                          512
                      ...
                      m(
                        t(
                          'select',
                          {
                            'onUpdate:modelValue': e[11] || (e[11] = (a) => (y.value = a)),
                            class:
                              'px-3 py-2.5 text-sm rounded-xl border border-slate-300 bg-white focus:ring-2 focus:ring-indigo-500 outline-none'
                          },
                          [
                      ...
                      m(
                        t(
                          'input',
                          {
                            'onUpdate:modelValue': e[12] || (e[12] = (a) => (B.value = a)),
                            type: 'text',
                              ...
                              $(
                                ...
                                (a) => (
                                  ...
                                  o(
                                    ...
                                    [
                                      t('span', De, i(h(E)(a.tanggal)), 1),
                                      t('div', $e, [
                                        t('p', Ve, i(a.keterangan || '-'), 1),
                                        t('p', Pe, [
                                          a.kategori
                                            ? (r(), o('span', Ee, i(a.kategori), 1))
                                            : I('', !0),
                                          a.ref_id
                                            ? (r(), o('span', Oe, '· #' + i(a.ref_id), 1))
                                            : I('', !0)
                                        ])
                                      ]),
                                      t('div', je, [
                                        a.tipe === 'masuk' || Number(a.masuk) > 0
                                          ? (r(), o('span', Fe, i(h(c)(a.masuk || a.nominal)), 1))
                                          : (r(), o('span', Ye, '—'))
                                      ]),
                                      t('div', Ge, [
                                        a.tipe === 'keluar' || Number(a.keluar) > 0
                                          ? (r(), o('span', qe, i(h(c)(a.keluar || a.nominal)), 1))
                                          : (r(), o('span', Re, '—'))
                                      ])
                                    ]
                                  )
                  ...
                  t('p', ze, [
                    ...
                    p(i(f.value.length) + ' transaksi · Vue 3 · Phase 5.14 ', 1)
                  ])
                ],
                64
              ))
