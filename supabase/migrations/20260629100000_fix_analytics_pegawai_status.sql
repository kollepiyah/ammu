-- ============================================================================
-- FIX (29 Jun 2026) — laporan PEGAWAI selalu kosong.
-- analytics_query laporan 'pegawai_per_lembaga' & 'pegawai_per_jabatan' memfilter
--   `where status = 'aktif'` (huruf kecil), padahal data guru memakai 'Aktif' /
--   'Tidak Aktif' (cek useGuruForm default 'Aktif'; resolver login pakai
--   `status is distinct from 'Tidak Aktif'`). Perbandingan teks Postgres
--   case-sensitive → 0 baris → kedua laporan KOSONG walau ada pegawai aktif.
-- Perbaikan: samakan dgn resolver login → `status is distinct from 'Tidak Aktif'`.
-- (re-create FULL fungsi; hanya 2 klausa WHERE pegawai yang berubah.)
-- ============================================================================
...
begin
  ...
  case report
    ...
    -- ===== PEGAWAI ===== (FIX: status case-insensitive — samakan resolver login)
