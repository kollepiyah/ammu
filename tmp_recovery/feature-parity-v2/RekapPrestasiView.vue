<template>
  <div class="p-3 md:p-5 max-w-7xl mx-auto space-y-4">
      ...
      <div class="flex flex-wrap items-start justify-between gap-3">
        <div>
          ...
          <p class="text-[11px] text-slate-500 dark:text-slate-400">
            ...
            Periode: <b class="text-amber-700">{{ bulan }} {{ tahun }}</b>
          </p>
        </div>
        <!-- View mode toggle -->
        <div class="flex gap-1 bg-slate-100 p-1 rounded-xl">
          <button
            ...
            :key="m.id"
            @click="mode = m.id"
            :class="['px-3 py-1.5 text-[11px] font-bold rounded-lg transition', mode === m.id ? 'bg-white shadow-sm text-amber-700' : 'text-slate-500 hover:text-slate-700']"
          >
    ...
    <div class="bg-white dark:bg-slate-800 rounded-2xl p-3 md:p-4 border border-slate-200 dark:border-slate-700 shadow-sm grid grid-cols-2 md:grid-cols-5 gap-2">
      <select v-model="bulan" class="px-3 py-2 text-sm rounded-xl border border-slate-300 bg-white focus:ring-2 focus:ring-amber-500 outline-none">
        <option v-for="b in BULAN_LIST" :key="b" :value="b">{{ b }}</option>
      </select>
      <select v-model="tahun" class="px-3 py-2 text-sm rounded-xl border border-slate-300 bg-white focus:ring-2 focus:ring-amber-500 outline-none">
        <option v-for="t in TAHUN_LIST" :key="t" :value="t">{{ t }}</option>
      </select>
      <select v-model="filterLembaga" class="px-3 py-2 text-sm rounded-xl border border-slate-300 bg-white focus:ring-2 focus:ring-amber-500 outline-none">
        <option value="">Semua lembaga</option>
        <option v-for="l in LEMBAGA_QIRAATI" :key="l" :value="l">{{ l }}</option>
      </select>
      <select v-model="filterKelas" class="px-3 py-2 text-sm rounded-xl border border-slate-300 bg-white focus:ring-2 focus:ring-amber-500 outline-none">
        <option value="">Semua kelas</option>
        <option v-for="k in uniqueKelas" :key="k" :value="k">{{ k }}</option>
      </select>
      <input v-model="search" type="text" placeholder="Cari nama..." class="px-3 py-2 text-sm rounded-xl border border-slate-300 bg-white focus:ring-2 focus:ring-amber-500 outline-none col-span-2 md:col-span-1" />
    </div>
    ...
    <div v-if="mode === 'bulanan'" class="bg-white dark:bg-slate-800 rounded-2xl p-3 md:p-4 border border-slate-200 dark:border-slate-700 shadow-sm flex flex-wrap items-center justify-between gap-2">
      ...
      <div class="flex flex-wrap gap-2">
        ...
        <button @click="simpanRekap()" class="bg-amber-600 hover:bg-amber-700 text-white text-xs font-black py-2 px-4 rounded-lg shadow-md" :disabled="busy">
        ...
        <div class="flex items-center gap-3">
          ...
          <div class="flex-1 min-w-0">
            ...
            <div class="flex flex-wrap gap-1 mt-1">
              <span class="text-[9px] bg-amber-100 text-amber-700 px-1.5 py-0.5 rounded font-bold">{{ countRiwayat(s) }} kenaikan</span>
              <span v-if="s.prestasi_awal" class="text-[9px] bg-blue-100 text-blue-700 px-1.5 py-0.5 rounded font-bold">Awal: {{ s.prestasi_awal }}</span>
              ...
              <span v-if="s.prestasi_total" class="text-[9px] bg-purple-100 text-purple-700 px-1.5 py-0.5 rounded font-bold">Total: {{ s.prestasi_total }}</span>
            </div>
          </div>
          <button @click="toggleExpand(s.id)" class="text-amber-500 hover:bg-amber-50 px-2 py-1 rounded">
            <i :class="['fas', expandedId === s.id ? 'fa-chevron-up' : 'fa-chevron-down']"></i>
          </button>
        </div>
        <div v-if="expandedId === s.id && Array.isArray(s.riwayat) && s.riwayat.length > 0" class="mt-2 pt-2 border-t border-amber-100">
          <p class="text-[10px] font-bold text-amber-700 uppercase mb-1">Riwayat Kenaikan ({{ s.riwayat.length }})</p>
          <div class="space-y-1">
            <div v-for="(r, i) in s.riwayat" :key="i" class="text-[11px] bg-amber-50 rounded px-2 py-1 flex justify-between gap-2">
      ...
      <table class="min-w-full text-xs border-collapse">
        <thead class="bg-slate-100 sticky top-0">
          <tr>
            ...
            <th v-if="hasPTPT" class="py-2 px-2 border border-slate-300 font-black text-center text-[10px] bg-red-50 text-red-900">Juz</th>
            ...
            <th class="py-2 px-2 border border-slate-300 font-black text-center text-[10px] bg-blue-100 text-blue-900">Total</th>
          </tr>
        </thead>
        <tbody>
          <template v-for="grp in groupedBulanan" :key="grp.key">
            ...
            <tr v-for="s in grp.santri" :key="s.id" class="hover:bg-amber-50/30">
              ...
              <td v-if="hasPTPT" class="p-1 border border-slate-300 bg-red-50/40">
                <input v-if="s.lembaga === 'PTPT'" type="number" min="1" max="30" :value="getJuzNum(s)" @input="setJuz(s.id, $event.target.value)" class="w-full text-center bg-red-50 text-red-900 rounded p-1 text-[11px] font-bold border border-red-300 outline-none focus:ring-2 focus:ring-red-400" />
      ...
      <div v-for="lmb in rankingPerLembaga" :key="lmb.lembaga" class="bg-white dark:bg-slate-800 rounded-2xl p-4 border border-slate-200 shadow-sm">
        ...
        <div v-if="lmb.isPTPT" class="grid grid-cols-3 gap-2 mb-3">
          <div class="bg-red-50 border border-red-200 rounded-lg p-2 text-center">
            <p class="text-[9px] font-bold text-red-700 uppercase">Kurang</p>
            <p class="text-lg font-black text-red-800">{{ lmb.kurang }}</p>
            <p class="text-[8px] text-red-600">&lt;5 hal</p>
          </div>
          <div class="bg-yellow-50 border border-yellow-200 rounded-lg p-2 text-center">
            <p class="text-[9px] font-bold text-yellow-700 uppercase">Cukup</p>
            <p class="text-lg font-black text-yellow-800">{{ lmb.cukup }}</p>
            <p class="text-[8px] text-yellow-600">5-9 hal</p>
          </div>
        ...
        <p class="text-[10px] font-black text-slate-600 uppercase mb-2">
          <i class="fas fa-medal text-amber-500 mr-1"></i>Top 5 Prestasi Tertinggi
        </p>
        <ol v-if="lmb.top5.length > 0" class="space-y-1">
          <li v-for="(t, idx) in lmb.top5" :key="t.id || idx" class="flex justify-between items-center bg-slate-50 px-3 py-2 rounded-lg border border-slate-200 text-xs">
            <span class="font-bold text-slate-800 truncate">
              <span class="inline-block w-5 text-amber-600 font-black">{{ idx + 1 }}.</span>{{ t.nama }}
            </span>
  ...
  for (const lembaga of LEMBAGA_QIRAATI) {
    ...
    const scored = list.map((s) => {
      ...
      if (isPTPT) {
        ...
        else if (computed >= 5) color = 'text-yellow-700'
        else if (computed > 0) color = 'text-red-700'
      }
...
function warnaPTPT(s) {
  ...
  const ak = extractNumber(getEdit(s.id, 'akhir') || s.prestasi_akhir)
  const tot = Math.max(0, ak - aw)
  if (tot > 0 && tot < 5) return 'bg-red-100 text-red-900 border-red-400'
  if (tot >= 5 && tot < 10) return 'bg-yellow-100 text-yellow-900 border-yellow-400'
