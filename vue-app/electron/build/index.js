...
const fingerprint_sync_1 = require('./fingerprint-sync')
...
electron_1.ipcMain.handle('theme:set', async (_event, payload) => {
  try {
    ...
    try {
      fs.writeFileSync(
        path.join(electron_1.app.getPath('userData'), 'ammu-theme.json'),
        JSON.stringify({ isDark: !!payload?.isDark })
      )
    } catch (e) {
...
function setupUpdater(win) {
  try {
    ...
    electron_updater_1.autoUpdater.on('update-not-available', (info) =>
      send('update:status', { status: 'none', version: info?.version })
    )
    electron_updater_1.autoUpdater.on('error', (err) =>
      send('update:status', { status: 'error', message: String(err?.message || err) })
    )
...
electron_1.ipcMain.handle('update:info', () => {
  try {
    return {
      ok: true,
      ...
      ;(0, child_process_1.execFile)(
        ...
        (err, _stdout, stderr) => {
          ...
          if (err)
            resolve({
              ok: false,
              error: String(stderr || (err && err.message) || '').trim() || 'Print gagal'
            })
...
electron_1.ipcMain.handle('print:pdf', async (_event, payload) => {
  ...
  try {
    ...
    tmpPdf = path.join(
      os.tmpdir(),
      `ammu_print_${Date.now()}_${Math.random().toString(36).slice(2, 8)}.pdf`
    )
...
electron_1.ipcMain.handle('fingerprint:read-attlog', async (_event, config = {}) => {
  try {
    const rows = await (0, fingerprint_sync_1.readAttLog)(config || {})
...
electron_1.app.whenReady().then(() => {
  ...
  electron_1.protocol.interceptFileProtocol('file', (request, callback) => {
    try {
      ...
      let urlPath = decodeURIComponent(request.url.split('?')[0].split('#')[0]).replace(
        /^file:\/\//,
        ''
      )
      // Windows: "/D:/..." → "D:/..."
  ...
  if (electron_1.app.isPackaged) {
    ...
    electron_updater_1.autoUpdater.checkForUpdatesAndNotify().catch((err) => {
      console.warn(
        '[AutoUpdate] check skip/gagal (abaikan kalau belum publish release):',
        err?.message || err
      )
    })
  }
