...
function createMainWindow() {
  ...
  if (isDev) {
    ...
    mainWindow.loadURL(devUrl).catch((err) => {
      ...
      dialog.showErrorBox('Gagal Load App', `URL: ${devUrl}\nError: ${err.message}`)
    })
  } else {
    ...
    mainWindow.loadFile(APP_HTML_PATH).catch((err) => {
      ...
      dialog.showErrorBox('Gagal Load App', `Path: ${APP_HTML_PATH}\nError: ${err.message}`)
    })
  }
...
ipcMain.handle('theme:set', async (_event, payload: { isDark: boolean }) => {
  try {
...
ipcMain.handle('print:silent', async (event, payload: { html: string; options?: any }) => {
  try {
    ...
    return new Promise((resolve, reject) => {
      printWindow.webContents.print(options, (success: boolean, errorType?: string) => {
        printWindow.close()
        if (success) resolve({ ok: true })
        else reject(new Error(errorType || 'Print gagal'))
      })
    })
  } catch (e: any) {
    return { ok: false, error: e?.message || String(e) }
...
ipcMain.handle('print:struk', async (_event, payload: {
  html: string
  deviceName?: string
    ...
    return new Promise((resolve, reject) => {
      printWindow.webContents.print(
        ...
        (success: boolean, errorType?: string) => {
          printWindow.close()
          if (success) resolve({ ok: true })
          else reject(new Error(errorType || 'Print struk gagal'))
        }
      )
    })
  } catch (e: any) {
...
ipcMain.handle('print:raw', async (_event, payload: { base64: string; deviceName?: string }) => {
  try {
    if (process.platform !== 'win32') {
      return { ok: false, error: 'Raw print hanya didukung di Windows' }
    }
    const buf = Buffer.from((payload && payload.base64) || '', 'base64')
    if (!buf.length) return { ok: false, error: 'Data struk kosong' }
    ...
    return await new Promise((resolve) => {
      execFile(
        ...
        (err, _stdout, stderr) => {
          try { fs.unlinkSync(tmpPrn) } catch (e) { /* ignore */ }
          try { fs.unlinkSync(tmpPs1) } catch (e) { /* ignore */ }
          if (err) resolve({ ok: false, error: String(stderr || (err && err.message) || '').trim() || 'Print gagal' })
...
ipcMain.handle('print:pdf', async (_event, payload: {
  ...
  try {
    ...
    try { printWindow.setPosition(-32000, -32000) } catch {}
    try { printWindow.showInactive() } catch {}
...
ipcMain.handle('print:pdf-preview', async (_event, payload: { pdfBase64: string }) => {
  ...
  protocol.interceptFileProtocol('file', (request, callback) => {
    try {
      ...
      if (driveRooted) {
        ...
        if (fs.existsSync(resolved)) { callback({ path: resolved }); return }
      ...
      callback({ error: -2 } as any) // -2 = FAILED
    }
  })
