    ...
    <div class="grid grid-cols-1 lg:grid-cols-2 gap-3">
      ...
      >
        ...
        <div class="relative h-64">
          <Bar
            v-if="chartGuruKehadiran"
            :data="chartGuruKehadiran"
      ...
      >
        <div class="flex items-center justify-between mb-2">
          ...
          <span class="text-[10px] text-slate-400 font-bold"
            >{{ santriList.length || 0 }} santri</span
          >
        </div>
        <div class="relative h-64">
      ...
      >
        <div class="flex items-center justify-between mb-2">
          ...
          <span class="text-[10px] text-slate-400 font-bold"
            >{{ kegiatanList.length }} kegiatan</span
          >
        </div>
        <div class="relative h-64">
...
onMounted(() => {
  unsubs.push(
    ...
    subscribeColl('kegiatan_pesantren', (docs) => {
      kegiatanList.value = docs
    })
  )
})
onUnmounted(() => {
...
const chartGuruKehadiran = computed(() => {
  ...
  for (const a of absensiGuru.value) {
    ...
    if (idx < 0) continue
    const status = String(a.status || a.keterangan || '')
      .toLowerCase()
      .trim()
