  ...
  <div class="p-3 md:p-5 max-w-5xl mx-auto space-y-4">
    ...
    >
      <h1 class="text-xl md:text-2xl font-black">
        <i class="fas fa-fingerprint text-emerald-500 mr-2"></i>Absensi Guru
      </h1>
    ...
    <div
      class="bg-white dark:bg-slate-800 rounded-2xl p-3 border border-slate-200 dark:border-slate-700 shadow-sm grid grid-cols-2 md:grid-cols-3 gap-2"
    >
      <select
    ...
    <div
      v-else-if="filteredGuru.length === 0"
    ...
    >
      ...
      <p class="text-sm text-slate-500 italic">Tidak ada guru.</p>
    </div>
    <div v-else class="space-y-2">
      <div
        v-for="g in filteredGuru"
        :key="g.id"
        class="bg-white rounded-xl p-3 border border-slate-200 shadow-sm flex items-center gap-3"
      >
        <div
...
onMounted(() => {
  ...
  unsubA = subscribeColl('absensi_guru', (docs) => {
    absensiRaw.value = docs
  })
})
onUnmounted(() => {
  if (unsubG) {
