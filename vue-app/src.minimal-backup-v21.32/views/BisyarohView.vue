<template>
  <div class="p-3 md:p-5 max-w-5xl mx-auto space-y-4">
      ...
      <div class="flex flex-col md:flex-row md:items-center md:justify-between gap-3">
        <div>
          <h1 class="text-xl md:text-2xl font-black">
            <i class="fas fa-hand-holding-usd text-emerald-500 mr-2"></i>Slip Bisyaroh Guru
          </h1>
        ...
        <div class="flex gap-2">
          <div class="px-3 py-1.5 rounded-full bg-emerald-50 border border-emerald-200 text-xs">
            <span class="text-emerald-700 font-bold">{{ totalTakeFmt }}</span>
            <span class="text-slate-500">total take home</span>
          </div>
          <button
            v-if="isFullAccess"
            @click="openModalNew"
          ...
          >
            <i class="fas fa-plus mr-1"></i>Buat Slip
          </button>
        </div>
      </div>
    </div>
    ...
    <div
      class="bg-white dark:bg-slate-800 rounded-2xl p-3 border border-slate-200 dark:border-slate-700 shadow-sm grid grid-cols-1 md:grid-cols-3 gap-2"
    >
      <input
        v-model="search"
        type="text"
    ...
    <div
      v-else-if="filteredGaji.length === 0"
        ...
        <div class="flex items-center gap-3 flex-wrap">
          ...
          <div class="flex-1 min-w-0">
            <p class="text-sm font-bold text-slate-800 truncate">
              {{ getNamaGuruGelar(g.guru_nama || guruNamaById(g.guru_id)) }}
            </p>
            <p class="text-[10px] text-slate-500">
              {{ g.lembaga || '-' }} · {{ g.jabatan || 'Guru' }} · Periode: {{ g.periode }}
            </p>
          </div>
          ...
          <button
            v-if="isFullAccess"
            @click="openEdit(g)"
            class="text-blue-500 hover:bg-blue-50 p-2 rounded"
          >
    ...
    >
      <div class="bg-white rounded-2xl shadow-2xl max-w-md w-full p-5">
        <h3 class="text-base font-black mb-3">
          <i class="fas fa-receipt text-emerald-500 mr-1"></i
          >{{ modalMode === 'edit' ? 'Edit Slip' : 'Buat Slip Baru' }}
        </h3>
        <div class="space-y-2">
          <select
            v-model="modalGuruId"
            :disabled="modalMode === 'edit'"
            class="w-full px-3 py-2 text-sm rounded-xl border border-slate-300 bg-white disabled:opacity-50"
          >
            <option value="">-- Pilih guru --</option>
            <option v-for="g in guruList" :key="g.id" :value="g.id">
              {{ getNamaGuruGelar(g.nama) }}
            </option>
          </select>
          <div class="grid grid-cols-2 gap-2">
            <select
              v-model.number="modalBulan"
              class="px-3 py-2 text-sm rounded-xl border border-slate-300 bg-white"
            >
              <option v-for="(b, i) in NAMA_BULAN" :key="b" :value="i + 1">{{ b }}</option>
            </select>
            <input
              v-model.number="modalTahun"
              type="number"
              min="2024"
              max="2030"
              class="px-3 py-2 text-sm rounded-xl border border-slate-300 bg-white text-center"
            />
          </div>
          <div>
            <label class="block text-xs font-bold text-slate-500 mb-1">Bisyaroh Pokok</label>
            <input
              v-model.number="modalPokok"
              type="number"
              min="0"
              class="w-full px-3 py-2 text-sm rounded-xl border border-slate-300 bg-white text-right"
            />
          </div>
          <div>
            <label class="block text-xs font-bold text-slate-500 mb-1">Bisyaroh Sekolah</label>
            <input
              v-model.number="modalSekolah"
              type="number"
              min="0"
              class="w-full px-3 py-2 text-sm rounded-xl border border-slate-300 bg-white text-right"
            />
          ...
          <div>
            ...
            <input
              v-model.number="modalPotongan"
              type="number"
              min="0"
              class="w-full px-3 py-2 text-sm rounded-xl border border-rose-300 bg-rose-50 text-right"
            />
          </div>
...
const filteredGaji = computed(() => {
  ...
  if (kw)
    list = list.filter((g) =>
      String(g.guru_nama || guruNamaById(g.guru_id))
        .toLowerCase()
        .includes(kw)
    )
...
const uniquePeriode = computed(() =>
  [...new Set(gaji.value.map((g) => g.periode).filter(Boolean))].sort().reverse()
)
const totalTakeFmt = computed(() =>
  fmtRp(filteredGaji.value.reduce((s, g) => s + (Number(g.take_home) || 0), 0))
)
...
const takeHomeCalc = computed(
  () =>
    (Number(modalPokok.value) || 0) +
    (Number(modalSekolah.value) || 0) +
    (Number(modalTambahan.value) || 0) -
    (Number(modalPotongan.value) || 0)
)
...
function openModalNew() {
  ...
  modalPokok.value = 0
  modalSekolah.value = 0
  modalTambahan.value = 0
  modalPotongan.value = 0
...
async function simpanSlip() {
  ...
  try {
    ...
    await setDoc(
      doc(db, 'keuangan_gaji', id),
      {
        id,
        guru_id: Number(modalGuruId.value) || modalGuruId.value,
        guru_nama: guru?.nama || '',
...
async function deleteSlip(g) {
  const ok = await confirmDlg({
    title: 'Hapus slip?',
    message: `Hapus slip ${getNamaGuruGelar(g.guru_nama)} periode ${g.periode}?`,
    confirmText: 'Hapus',
    danger: true
  })
  if (!ok) return
  try {
    await deleteDoc(doc(db, 'keuangan_gaji', String(g.id)))
...
onMounted(() => {
  unsubGaji = subscribeColl('keuangan_gaji', (docs) => {
    gaji.value = docs
    loading.value = false
  })
