  ...
  <div class="p-3 md:p-5 max-w-6xl mx-auto space-y-4">
    ...
    <template v-else>
      ...
      >
        <div class="flex flex-col lg:flex-row lg:items-center lg:justify-between gap-3">
          ...
          <div class="flex flex-wrap gap-2 items-center">
            ...
            <button
              @click="printPage"
              class="h-9 px-3 inline-flex items-center gap-1.5 rounded-xl bg-rose-500 hover:bg-rose-600 text-white text-xs font-bold transition cursor-pointer no-print"
            >
        ...
        >
          ...
          >
            <h3 class="text-base font-black text-slate-800 dark:text-white">
              <i class="fas fa-file-import text-blue-600 mr-2"></i>Preview Impor Guru —
      ...
      >
        ...
        <label
          class="h-8 px-2 inline-flex items-center gap-1 rounded-lg bg-white dark:bg-slate-700 border border-slate-300 dark:border-slate-600 text-[11px]"
        >
          <select
            v-model="bulkRoleSistem"
            class="bg-transparent text-[11px] font-bold text-slate-700 dark:text-slate-200 outline-none cursor-pointer"
          >
          ...
          <button
            @click="bulkSetRole"
            :disabled="bulkSaving"
            class="ml-1 px-2 py-0.5 rounded bg-fuchsia-600 hover:bg-fuchsia-700 disabled:opacity-50 text-white text-[10px] font-bold cursor-pointer"
          >
            Set Role
          </button>
        </label>
      ...
      <div
        ...
        class="bg-white dark:bg-slate-800 rounded-2xl px-4 py-2 border border-slate-200 dark:border-slate-700 shadow-sm flex items-center gap-2 mb-2"
      >
    ...
    for (const r of rows) {
      ...
      allMapped.push({
        ...
        data: {
          ...
          status: statusVal,
          tipe_pegawai: String(_pick(r, 'Tipe Pegawai', 'tipe_pegawai') || 'guru')
            .trim()
            .toLowerCase(),
          shift: String(_pick(r, 'Shift', 'shift') || 'pagi_sore')
            .trim()
            .toLowerCase(),
          role_sistem: String(_pick(r, 'Role Sistem', 'role_sistem') || 'user')
            .trim()
            .toLowerCase(),
          custom_fields: {}
        }
