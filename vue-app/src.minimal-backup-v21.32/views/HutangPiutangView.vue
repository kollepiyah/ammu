  ...
  <div class="p-3 md:p-5 max-w-5xl mx-auto space-y-4">
      ...
      <div class="flex flex-col md:flex-row md:items-center md:justify-between gap-3">
        <div>
          <h1 class="text-xl md:text-2xl font-black">
            <i class="fas fa-balance-scale text-rose-500 mr-2"></i>Hutang Piutang
          </h1>
          <p class="text-xs text-slate-500 mt-0.5">Catatan hutang + piutang pesantren</p>
        </div>
        <button
          @click="openModal()"
          class="bg-rose-600 hover:bg-rose-700 text-white text-xs font-black px-3 py-1.5 rounded-full shadow"
        >
    ...
    <div class="bg-white rounded-2xl border border-slate-200 shadow-sm overflow-hidden">
      <div class="px-4 py-3 border-b border-slate-100 grid grid-cols-2 gap-2">
        <button
          ...
          :class="[
            ...
            filterJenis === '' ? 'bg-rose-600 text-white' : 'bg-slate-100 text-slate-600'
          ]"
        >
          Semua ({{ items.length }})
        </button>
      ...
      <div v-if="filteredItems.length === 0" class="p-10 text-center">
        ...
        <p class="text-sm text-slate-500 italic">Belum ada catatan.</p>
      </div>
        ...
        <div v-for="h in filteredItems" :key="h.id" class="p-3 flex items-center gap-3">
          <div
            :class="[
              ...
              h.jenis === 'piutang' ? 'bg-cyan-500' : 'bg-amber-500'
            ]"
          >
          ...
          <div class="text-right">
            <p
              :class="[
                'text-sm font-black',
                h.jenis === 'piutang' ? 'text-cyan-700' : 'text-amber-700'
              ]"
            >
              {{ fmtRp(h.nominal) }}
            </p>
          ...
          <button @click="deleteItem(h)" class="text-rose-500 hover:bg-rose-50 p-2 rounded">
...
async function toggleLunas(h) {
  try {
    await setDoc(
      doc(db, 'keuangan_hutang_piutang', String(h.id)),
      { lunas: !h.lunas, _last_update: serverTimestamp() },
      { merge: true }
    )
...
async function deleteItem(h) {
  ...
  if (!ok) return
  try {
    await deleteDoc(doc(db, 'keuangan_hutang_piutang', String(h.id)))
