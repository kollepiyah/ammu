  ...
  <div class="p-3 md:p-5 max-w-5xl mx-auto space-y-4">
    ...
    >
      <h1 class="text-xl md:text-2xl font-black">
        <i class="fas fa-edit text-teal-500 mr-2"></i>Input Bulanan
      </h1>
      <p class="text-xs text-slate-500 mt-0.5">Input prestasi/nilai bulanan santri</p>
    </div>
    <div
      class="bg-white dark:bg-slate-800 rounded-2xl p-3 border border-slate-200 dark:border-slate-700 shadow-sm grid grid-cols-1 md:grid-cols-4 gap-2"
    >
      <select
    ...
    >
      ...
      <p class="text-sm text-slate-500 italic">Tidak ada santri.</p>
    </div>
    <div v-else class="space-y-2">
      ...
      >
        <div class="flex items-center gap-3">
          ...
          <input
            v-model="prestasiMap[s.id]"
            type="text"
            placeholder="Nilai/catatan"
            class="w-40 px-2 py-1.5 text-xs border border-slate-300 rounded-lg bg-white"
          />
        </div>
      </div>
      <button
        ...
        @click="simpanBatch"
        :disabled="saving"
        class="w-full px-4 py-2.5 bg-teal-600 hover:bg-teal-700 disabled:opacity-50 text-white font-bold rounded-xl text-sm shadow"
      >
...
const search = ref('')
const prestasiMap = ref({})
const saving = ref(false)
let unsub = null
const uniqueLembaga = computed(() =>
  [...new Set(santriRaw.value.map((s) => s.lembaga).filter(Boolean))].sort()
)
...
async function simpanBatch() {
  ...
  try {
    ...
    for (const sid of Object.keys(prestasiMap.value)) {
      ...
      try {
        await setDoc(
          doc(db, 'prestasi_bulanan', `${periode}_${sid}`),
          { santri_id: sid, periode, nilai, updated_at: serverTimestamp() },
          { merge: true }
        )
        ok++
      } catch (e) {
