<template>
  <div class="p-3 md:p-5 max-w-5xl mx-auto space-y-4">
      ...
      <div class="flex flex-col md:flex-row md:items-center md:justify-between gap-3">
        <div>
          <h1 class="text-xl md:text-2xl font-black">
            <i class="fas fa-calendar-check text-purple-500 mr-2"></i>Kegiatan Pesantren
          </h1>
        ...
        <button
          v-if="isAdmin"
          @click="openModal()"
          class="bg-purple-600 hover:bg-purple-700 text-white text-xs font-black px-3 py-1.5 rounded-full shadow"
        >
    ...
    >
      ...
      <p class="text-sm text-slate-500 italic">Belum ada kegiatan.</p>
    </div>
        ...
        <div class="flex items-center gap-3">
          ...
          <div class="flex-1 min-w-0">
            ...
            <p class="text-[10px] text-slate-500">
              {{ k.tanggal || '-' }} {{ k.waktu ? '· ' + k.waktu : '' }} · {{ k.lokasi || '-' }}
            </p>
    ...
    >
      <div class="bg-white rounded-2xl shadow-2xl max-w-md w-full p-5 space-y-3">
        <h3 class="text-base font-black">
          <i class="fas fa-calendar-plus text-purple-500 mr-1"></i>Tambah Kegiatan
        </h3>
        <input
          v-model="modalNama"
          type="text"
        ...
        <div class="flex gap-2">
          ...
          <button
            @click="simpan"
            :disabled="saving || !modalNama"
            class="flex-1 px-4 py-2 bg-purple-600 hover:bg-purple-700 disabled:opacity-50 text-white font-bold rounded-xl text-sm"
          >
...
const filteredItems = computed(() => {
  ...
  if (filterTanggal.value)
    list = list.filter((k) => String(k.tanggal || '') === filterTanggal.value)
...
async function simpan() {
  ...
  try {
    ...
    await setDoc(doc(db, 'kegiatan_pesantren', id), {
      ...
      waktu: modalWaktu.value,
      lokasi: modalLokasi.value,
      deskripsi: modalDeskripsi.value,
...
async function deleteItem(k) {
  const ok = await confirmDlg({
    title: 'Hapus?',
    message: `Hapus kegiatan "${k.nama}"?`,
    confirmText: 'Hapus',
    danger: true
  })
  if (!ok) return
  try {
    await deleteDoc(doc(db, 'kegiatan_pesantren', String(k.id)))
