  ...
  <div class="p-3 md:p-5 max-w-5xl mx-auto space-y-4">
    ...
    >
      <h1 class="text-xl md:text-2xl font-black">
        <i class="fas fa-chart-line text-indigo-500 mr-2"></i>Laporan Keuangan
      </h1>
      <p class="text-xs text-slate-500 mt-0.5">Ringkasan pemasukan + pengeluaran per periode</p>
    </div>

    <div
      class="bg-white dark:bg-slate-800 rounded-2xl p-3 border border-slate-200 dark:border-slate-700 shadow-sm grid grid-cols-2 gap-2"
    >
      <select
    ...
    <div class="grid grid-cols-1 md:grid-cols-3 gap-3">
      <div class="bg-emerald-50 rounded-2xl p-4 border-2 border-emerald-200">
        ...
        <p class="text-[10px] text-emerald-600 mt-1">
          {{ pembayaranList.length }} pembayaran + {{ setorList.length }} setor tabungan
        </p>
      </div>
      <div class="bg-rose-50 rounded-2xl p-4 border-2 border-rose-200">
        ...
        <p class="text-[10px] text-rose-600 mt-1">
          {{ bisyarohList.length }} slip bisyaroh + {{ tarikList.length }} tarik tabungan
        </p>
      </div>
      <div
        :class="[
          'rounded-2xl p-4 border-2',
          saldoBersih >= 0 ? 'bg-blue-50 border-blue-200' : 'bg-amber-50 border-amber-200'
        ]"
      >
        <p
          class="text-[10px] uppercase font-black"
          :class="saldoBersih >= 0 ? 'text-blue-700' : 'text-amber-700'"
        >
          Saldo Bersih
        </p>
        <p
          class="text-2xl font-black mt-1"
          :class="saldoBersih >= 0 ? 'text-blue-800' : 'text-amber-800'"
        >
          {{ fmtRp(saldoBersih) }}
        </p>
        <p class="text-[10px] mt-1" :class="saldoBersih >= 0 ? 'text-blue-600' : 'text-amber-600'">
          {{ saldoBersih >= 0 ? 'Surplus' : 'Defisit' }} periode ini
        </p>
      </div>
    </div>
    ...
    <div class="grid grid-cols-1 md:grid-cols-2 gap-3">
      <div class="bg-white rounded-2xl p-4 border border-slate-200 shadow-sm">
        <h3 class="text-xs font-black uppercase mb-2 text-emerald-700">
          <i class="fas fa-arrow-down mr-1"></i>Pemasukan
        </h3>
        <div class="space-y-1 text-xs">
          <div class="flex justify-between">
            <span>Pembayaran Tagihan</span
            ><b class="text-emerald-700">{{ fmtRp(sumPembayaran) }}</b>
          </div>
          <div class="flex justify-between">
            <span>Setor Tabungan</span><b class="text-emerald-700">{{ fmtRp(sumSetor) }}</b>
          </div>
          <div class="border-t pt-1 flex justify-between font-black">
            <span>Total</span><span class="text-emerald-800">{{ fmtRp(totalPemasukan) }}</span>
          </div>
        </div>
      </div>
      <div class="bg-white rounded-2xl p-4 border border-slate-200 shadow-sm">
        <h3 class="text-xs font-black uppercase mb-2 text-rose-700">
        ...
        <div class="space-y-1 text-xs">
          ...
          <div class="flex justify-between">
            <span>Tarik Tabungan</span><b class="text-rose-700">{{ fmtRp(sumTarik) }}</b>
          </div>
          <div class="border-t pt-1 flex justify-between font-black">
            <span>Total</span><span class="text-rose-800">{{ fmtRp(totalPengeluaran) }}</span>
          </div>
        </div>
      </div>
    </div>
...
const pembayaranList = computed(() =>
  pembayaranRaw.value.filter((p) => matchPeriode(p.tanggal || p.created_at))
)
const setorList = computed(() =>
  tabunganRaw.value.filter(
    (m) =>
      (String(m.jenis).includes('setor') || String(m.jenis).includes('bayar')) &&
      matchPeriode(m.tanggal || m.created_at)
  )
)
const tarikList = computed(() =>
  tabunganRaw.value.filter(
    (m) => String(m.jenis).includes('tarik') && matchPeriode(m.tanggal || m.created_at)
  )
)
const bisyarohList = computed(() =>
  bisyarohRaw.value.filter((g) => {
...
const sumPembayaran = computed(() =>
  pembayaranList.value.reduce((s, p) => s + (Number(p.nominal) || 0), 0)
)
...
onMounted(() => {
  unsubP = subscribeColl('keuangan_pembayaran', (docs) => {
    pembayaranRaw.value = docs
  })
