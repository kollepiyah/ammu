<template>
  <div class="p-3 md:p-5 max-w-3xl mx-auto space-y-4">
    ...
    >
      <div>
        <h1 class="text-lg md:text-xl font-black text-slate-800 dark:text-white">
          <i
            :class="editingId ? 'fas fa-edit text-amber-500' : 'fas fa-building text-teal-500'"
            class="mr-2"
          ></i>
    ...
    <form v-else @submit.prevent="onSubmit" class="space-y-4">
      ...
      >
        ...
        <div
          ...
          class="text-center text-xs text-slate-400 italic py-6 bg-white/40 rounded-xl border border-dashed border-teal-300"
        >
      ...
      <div
        class="bg-fuchsia-50/40 dark:bg-fuchsia-900/20 rounded-2xl p-4 md:p-5 border-2 border-fuchsia-300 dark:border-fuchsia-700 shadow-sm"
      >
        <div class="flex items-center justify-between mb-3">
          <h3
          ...
          >
            ...
            <span class="ml-1 text-[10px] bg-fuchsia-100 text-fuchsia-700 px-1.5 py-0.5 rounded">{{
              form.jabatan.length
            }}</span>
          </h3>
        </div>
        <p class="text-[10px] text-slate-500 mb-2">
          <i class="fas fa-info-circle mr-1"></i>Jabatan yang tersedia di lembaga ini (contoh:
          Kepala TPQ, Wali Kelas, PJ Administrasi). Pilih <b>tipe</b> untuk filter di Form Guru.
        </p>
        ...
        <div class="grid grid-cols-[1fr_140px_auto] gap-2 mb-3">
          ...
          />
          <select
            v-model="newJabatanTipe"
            class="px-2 py-2 text-sm rounded-xl border border-fuchsia-300 bg-white cursor-pointer focus:ring-2 focus:ring-fuchsia-500 outline-none"
          >
        ...
        <!-- Jabatan list -->
        <div
          v-if="form.jabatan.length === 0"
          class="text-center text-xs text-slate-400 italic py-6 bg-white/40 rounded-xl border border-dashed border-fuchsia-300"
        >
        ...
        <div v-else class="space-y-1.5">
          ...
          >
            <span class="text-[10px] bg-fuchsia-100 text-fuchsia-700 font-black px-2 py-0.5 rounded"
              >#{{ idx + 1 }}</span
            >
            <span class="flex-1 text-sm font-bold text-slate-800">{{
              typeof j === 'string' ? j : j.nama
            }}</span>
            <select
    ...
    <p class="text-center text-[10px] text-slate-400 pt-2">
      <i class="fas fa-circle-info mr-1"></i>Vue 3 · v.21.23.0526 — CRUD Lembaga + Jabatan
      per-lembaga
    </p>
  </div>
</template>
