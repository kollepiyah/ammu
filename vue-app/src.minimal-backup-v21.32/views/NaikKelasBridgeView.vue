...
<template>
  <div class="p-3 md:p-5 max-w-3xl mx-auto space-y-4">
    ...
    <div
      v-if="isAdmin"
      class="bg-gradient-to-br from-amber-50 to-orange-50 dark:from-amber-900/20 dark:to-orange-900/20 rounded-2xl p-5 md:p-6 border-2 border-amber-200 dark:border-amber-700 shadow-sm"
    >
      <div class="flex items-start gap-3 mb-4">
        <div
          class="flex-shrink-0 w-12 h-12 rounded-2xl bg-amber-100 dark:bg-amber-800 flex items-center justify-center"
        >
        ...
        <div class="flex-1">
          <h3 class="text-base md:text-lg font-black text-slate-800 dark:text-white">
            Fitur Lengkap di Versi Lama
          </h3>
          <p class="text-xs md:text-sm text-slate-600 dark:text-slate-300 mt-1 leading-relaxed">
            Kartu Kenaikan dengan matrix kelas × items × ceremonial dates masih live di sistem lama.
            Vue migration di-defer karena schema kompleks (Qiraati: jilid 1-6 + Al-Qur'an, Sekolah:
            kelas 1-6 + cawu, dll).
          </p>
        </div>
      </div>
