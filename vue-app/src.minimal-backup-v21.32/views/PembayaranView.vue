<template>
  <div class="p-3 md:p-5 max-w-5xl mx-auto space-y-4">
      ...
      <div class="flex flex-col md:flex-row md:items-center md:justify-between gap-3">
        <div>
          <h1 class="text-xl md:text-2xl font-black">
            <i class="fas fa-money-check-alt text-blue-500 mr-2"></i>Pembayaran
          </h1>
          <p class="text-xs text-slate-500 mt-0.5">
            {{ isSantriOnly ? 'Riwayat pembayaran Anda' : 'Log pembayaran semua santri' }}
          </p>
        </div>
    ...
    >
      ...
      <p class="text-sm text-slate-500 italic">Belum ada pembayaran.</p>
    </div>
...
const filteredItems = computed(() => {
  ...
  if (isSantriOnly.value)
    list = list.filter((p) => String(p.santri_id) === String(auth.sesiAktif?.id))
...
onMounted(() => {
  unsubBayar = subscribeColl('keuangan_pembayaran', (docs) => {
