  ...
  <div class="p-3 md:p-5 max-w-5xl mx-auto space-y-4">
    ...
    >
      <h1 class="text-xl md:text-2xl font-black">
        <i class="fas fa-cog text-emerald-500 mr-2"></i>Pengaturan Keuangan
      </h1>
    ...
    <!-- Save bar -->
    <div
      class="bg-emerald-50 border border-emerald-200 rounded-xl p-3 flex items-center justify-between gap-2"
    >
      <p class="text-xs text-emerald-800">
        <i class="fas fa-info-circle mr-1"></i>Setting bisyaroh pokok/sekolah per guru — dipakai
        saat generate slip bisyaroh.
      </p>
      <button
        @click="simpan"
        :disabled="saving"
        class="px-3 py-1.5 bg-emerald-600 hover:bg-emerald-700 disabled:opacity-50 text-white text-xs font-black rounded-lg shadow"
      >
    ...
    <!-- Filter -->
    <div
      class="bg-white dark:bg-slate-800 rounded-2xl p-3 border border-slate-200 dark:border-slate-700 shadow-sm grid grid-cols-1 md:grid-cols-2 gap-2"
    >
      <input
        v-model="search"
        type="text"
    ...
    >
      <div
        class="px-4 py-3 border-b border-slate-100 bg-slate-50 grid grid-cols-[1fr_140px_140px] gap-2 items-center text-xs font-black uppercase tracking-wider text-slate-600"
      >
        <span>Guru</span>
      ...
      <div class="divide-y divide-slate-100 max-h-[500px] overflow-y-auto">
        <div
          ...
          :key="g.id"
          class="px-4 py-2 grid grid-cols-[1fr_140px_140px] gap-2 items-center hover:bg-slate-50"
        >
          <div class="min-w-0">
            <p class="text-sm font-bold truncate">{{ g.nama }}</p>
            <p class="text-[10px] text-slate-500">
              {{ g.lembaga || '-' }} · {{ g.jabatan || 'Guru' }}
            </p>
          </div>
          <input
            v-model.number="bisyarohPokokMap[g.id]"
            type="number"
            min="0"
            placeholder="0"
            class="px-2 py-1.5 text-sm rounded-lg border border-slate-300 bg-white text-right font-bold"
          />
          <input
            v-model.number="bisyarohSekolahMap[g.id]"
            type="number"
            min="0"
            placeholder="0"
            class="px-2 py-1.5 text-sm rounded-lg border border-slate-300 bg-white text-right font-bold"
...
const uniqueLembaga = computed(() =>
  [...new Set(guruList.value.map((g) => g.lembaga).filter(Boolean))].sort()
)
