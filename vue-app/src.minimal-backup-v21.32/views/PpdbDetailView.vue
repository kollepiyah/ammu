<template>
  <div class="p-3 md:p-5 max-w-3xl mx-auto space-y-4">
    ...
    >
      <router-link to="/psb" class="text-xs text-blue-600 hover:underline"
        ><i class="fas fa-arrow-left mr-1"></i>Kembali ke PSB</router-link
      >
      <h1 class="text-xl md:text-2xl font-black mt-2">
        <i class="fas fa-id-card text-teal-500 mr-2"></i>Detail Pendaftar
      </h1>
    </div>
    ...
    <div v-else-if="!pendaftar" class="bg-white rounded-2xl p-10 text-center">
      <i class="fas fa-inbox text-slate-300 text-3xl mb-2"></i>
      <p class="text-sm text-slate-500">Pendaftar tidak ditemukan.</p>
    </div>
    <div v-else class="space-y-3">
      <div class="bg-white rounded-2xl p-4 border border-slate-200 shadow-sm">
        <h3 class="text-sm font-black mb-2">
          <i class="fas fa-user text-blue-500 mr-1"></i>Identitas Calon Santri
        </h3>
      ...
      <div class="bg-white rounded-2xl p-4 border border-slate-200 shadow-sm">
        <h3 class="text-sm font-black mb-2">
          <i class="fas fa-users text-emerald-500 mr-1"></i>Data Wali
        </h3>
        <div class="grid grid-cols-1 md:grid-cols-2 gap-2 text-xs">
      ...
      <div
        v-if="pendaftar.custom_fields && Object.keys(pendaftar.custom_fields).length > 0"
        class="bg-white rounded-2xl p-4 border border-slate-200 shadow-sm"
      >
        <h3 class="text-sm font-black mb-2">
          <i class="fas fa-list text-purple-500 mr-1"></i>Data Tambahan (ACF)
        </h3>
        <div class="grid grid-cols-1 md:grid-cols-2 gap-2 text-xs">
          <div v-for="(v, k) in pendaftar.custom_fields" :key="k">
            <b>{{ k }}:</b> {{ v || '-' }}
          </div>
        </div>
      </div>
      ...
      <div class="bg-white rounded-2xl p-4 border border-slate-200 shadow-sm">
        <h3 class="text-sm font-black mb-2">
          <i class="fas fa-check-circle text-amber-500 mr-1"></i>Status Pendaftaran
        </h3>
        <div class="flex flex-wrap gap-2 mb-3">
          <span :class="['text-xs font-black px-3 py-1 rounded-full', statusBg]">{{
        ...
        <div class="flex flex-wrap gap-2">
          <button
            ...
            class="px-3 py-1.5 bg-emerald-600 hover:bg-emerald-700 text-white text-xs font-bold rounded-lg"
          >
          ...
          <button
            ...
            class="px-3 py-1.5 bg-rose-600 hover:bg-rose-700 text-white text-xs font-bold rounded-lg"
          >
          ...
          <button
            ...
            class="px-3 py-1.5 bg-purple-600 hover:bg-purple-700 text-white text-xs font-bold rounded-lg"
          >
            <i class="fas fa-user-plus mr-1"></i>Convert ke Santri
          </button>
        </div>
      </div>
    </div>
...
async function updateStatus(status) {
  try {
    await setDoc(
      doc(db, 'ppdb_pendaftar', String(route.params.id)),
      { status, _updated_at: serverTimestamp() },
      { merge: true }
    )
...
async function convertToSantri() {
  ...
  try {
    ...
    await setDoc(doc(db, 'santri', String(newId)), {
      ...
      lembaga: p.lembaga_tujuan,
      kelas: p.kelas_sekolah || '',
      wali: p.nama_wali,
      wa: p.wa_wali,
      alamat: p.alamat,
      aktif: true,
      tgl_masuk: new Date().toISOString().slice(0, 10),
      from_ppdb_id: route.params.id,
      custom_fields: p.custom_fields || {},
      created_at: serverTimestamp()
    })
    await setDoc(
      doc(db, 'ppdb_pendaftar', String(route.params.id)),
      { status: 'enrolled', _converted_at: serverTimestamp() },
      { merge: true }
    )
