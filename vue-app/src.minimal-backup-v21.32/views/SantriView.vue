<template>
  <div class="p-3 md:p-5 max-w-6xl mx-auto space-y-4">
      ...
      <div class="flex flex-col lg:flex-row lg:items-center lg:justify-between gap-3">
        <div class="flex items-baseline gap-2 flex-wrap">
          ...
          <p class="text-[11px] text-slate-500 dark:text-slate-400">
            — {{ isFullAccess ? 'Semua santri pondok' : 'Santri yang Anda ampu' }}
          </p>
        </div>
        <div class="flex flex-wrap gap-2 items-center">
          ...
          <button
            v-if="isFullAccess"
            @click="cetakPdf"
            class="h-9 px-3 inline-flex items-center gap-1.5 rounded-xl bg-rose-500 hover:bg-rose-600 text-white text-xs font-bold transition cursor-pointer"
          >
          ...
          <router-link
            ...
            class="h-9 px-3 inline-flex items-center gap-1.5 rounded-xl bg-slate-200 hover:bg-slate-300 text-slate-700 text-xs font-bold transition"
          ...
          <template v-if="isFullAccess && isMasterMode">
            <button
              ...
              class="h-9 px-3 inline-flex items-center gap-1.5 rounded-xl bg-amber-600 hover:bg-amber-700 text-white text-xs font-bold transition cursor-pointer"
            >
              <i class="fas fa-download"></i>Template
            </button>
            <label
              class="h-9 px-3 inline-flex items-center gap-1.5 rounded-xl bg-blue-600 hover:bg-blue-700 text-white text-xs font-bold transition cursor-pointer"
            >
              <i :class="['fas', importing ? 'fa-spinner fa-spin' : 'fa-upload']"></i
              >{{ importing ? 'Impor...' : 'Impor XLSX' }}
              <input
                type="file"
                accept=".xlsx,.xls"
                class="hidden"
    ...
    <div
      v-if="isMasterMode && isFullAccess && selectedCount > 0"
      class="bg-gradient-to-r from-teal-50 to-cyan-50 dark:from-teal-900/30 dark:to-cyan-900/30 rounded-2xl p-3 border border-teal-300 dark:border-teal-700 shadow-sm flex flex-wrap items-center gap-2"
    >
      ...
      <button
        ...
        :disabled="bulkSaving"
        class="h-8 px-3 inline-flex items-center gap-1 rounded-lg bg-emerald-600 hover:bg-emerald-700 disabled:opacity-50 text-white text-[11px] font-bold transition cursor-pointer"
      >
      ...
      <button
        ...
        :disabled="bulkSaving"
        class="h-8 px-3 inline-flex items-center gap-1 rounded-lg bg-amber-600 hover:bg-amber-700 disabled:opacity-50 text-white text-[11px] font-bold transition cursor-pointer"
      >
      ...
      <button
        ...
        class="h-8 px-3 inline-flex items-center gap-1 rounded-lg bg-slate-200 hover:bg-slate-300 text-slate-700 text-[11px] font-bold transition cursor-pointer ml-auto"
      >
    ...
    >
      <div
        class="bg-white dark:bg-slate-800 rounded-2xl shadow-2xl max-w-4xl w-full max-h-[85vh] flex flex-col"
      >
        <div
        ...
        >
          <h3 class="text-base font-black text-slate-800 dark:text-white">
            <i class="fas fa-file-import text-blue-600 mr-2"></i>Preview Impor —
            {{ importPreview.rows.length }} baris
          </h3>
          <button @click="importPreview = null" class="text-slate-400 hover:text-rose-600 text-xl">
        ...
        <div class="p-4 overflow-auto flex-1 text-xs">
          ...
          <table class="w-full border border-slate-200">
            ...
            <tbody>
              <tr
                v-for="(r, i) in importPreview.preview"
                :key="i"
              ...
              >
                ...
                <td class="px-2 py-1">
                  <span
                    :class="r.action === 'new' ? 'text-emerald-700' : 'text-amber-700'"
                    class="font-bold"
          ...
          <p
            v-if="importPreview.rows.length > importPreview.preview.length"
        ...
        <div class="p-4 border-t border-slate-200 dark:border-slate-700 flex justify-end gap-2">
          ...
          <button
            ...
            :disabled="importing"
            class="px-4 py-2 text-xs font-bold rounded-lg bg-emerald-600 hover:bg-emerald-700 text-white disabled:opacity-50"
          >
            <i :class="['fas', importing ? 'fa-spinner fa-spin' : 'fa-check', 'mr-1']"></i
            >{{
              importing
                ? 'Importing...'
                : `Konfirmasi (${importPreview.newCount + importPreview.updateCount})`
            }}
          </button>
        </div>
      </div>
    ...
    <div v-else class="space-y-2">
      <div
        v-for="s in santri"
        :key="s.id"
        class="bg-white dark:bg-slate-800 rounded-xl p-3 md:p-4 border border-slate-200 dark:border-slate-700 shadow-sm hover:shadow-md transition"
      >
        <div class="flex items-start gap-3">
          ...
          <input
            ...
            class="flex-shrink-0 mt-2 w-4 h-4 rounded border-slate-300 text-teal-600 focus:ring-teal-500 cursor-pointer"
          />
          <div
            class="flex-shrink-0 w-12 h-12 md:w-14 md:h-14 rounded-full bg-gradient-to-br from-cyan-100 to-blue-100 dark:from-cyan-700 dark:to-blue-700 border-2 border-white dark:border-slate-700 flex items-center justify-center overflow-hidden"
          >
          ...
          <div class="flex-1 min-w-0">
            <div class="flex items-start justify-between gap-2">
              ...
              <span
                v-if="s.is_mukim"
                class="text-[9px] bg-purple-100 dark:bg-purple-900/40 text-purple-700 dark:text-purple-300 border border-purple-200 dark:border-purple-700 px-2 py-0.5 rounded font-black uppercase tracking-wider flex-shrink-0"
                >Ma'had</span
              >
              <span
                v-else-if="s.is_fullday"
                class="text-[9px] bg-amber-100 dark:bg-amber-900/40 text-amber-700 dark:text-amber-300 border border-amber-200 dark:border-amber-700 px-2 py-0.5 rounded font-black uppercase tracking-wider flex-shrink-0"
                >Fullday</span
              >
              <span
                v-if="s.aktif === false"
                class="text-[9px] bg-rose-100 dark:bg-rose-900/40 text-rose-700 dark:text-rose-300 border border-rose-200 dark:border-rose-700 px-2 py-0.5 rounded font-black uppercase tracking-wider flex-shrink-0"
                >Tidak Aktif</span
              >
            </div>
            <div class="flex flex-wrap gap-1 mt-1.5">
              <span
                ...
                >{{ s.lembaga }} · {{ s.kelas || '-' }}</span
              >
              <span
                v-if="s.kelas_sekolah"
            ...
            <div class="flex flex-wrap gap-1 mt-1">
              <span
                v-for="(label, k) in formatGuru(s)"
                :key="k"
                class="text-[10px] bg-slate-50 dark:bg-slate-700/40 text-slate-600 dark:text-slate-300 border border-slate-200 dark:border-slate-600 px-1.5 py-0.5 rounded"
                >{{ label }}</span
              >
            </div>
            <div
              class="flex items-center gap-2 mt-1.5 text-[11px] text-slate-500 dark:text-slate-400"
            >
              <i class="fas fa-user-friends"></i>
              <span class="truncate">{{ s.wali || '—' }}</span>
              <a
                v-if="s.wa"
                :href="`https://wa.me/${cleanWa(s.wa)}`"
                target="_blank"
            ...
            <div v-if="isFullAccess && isMasterMode" class="mt-2 flex justify-end gap-2 flex-wrap">
              <button
                ...
                class="text-[10px] text-blue-700 dark:text-blue-300 hover:underline font-bold"
...
const isAllVisibleSelected = computed(
  () => santri.value.length > 0 && santri.value.every((s) => selected.value.has(String(s.id)))
)
...
async function onImportSantri(e) {
  ...
  try {
    ...
    for (const r of rows) {
      ...
      const lembaga = String(_pick(r, 'Lembaga Qiraati', 'Lembaga', 'lembaga') || '')
        .trim()
        .toUpperCase()
      ...
      allMapped.push({
        ...
        data: {
          nis,
          nama,
          jk: String(_pick(r, 'L/P', 'JK', 'Jenis Kelamin', 'jk') || 'L')
            .trim()
            .toUpperCase()
            .charAt(0),
          tgl_lahir: parseTglDDMMYYYY(
            _pick(r, 'Tgl Lahir (DD/MM/YYYY)', 'Tgl Lahir', 'Tanggal Lahir', 'tgl_lahir')
          ),
          tgl_masuk: parseTglDDMMYYYY(
            _pick(r, 'Tgl Masuk (DD/MM/YYYY)', 'Tgl Masuk', 'Tanggal Masuk', 'tgl_masuk')
          ),
          wali: toTitleCase(_pick(r, 'Nama Wali', 'Wali', 'wali') || ''),
          wa: waList[0] || '',
          wa_2: waList[1] || '',
          ...
          juz: String(_pick(r, 'Juz (PTPT)', 'Juz', 'juz') || '')
            .trim()
            .toUpperCase(),
          lembaga_sekolah: String(_pick(r, 'Lembaga Sekolah', 'lembaga_sekolah') || '')
            .trim()
            .toUpperCase(),
          kelas_sekolah: String(_pick(r, 'Kelas Sekolah', 'kelas_sekolah') || '').trim(),
          guru_sekolah: String(
            _pick(r, 'Guru Sekolah (pisah |)', 'Guru Sekolah', 'guru_sekolah') || ''
          )
            .split('|')
            .map((s) => s.trim())
            .filter(Boolean),
