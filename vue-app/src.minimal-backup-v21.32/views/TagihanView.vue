<template>
  <div class="p-3 md:p-5 max-w-5xl mx-auto space-y-4">
      ...
      <div class="flex flex-col md:flex-row md:items-center md:justify-between gap-3">
        <div>
          <h1 class="text-xl md:text-2xl font-black">
            <i class="fas fa-file-invoice-dollar text-amber-500 mr-2"></i>Tagihan Santri
          </h1>
        ...
        <div class="flex gap-2">
          <div class="px-3 py-1.5 rounded-full bg-amber-50 border border-amber-200 text-xs">
            <span class="text-amber-700 font-bold">{{ stats.totalTagihan }}</span>
          ...
          <div class="px-3 py-1.5 rounded-full bg-rose-50 border border-rose-200 text-xs">
            ...
            <span class="text-slate-500">tunggakan</span>
          </div>
          <button
            v-if="isFullAccess"
            @click="openModalNew"
            class="bg-amber-600 hover:bg-amber-700 text-white text-xs font-black px-3 py-1.5 rounded-full shadow"
          >
    ...
    >
      ...
      <p class="text-sm text-slate-500 italic">Belum ada tagihan.</p>
    </div>
    ...
    >
      <div class="bg-white rounded-2xl shadow-2xl max-w-md w-full p-5">
        <h3 class="text-base font-black mb-3">
          <i class="fas fa-file-invoice text-amber-500 mr-1"></i
        ...
        <div class="mt-4 flex gap-2">
          ...
          <button
            @click="simpanModal"
            :disabled="saving"
            class="flex-1 px-4 py-2 bg-amber-600 hover:bg-amber-700 disabled:opacity-50 text-white font-bold rounded-xl text-sm"
          >
...
async function simpanModal() {
  ...
  try {
    if (modalMode.value === 'new') {
      ...
      await setDoc(
        doc(db, 'keuangan_tagihan', String(t.id)),
        { bayar: newBayar, _last_bayar_at: serverTimestamp() },
        { merge: true }
      )
...
async function deleteTagihan(t) {
  ...
  if (!ok) return
  try {
    await deleteDoc(doc(db, 'keuangan_tagihan', String(t.id)))
