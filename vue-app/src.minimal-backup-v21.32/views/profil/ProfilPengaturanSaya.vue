<template>
  ...
  >
    <h3 class="text-base font-black mb-3">
      <i class="fas fa-cog text-slate-500 mr-2"></i>Pengaturan Profil
    </h3>
    <p class="text-xs text-slate-500 mb-4">Kaitkan akun Google supaya bisa login 1 klik.</p>
    <div class="grid grid-cols-1 md:grid-cols-3 gap-3">
      <button
        @click="onClickPassword"
        class="bg-white border border-slate-200 hover:border-blue-300 rounded-xl p-3 text-left"
      >
      ...
      <button
        @click="onClickWA"
        class="bg-emerald-50 border border-emerald-200 hover:border-emerald-400 rounded-xl p-3 text-left"
      >
      ...
      <button
        @click="onClickGoogle"
        :disabled="busy"
        class="bg-white border border-slate-200 hover:border-blue-300 disabled:opacity-50 rounded-xl p-3 text-left"
      >
        <i class="fab fa-google text-rose-500"></i>
        <p class="text-sm font-bold mt-2">
          {{ entity?.linked_uid ? 'Lepas Akun Google' : 'Kaitkan Akun Google' }}
        </p>
        <p class="text-[10px] text-slate-500">
          {{ entity?.linked_email || (entity?.linked_uid ? 'Terkait' : 'Belum tertaut') }}
        </p>
      </button>
    </div>
    <div
    ...
    >
      <form
        @submit.prevent="simpanPassword"
        class="bg-white rounded-2xl shadow-2xl max-w-md w-full p-5 space-y-3"
      >
...
function onClickWA() {
  const wa = props.entity?.wa
  if (wa) window.open(`https://wa.me/${String(wa).replace(/^0/, '62')}`, '_blank')
}
async function simpanPassword() {
  ...
  if (!props.entityId) {
    toast.warning('Entity ID kosong')
    return
  }
  savingPwd.value = true
  try {
    await setDoc(
      doc(db, props.role === 'santri' ? 'santri' : 'guru', String(props.entityId)),
  ...
  try {
    if (props.entity?.linked_uid) {
      const ok = await confirmDlg({
        ...
        message: 'Setelah lepas, login Google tidak akan masuk ke akun ini.',
        confirmText: 'Lepas',
        danger: true
      })
      if (!ok) {
      ...
      if (props.entity) {
        props.entity.linked_uid = null
        props.entity.linked_email = null
      }
      toast.success('Akun Google dilepas')
    } else {
      const res = await linkGoogleAccount(props.role, props.entityId)
      if (props.entity) {
        props.entity.linked_uid = res.uid
        props.entity.linked_email = res.email
      }
