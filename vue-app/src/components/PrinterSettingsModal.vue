<template>
  ...
  >
    <div
      class="bg-[var(--bg-card)] rounded-2xl shadow-2xl w-full max-w-md p-5 border border-[var(--border-subtle)]"
    >
      <h3 class="text-base font-black text-[var(--text-primary)] mb-1">
        <i class="fas fa-print text-teal-500 mr-2"></i>Pengaturan Printer
      </h3>
      <p class="text-xs text-[var(--text-secondary)] mb-3">
        Pilih printer default untuk tombol "Cetak Langsung" struk POS. Daftar diambil dari printer
        yang terhubung ke Windows.
      </p>
      <div class="flex items-center justify-between mb-1">
        <label class="block text-xs font-bold text-[var(--text-secondary)] uppercase"
          >Printer Default</label
        >
        <button
          type="button"
          :disabled="loading"
          class="text-[11px] font-bold text-teal-600 hover:text-teal-700 disabled:opacity-50"
          @click="refresh"
        >
          <i :class="loading ? 'fas fa-spinner fa-spin' : 'fas fa-sync'" class="mr-1"></i
          >{{ loading ? 'Mendeteksi…' : 'Deteksi ulang' }}
        </button>
      </div>
      <select
        v-model="selected"
      ...
      <p
        v-if="isDesktop && printers.length > 0"
        class="text-[11px] text-[var(--text-secondary)] mt-2"
      >
        <i class="fas fa-check-circle text-emerald-500 mr-1"></i>{{ printers.length }} printer
        terdeteksi di Windows.
      </p>
      <p v-else-if="isDesktop && !loading" class="text-[11px] text-amber-600 mt-2">
        Tidak ada printer terdeteksi. Pastikan printer menyala &amp; terpasang di Windows, lalu klik
        "Deteksi ulang".
      </p>
      <p v-else-if="!isDesktop" class="text-[11px] text-amber-600 mt-2">
        Deteksi printer hanya tersedia di aplikasi Desktop (Windows). Di web/HP, struk dicetak lewat
        dialog browser.
      </p>
      <button
        v-if="isDesktop"
        ...
        @click="tesCetak"
      >
        <i :class="testing ? 'fas fa-spinner fa-spin' : 'fas fa-print'" class="mr-1"></i
        >{{ testing ? 'Mengirim…' : 'Tes Cetak' }}
      </button>
      <div class="flex justify-end gap-2 mt-4">
        ...
        <button
          type="button"
          class="px-4 py-2 text-sm font-bold rounded-xl bg-teal-600 hover:bg-teal-700 text-white"
          @click="simpan"
        >
          Simpan
        </button>
...
function simpan() {
  ...
  toast.success(
    selected.value ? 'Printer default: ' + selected.value : 'Pakai printer default sistem'
  )
  open.value = false
}
...
async function tesCetak() {
  ...
  try {
    ...
    const dummy = {
      ...
      metode: 'TUNAI',
      operator: selected.value || 'Printer',
      penyetor: '',
