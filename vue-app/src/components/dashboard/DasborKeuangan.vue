  ...
  <div>
    ...
    <div class="rb-fin-row">
      ...
      <router-link to="/bisyaroh" class="rb-fin-card" style="display: block; text-decoration: none">
        ...
        <div class="rb-fin-sub">{{ gedungScoped ? 'slip · semua gedung' : 'slip' }}</div>
      </router-link>
      <router-link
        to="/buku-induk"
        class="rb-fin-card"
    ...
    <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 16px; margin-top: 16px">
      ...
      <div class="rb-card">
        ...
        <p style="font-size: 11px; color: var(--rb-text-dim); margin-bottom: 6px">
    ...
    <div v-if="showKategori" class="rb-card" style="margin-top: 16px">
      ...
      <div
        v-else
        style="display: grid; grid-template-columns: 220px 1fr; gap: 18px; align-items: center"
      >
        ...
        <div style="display: flex; flex-direction: column; gap: 6px">
          <div
            v-for="(k, i) in kategoriBulanIni"
            :key="k.nama"
            style="display: flex; align-items: center; gap: 8px"
          >
            <span
              :style="{ background: PALETTE[i % PALETTE.length] }"
              style="width: 12px; height: 12px; border-radius: 3px; flex: none"
            ></span>
            <span style="flex: 1; font-size: 12px; font-weight: 600">{{ k.nama }}</span>
            <span style="font-size: 12px; font-weight: 800">{{ fmtRp(k.total) }}</span>
            <span style="font-size: 10px; color: var(--rb-text-dim); width: 38px; text-align: right"
              >{{ k.pct }}%</span
            >
          </div>
        </div>
      ...
      <table v-else class="rb-table">
        ...
        <tbody>
          <tr v-for="(t, i) in transaksiTerakhir" :key="i">
            ...
            <td>{{ t.ket }}</td>
            <td>
              <span class="rb-pill" :class="t.jenis === 'Masuk' ? 'masuk' : 'keluar'">{{
                t.jenis
              }}</span>
            </td>
