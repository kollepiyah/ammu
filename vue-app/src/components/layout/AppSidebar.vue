<template>
  ...
  <aside :class="['bg-white dark:bg-slate-900 text-slate-700 dark:text-white flex-shrink-0 flex flex-col fixed inset-y-0 left-0 transform transition-all duration-300 z-50 shadow-xl border-r border-emerald-100 dark:border-white/5 md:relative overflow-hidden', ui.sidebarOpen ? 'translate-x-0 w-72' : '-translate-x-full w-72 md:translate-x-0 md:w-0 md:border-0 md:shadow-none']" aria-label="Sidebar navigasi">
    <div class="px-6 pt-8 pb-6 flex flex-col items-center gap-3 relative bg-emerald-50/60 dark:bg-black/20" style="padding-top: calc(2rem + env(safe-area-inset-top))">
      <button @click="ui.closeSidebar()" class="md:hidden absolute top-4 right-4 text-slate-500 hover:text-slate-700 dark:text-white/70 dark:hover:text-white text-xl" style="top: calc(1rem + env(safe-area-inset-top))" aria-label="Tutup sidebar">
      ...
      <div class="text-center w-full mt-1">
        <h1 class="text-[19px] font-black text-slate-800 dark:text-white leading-tight tracking-tight">
          <span class="block">{{ namaLine1 }}</span>
          <span v-if="namaLine2" class="block">{{ namaLine2 }}</span>
        </h1>
        <p class="text-[10px] text-teal-700 dark:text-teal-300 font-bold uppercase tracking-widest mt-3 bg-teal-100/60 dark:bg-white/10 px-3 py-1 rounded-full inline-block truncate">
          {{ periodeHijri }}
        </p>
      </div>
    </div>
    ...
    <div class="px-4 py-4 text-center bg-emerald-50/60 dark:bg-black/20 border-t border-emerald-100 dark:border-white/10" style="padding-bottom: calc(1rem + env(safe-area-inset-bottom))">
  ...
  <div v-if="ui.sidebarOpen" @click="ui.closeSidebar()" class="md:hidden fixed inset-0 bg-slate-900/50 z-40 backdrop-blur-sm"></div>
</template>
