          ...
          <div class="mbody">
            <div v-if="santri" class="santri-info">
              ...
              <div>
                ...
                <p class="sub">
                  NIS: {{ santri.nis || '—' }}{{ santri.lembaga ? ' · ' + santri.lembaga : '' }}{{ santri.kelas ? ' · ' + santri.kelas : '' }}
                </p>
              </div>
            </div>
            <div class="presets">
              <button v-for="p in presetList" :key="p.label" type="button" class="preset" @click="addItem(p)" :title="p.nominal_default > 0 ? `Default: Rp ${new Intl.NumberFormat('id-ID').format(p.nominal_default)}` : ''">
                + {{ p.label }}<span v-if="p.nominal_default > 0" class="text-[10px] opacity-70 ml-1">({{ new Intl.NumberFormat('id-ID').format(p.nominal_default) }})</span>
              </button>
            </div>
            <div class="items">
...
.modal { background: white; border-radius: 1rem; box-shadow: 0 25px 50px -12px rgb(0 0 0 / 0.25); max-width: 42rem; width: 100%; color: #0f172a; }
:global(.dark) .modal, .dark-mode .modal { background: #18181b; color: #fafafa; }
.mhdr { display: flex; align-items: center; justify-content: space-between; padding: 1rem; border-bottom: 1px solid #e2e8f0; }
...
.ava { width: 40px; height: 40px; border-radius: 9999px; background: #0f766e; color: white; display: flex; align-items: center; justify-content: center; font-weight: 900; }
...
.preset { padding: 0.375rem 0.75rem; font-size: 0.75rem; font-weight: 700; border-radius: 0.5rem; background: #f1f5f9; border: none; cursor: pointer; transition: background 150ms; }
.preset:hover { background: #ccfbf1; }
.items { display: flex; flex-direction: column; gap: 0.5rem; margin-bottom: 1rem; min-height: 60px; }
.empty-cart { text-align: center; color: #94a3b8; font-size: 0.8rem; padding: 1rem; margin: 0; }
.item { display: grid; grid-template-columns: 4fr 3fr 4fr auto; gap: 0.5rem; align-items: center; }
...
.inp-bayar { width: 8rem; padding: 0.375rem 0.75rem; border-radius: 0.5rem; border: 1px solid #cbd5e1; text-align: right; font-weight: 700; background: white; color: #0f172a; }
.mfoot { display: flex; align-items: center; justify-content: flex-end; gap: 0.5rem; padding: 1rem; border-top: 1px solid #e2e8f0; background: #f8fafc; border-bottom-left-radius: 1rem; border-bottom-right-radius: 1rem; }
