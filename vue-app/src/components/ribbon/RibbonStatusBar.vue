<template>
  <div class="rb-statusbar">
    ...
    <div class="sb-r">
      <span style="opacity: 0.85">{{ santriCount.toLocaleString('id-ID') }} santri · {{ guruCount }} guru</span>
...
const hijri = computed(() => {
  try {
    return new Intl.DateTimeFormat('ar-SA-u-ca-islamic-umalqura', { month: 'long', year: 'numeric' }).format(new Date())
  } catch (e) {
    return ''
  }
})
