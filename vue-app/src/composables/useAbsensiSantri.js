...
export function useAbsensiSantri() {
  ...
  const santriNonMukim = computed(() =>
    santriList.value.filter((s) => s.aktif !== false && (s.is_mukim === false || s.is_fullday === true))
  )
  ...
  const filteredAbsensi = computed(() => {
    ...
    if (selectedLembaga.value) list = list.filter((a) => String(a.lembaga || '').toLowerCase() === selectedLembaga.value.toLowerCase())
    if (selectedKelas.value) list = list.filter((a) => String(a.kelas || '') === selectedKelas.value)
    if (selectedStatus.value) list = list.filter((a) => String(a.status || '').toUpperCase() === selectedStatus.value.toUpperCase())
    return list
  })
  ...
  const rekapList = computed(() => {
    ...
    for (const s of santriNonMukim.value) {
      ...
      if (selectedLembaga.value && String(s.lembaga || '').toLowerCase() !== selectedLembaga.value.toLowerCase()) continue
      if (selectedKelas.value && String(s.kelas || '') !== selectedKelas.value) continue
      map.set(sid, { santri_id: sid, nama: s.nama || '(unknown)', lembaga: s.lembaga || '', kelas: s.kelas || '', hadir: 0, alfa: 0, sakit: 0, izin: 0, total: 0 })
    }
    // Count from absensi
    for (const a of filteredAbsensi.value) {
      ...
      if (!r) {
        ...
        r = { santri_id: sid, nama: a.nama_cache || '(unknown)', lembaga: a.lembaga || '', kelas: a.kelas || '', hadir: 0, alfa: 0, sakit: 0, izin: 0, total: 0 }
        map.set(sid, r)
      }
  ...
  const uniqueLembaga = computed(() => [...new Set(santriNonMukim.value.map((s) => s.lembaga).filter(Boolean))].sort())
