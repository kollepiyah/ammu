...
export async function cetakStrukSlipPdf(trx, settings = {}, { preview = false } = {}) {
  ...
  const right = slipW - 9
  const dash = (pat) => {
    if (typeof doc.setLineDashPattern === 'function') doc.setLineDashPattern(pat || [], 0)
  }
  ...
  if (muassis) {
    const muH = 7
    try {
      doc.addImage(muassis, 'PNG', left, y - 4, muH * MUASSIS_RATIO, muH, undefined, 'FAST')
    } catch (_e) {}
    ky = y + 6.5
  } else {
  ...
  if (kop.line2) {
    doc.setFont(font, 'bold')
    doc.setFontSize(8.3)
    doc.text(String(kop.line2), left, ky)
    ky += 3.2
  }
  doc.setFont(font, 'normal')
  doc.setFontSize(7)
  ...
  for (let i = 0; i < items.length; i++) {
    ...
    if (as > ne) {
      ...
      doc.line(ne, y - 0.8, as, y - 0.8)
      dash(null)
    }
    y += 4.4
  }
  if (!items.length) {
    doc.text('—', slipW / 2, y, { align: 'center' })
    y += 4.4
  }
  y += 0.8
  doc.setLineWidth(0.5)
  ...
  const sy = y + 14
  const penyetorPdf = String(trx.penyetor || '').trim()
  doc.text(penyetorPdf ? '( ' + penyetorPdf + ' )' : '( .................. )', c1, sy, {
    align: 'center'
  })
...
export async function cetakSlipBisyarohPdf(slip = {}, settings = {}, { preview = false } = {}) {
  ...
  // v.111: bisyaroh Tes Glondongan PTPT (per juz disimak)
  const glondTotal = Number((slip.bonus_glondongan || {}).total || 0)
  if (glondTotal > 0) {
    const jz = Number((slip.bonus_glondongan || {}).juz || 0)
    body.push([
      'Bisyaroh Glondongan PTPT' + (jz ? ' (' + jz + ' juz)' : ''),
      fmtRpStruk(glondTotal)
    ])
  }
...
export async function exportRekapBisyarohPdf(slips = [], settings = {}, periodeLabel = '') {
  ...
  let y = await drawKopLetterhead(doc, kop, { y: 10 })
  drawTitle(doc, 'REKAP SLIP BISYAROH' + (periodeLabel ? ' — ' + periodeLabel : ''), {
    y: y + 7,
    size: 13
  })
  y += 14
  ...
  const tunjOf = (s) =>
    Array.isArray(s.tunjangan_list)
      ? s.tunjangan_list.reduce((a, t) => a + (Number(t.nominal) || 0), 0)
      : 0
  ...
  body.push([
    '',
    'TOTAL (' + slips.length + ' slip)',
    '',
    fmtRpStruk(tot.pokok),
    fmtRpStruk(tot.bonus),
    fmtRpStruk(tot.tunj),
    fmtRpStruk(tot.pot),
    fmtRpStruk(tot.take)
  ])
  ...
  drawTable(doc, {
    startY: y,
    head: [
      ['No', 'Nama', 'Jabatan', 'Bisyaroh Pokok', 'Bonus', 'Tunjangan', 'Potongan', 'Take Home']
    ],
    body,
    columnStyles: {
      0: { halign: 'center', cellWidth: 10 },
...
export async function cetakSlipTabunganPdf(
  mut = {},
  settings = {},
  { preview = true, saldo = null, santri = {}, label = 'TABUNGAN' } = {}
) {
  const kop = buildKopFromSettings(settings)
  const num = (v, lo, hi, def) => {
    const n = parseFloat(v)
  ...
  savePdf(
    doc,
    'slip_' + (isUS ? 'uangsaku' : 'tabungan') + '_' + safe + '_' + (mut.tanggal || '') + '.pdf',
    { preview }
  )
  return doc
}
...
function slipShellHtml({
  ...
  const kopRest = [kop.line3, kop.line4, kop.line5]
    .filter(Boolean)
    .map((l) => `<div class="ks">${escapeHtml(l)}</div>`)
    .join('')
  ...
  return (
    '<!DOCTYPE html><html><head><meta charset="utf-8"><title>Slip</title><style>' +
...
export function buildSlipTabunganHtml(
  mut = {},
  settings = {},
  { saldo = null, santri = {}, label = 'TABUNGAN' } = {}
) {
  const isSetor = String(mut.jenis || 'setor').toLowerCase() === 'setor'
  const isUS = String(label).toUpperCase().includes('SAKU')
  const nama = mut.nama_cache || santri.nama || '-'
  ...
  const ket =
    ...
    (mut.catatan ? ' (' + mut.catatan + ')' : '')
  const midHtml =
    '<div class="hdr">Dengan rincian sebagai berikut :</div>' +
  ...
  const totHtml = totRows
    .map(
      ([l, v, b]) =>
        `<div class="tr${b ? ' big' : ''}"><span>${escapeHtml(l)}</span><span>${escapeHtml(v)}</span></div>`
    )
    .join('')
  const footHtml =
...
export function buildStrukSlipHtml(trx = {}, settings = {}) {
  ...
  return slipShellHtml({
    ...
    infoRight: [
      ['Tgl. Bayar', formatTglDdMmYyyy(trx.tanggal)],
      ['No. Transaksi', trx.no_struk || '-'],
      ['Metode', trx.metode || 'TUNAI']
    ],
    midHtml,
...
export async function exportRekapTabunganPdf(
  items = [],
  settings = {},
  { title = 'REKAP TABUNGAN', namaOf = null } = {}
) {
  const kop = buildKopFromSettings(settings)
  const doc = await createPdf({ kind: 'umum', orientation: 'p', format: 'F4' })
  let y = await drawKopLetterhead(doc, kop, { y: 10 })
  ...
  const body = items.map((t, i) => {
    ...
    return [
      String(i + 1),
      namaOf ? namaOf(t) : t.nama_cache || t.santri_id || '-',
      t.terakhir_update || '-',
      fmtRpStruk(s)
    ]
  })
...
const PAPER = {
  9.5: {
    cols: 80,
    pageCss:
      '@page { size: 241mm 279mm; margin: 5mm 5mm; } body { font-size: 17px; font-weight: 700; color: #000; letter-spacing: 0.3px; }'
  }, // 9.5x11 inci; font 17px = 80 kolom mengisi penuh lebar 9.5"
  thermal80: {
    cols: 42,
    pageCss: '@page { size: 80mm auto; margin: 2mm; } body { font-size: 11px; }'
  },
  thermal58: {
    cols: 32,
    pageCss: '@page { size: 58mm auto; margin: 2mm; } body { font-size: 10px; }'
  }
}
...
function buildStrukWide(trx, settings) {
  ...
  const cell = (str, wd) => {
    str = String(str)
    return str.length >= wd ? str.slice(0, wd) : str + ' '.repeat(wd - str.length)
  }
  const totRow = (label, val) => lr(label + ' Rp.', fmtNum(val), W - C3)
  const sigName = (n) => '( ' + (n ? String(n) : '..................') + ' )'
  out.push(
    cell(center('Penyetor,', C2), C2) +
      cell(center('Penerima,', C3 - C2), C3 - C2) +
      totRow('Jumlah', trx.total)
  )
  out.push(cell('', C2) + cell('', C3 - C2) + totRow('Pembayaran', trx.bayar))
  out.push(cell('', C2) + cell('', C3 - C2) + totRow('Kembali', trx.kembali))
  out.push(
    cell(center(sigName(penyetorWide), C2), C2) +
      cell(center(sigName(trx.operator), C3 - C2), C3 - C2)
  )
  return out.join('\n')
}
...
function buildStrukNarrow(trx, settings, w) {
  ...
  if (String(trx.penyetor || '').trim())
    out.push('Wali  : ' + clip(String(trx.penyetor).trim(), w - 8))
  out.push(repCh('-', w))
  for (const it of trx.items || []) {
...
function _row(label, val) {
  return (
    '<tr><td style="vertical-align:top;white-space:nowrap;padding:1px 0;">' +
    escapeHtml(label) +
    '</td><td style="vertical-align:top;padding:1px 0 1px 6px;">: ' +
...
export function buildStrukHtmlWide(trx, s = {}) {
  ...
  const items = (trx.items || [])
    .map(
      (it, i) =>
        '<tr><td style="vertical-align:top;padding:2px 4px;width:22px;">' +
        (i + 1) +
        '.</td>' +
        '<td style="vertical-align:top;padding:2px 4px;">' +
        escapeHtml(
          clip(String(it.jenis || '-') + (it.keterangan ? ' (' + it.keterangan + ')' : ''), 58)
        ) +
        '</td>' +
        '<td style="vertical-align:top;padding:2px 4px;text-align:right;white-space:nowrap;">Rp. ' +
        fmtNum(it.nominal) +
        '</td></tr>'
    )
    .join('')
  return (
    '<!DOCTYPE html><html><head><meta charset="utf-8"><title>Struk</title><style>' +
    ...
    (muassisDataUrlSync()
      ? '<div><img src="' +
        muassisDataUrlSync() +
        '" alt="" style="height:7mm;display:block" /></div>'
    ...
    '<td style="vertical-align:top;padding:1px 0 1px 6px;">: ' +
    escapeHtml(kelas) +
    ' &nbsp;&nbsp;&nbsp;&nbsp;Status Siswa : ' +
    escapeHtml(trx.status_siswa || 'Aktif') +
    '</td></tr>' +
    _row('Terbilang', terb) +
    ...
    '<td style="width:34%;text-align:center;vertical-align:top;">Penyetor,<div style="height:20px;"></div>( ' +
    (penyetor ? escapeHtml(penyetor) : '&nbsp;..........&nbsp;') +
    ' )</td>' +
    '<td style="width:34%;text-align:center;vertical-align:top;">Penerima,<div style="height:16px;"></div>( ' +
    escapeHtml(String(trx.operator || '')) +
    ' )</td>' +
    '<td style="vertical-align:top;"><table>' +
    '<tr><td style="text-align:right;font-weight:bold;padding:1px 0;">Jumlah Rp.</td><td style="text-align:right;font-weight:bold;white-space:nowrap;padding:1px 0 1px 10px;">' +
    fmtNum(trx.total) +
    '</td></tr>' +
    '<tr><td style="text-align:right;padding:1px 0;">Pembayaran Rp.</td><td style="text-align:right;white-space:nowrap;padding:1px 0 1px 10px;">' +
    fmtNum(trx.bayar) +
    '</td></tr>' +
    '<tr><td style="text-align:right;padding:1px 0;">Kembali Rp.</td><td style="text-align:right;white-space:nowrap;padding:1px 0 1px 10px;">' +
    fmtNum(trx.kembali) +
    '</td></tr>' +
    '</table></td></tr></table>' +
    '</div></body></html>'
