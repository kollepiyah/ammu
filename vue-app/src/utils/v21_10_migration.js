...
export async function runV21_10Migration({ santriList, guruList, lembagaList, onProgress } = {}) {
  const result = { santri: { ok: 0, fail: 0 }, guru: { ok: 0, fail: 0 }, lembaga: { ok: 0, fail: 0 }, errors: [] }
  ...
  while (i < santriQueue.length) {
    ...
    for (const item of chunk) {
      try {
        batch.set(doc(db, 'santri', String(item.id)), {
    ...
    i += chunk.length
    if (onProgress) onProgress({ phase: 'santri', i: result.santri.ok + result.santri.fail, total: santriQueue.length })
  }
  ...
  while (i < guruQueue.length) {
    ...
    for (const item of chunk) {
      try {
        batch.set(doc(db, 'guru', String(item.id)), {
          lembaga_refs: item.refs,
          _migrated_v21_10_at: stamp
        }, { merge: true })
      } catch (e) {
    ...
    i += chunk.length
    if (onProgress) onProgress({ phase: 'guru', i: result.guru.ok + result.guru.fail, total: guruQueue.length })
  }
  ...
  if (lembagaList && lembagaList.length > 0) {
    try {
      ...
      await setDoc(doc(db, 'master', 'lembaga'), {
        list: updated,
        _migrated_v21_10_at: stamp
      }, { merge: true })
    ...
    if (onProgress) onProgress({ phase: 'lembaga', i: lembagaList.length, total: lembagaList.length })
  }
