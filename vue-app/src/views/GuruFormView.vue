<template>
  <div class="p-3 md:p-5 max-w-4xl mx-auto space-y-4">
    ...
    <div class="bg-white dark:bg-slate-800 rounded-2xl p-4 md:p-5 border border-slate-200 dark:border-slate-700 shadow-sm flex items-center justify-between gap-3">
      ...
      <router-link to="/guru" class="px-3 py-2 text-sm bg-slate-100 dark:bg-slate-700 hover:bg-slate-200 text-slate-700 dark:text-slate-300 font-bold rounded-xl transition">
    ...
    <form v-else @submit.prevent="onSubmit" class="space-y-4">
      ...
      <div class="bg-white dark:bg-slate-800 rounded-2xl p-4 md:p-5 border border-slate-200 dark:border-slate-700 shadow-sm">
        ...
        <div class="grid grid-cols-1 md:grid-cols-4 gap-3">
          <div class="md:col-span-3">
            <label class="block text-xs font-bold text-slate-500 mb-1 uppercase">Nama Lengkap (dengan gelar) *</label>
            <input v-model="form.nama" type="text" required placeholder="cth: Ust. Ahmad Sholeh, M.Pd" class="w-full px-3 py-2 text-sm rounded-xl border border-slate-300 dark:border-slate-600 bg-slate-50 dark:bg-slate-900 focus:ring-2 focus:ring-teal-500 outline-none" />
          </div>
          <div>
      ...
      <div class="bg-white dark:bg-slate-800 rounded-2xl p-4 md:p-5 border border-slate-200 dark:border-slate-700 shadow-sm">
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-3">
          ...
          <div>
            <label class="block text-xs font-bold text-slate-500 mb-1 uppercase">Jabatan Utama *</label>
            <select v-model="form.jabatan" required class="w-full px-3 py-2 text-sm rounded-xl border border-slate-300 bg-slate-50 focus:ring-2 focus:ring-cyan-500 outline-none">
              <option v-for="j in jabatanOptionsFiltered" :key="j" :value="j">{{ j }}</option>
            </select>
          </div>
          <div>
            <label class="block text-xs font-bold text-slate-500 mb-1 uppercase">Jabatan Tambahan</label>
            <select v-model="form.jabatan_tambahan" class="w-full px-3 py-2 text-sm rounded-xl border border-slate-300 bg-slate-50 focus:ring-2 focus:ring-cyan-500 outline-none">
          ...
          <div class="md:col-span-2">
            <label class="block text-xs font-bold text-slate-500 mb-1 uppercase">Tipe Pegawai *</label>
            <div class="flex flex-wrap gap-2">
              <label v-for="t in TIPE_PEGAWAI_OPTIONS" :key="t.value" class="flex items-center gap-2 px-3 py-2 rounded-xl border border-slate-300 bg-slate-50 cursor-pointer hover:bg-slate-100 text-sm">
          ...
          <div v-if="form.tipe_pegawai === 'pegawai'" class="md:col-span-2 bg-slate-50 dark:bg-slate-900/40 rounded-lg p-3 space-y-2">
            ...
            <div>
              <label class="block text-xs font-bold text-slate-500 mb-1 uppercase">Shift Kerja</label>
              <select v-model="form.shift" class="w-full px-3 py-2 text-sm rounded-xl border border-slate-300 bg-white focus:ring-2 focus:ring-amber-500 outline-none">
          ...
          <template v-if="showLembagaQiraati || showLembagaSekolah">
            ...
            <div v-if="form.lembaga">
              <label class="block text-xs font-bold text-slate-500 mb-1 uppercase">Shift Qiraati</label>
              <select v-model="form.shift" class="w-full px-3 py-2 text-sm rounded-xl border border-slate-300 bg-slate-50 focus:ring-2 focus:ring-amber-500 outline-none">
          ...
          <div>
            <label class="block text-xs font-bold text-slate-500 mb-1 uppercase">Tanggal Tugas</label>
            <input v-model="form.tanggal_tugas" type="date" class="w-full px-3 py-2 text-sm rounded-xl border border-slate-300 bg-slate-50 focus:ring-2 focus:ring-cyan-500 outline-none cursor-pointer" />
          </div>
        </div>
      </div>
      ...
      <div class="bg-white dark:bg-slate-800 rounded-2xl p-4 md:p-5 border border-slate-200 dark:border-slate-700 shadow-sm">
        ...
        <div class="grid grid-cols-1 md:grid-cols-3 gap-3">
          <div>
            ...
            <input v-model="form.nik" type="text" maxlength="16" placeholder="Diisi user nanti" class="w-full px-3 py-2 text-sm rounded-xl border border-slate-300 bg-slate-50 focus:ring-2 focus:ring-purple-500 outline-none font-mono" />
          </div>
          <div>
            <label class="block text-xs font-bold text-slate-500 mb-1 uppercase">No. WhatsApp *</label>
            <input v-model="form.wa" type="tel" required placeholder="08..." class="w-full px-3 py-2 text-sm rounded-xl border border-slate-300 bg-slate-50 focus:ring-2 focus:ring-purple-500 outline-none" />
          </div>
          <div>
            <label class="block text-xs font-bold text-slate-500 mb-1 uppercase">No. Syahadah Qiraati</label>
            <input v-model="form.no_ekgq" type="text" placeholder="Opsional" class="w-full px-3 py-2 text-sm rounded-xl border border-slate-300 bg-slate-50 focus:ring-2 focus:ring-purple-500 outline-none" />
          </div>
          <div>
            <label class="block text-xs font-bold text-slate-500 mb-1 uppercase">ID Fingerprint</label>
            <input v-model="form.id_fingerprint" type="text" placeholder="Opsional" class="w-full px-3 py-2 text-sm rounded-xl border border-slate-300 bg-slate-50 focus:ring-2 focus:ring-purple-500 outline-none" />
          </div>
        </div>
      </div>
      ...
      <div class="bg-white dark:bg-slate-800 rounded-2xl p-4 md:p-5 border border-slate-200 dark:border-slate-700 shadow-sm">
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-3">
          <div>
            ...
            <select v-model="form.status" class="w-full px-3 py-2 text-sm rounded-xl border border-slate-300 bg-slate-50 focus:ring-2 focus:ring-emerald-500 outline-none">
              <option value="Aktif">Aktif</option>
              <option value="Tidak Aktif">Tidak Aktif</option>
            </select>
          </div>
          <div v-if="isSuperAdmin">
            <label class="block text-xs font-bold text-fuchsia-700 mb-1 uppercase">Role Sistem (super-admin only)</label>
            <select v-model="form.role_sistem" class="w-full px-3 py-2 text-sm rounded-xl border border-fuchsia-300 bg-fuchsia-50 focus:ring-2 focus:ring-fuchsia-500 outline-none">
      ...
      <div class="bg-white dark:bg-slate-800 rounded-2xl p-3 border border-slate-200 dark:border-slate-700 shadow-sm sticky bottom-3 flex gap-2">
        ...
        <button type="submit" :disabled="isSaving" class="flex-1 px-4 py-3 bg-teal-600 hover:bg-teal-700 disabled:opacity-50 text-white font-bold rounded-xl text-sm shadow-md transition">
