  ...
  <div class="p-3 md:p-5 max-w-5xl mx-auto space-y-4">
      ...
      <div class="flex flex-col md:flex-row md:items-center md:justify-between gap-3">
        <div>
          <h1 class="text-base md:text-lg font-black">
            <i class="fas fa-balance-scale text-cyan-500 mr-2"></i>Hutang Piutang
          </h1>
    ...
    <div class="grid grid-cols-2 gap-3">
      <div class="bg-rose-50 rounded-2xl p-4 border-2 border-rose-200">
        <p class="text-[10px] uppercase font-black text-rose-700">
          <i class="fas fa-arrow-down mr-1"></i>Hutang (kewajiban)
        </p>
      ...
      <div class="bg-emerald-50 rounded-2xl p-4 border-2 border-emerald-200">
        <p class="text-[10px] uppercase font-black text-emerald-700">
          <i class="fas fa-arrow-up mr-1"></i>Piutang (klaim)
        </p>
    ...
    >
      <div class="px-4 py-3 border-b border-[var(--border-subtle)] grid grid-cols-2 gap-2">
        ...
        <select
          ...
          class="px-2 py-1.5 text-xs rounded-lg border border-[var(--border-default)] bg-[var(--bg-card)]"
        >
      ...
      <div v-if="filteredItems.length === 0" class="p-10 text-center">
        <i class="fas fa-inbox text-[var(--text-tertiary)] text-3xl mb-2"></i>
        <p class="text-sm text-[var(--text-secondary)] italic">Belum ada catatan.</p>
      </div>
        ...
        <div v-for="h in filteredItems" :key="h.id" class="p-3 flex items-center gap-3">
          <div
            :class="[
              ...
              h.jenis === 'piutang' ? 'bg-cyan-500' : 'bg-[var(--color-accent)]'
            ]"
          >
          ...
          <div class="flex-1 min-w-0">
            ...
            <p class="text-[10px] text-[var(--text-secondary)]">
              {{ h.jenis === 'piutang' ? 'Piutang' : 'Hutang' }} · {{ h.catatan || '-' }} ·
              {{ h.tanggal || '-' }}
            </p>
          </div>
          <div class="text-right">
            <p
              :class="[
                'text-sm font-black',
                h.jenis === 'piutang' ? 'text-cyan-700' : 'text-cyan-700'
              ]"
            >
              {{ fmtRp(h.nominal) }}
            </p>
            <p class="text-[9px] uppercase" :class="h.lunas ? 'text-emerald-600' : 'text-rose-600'">
              {{ h.lunas ? 'Lunas' : 'Open' }}
            </p>
          </div>
          <button
    ...
    >
      <div class="bg-[var(--bg-card)] rounded-2xl shadow-2xl max-w-md w-full p-5 space-y-3">
        <h3 class="text-base font-black">
          <i class="fas fa-balance-scale text-rose-500 mr-1"></i>Tambah Hutang/Piutang
        </h3>
        <select
          v-model="modalJenis"
        ...
        <textarea
          ...
          rows="2"
          class="w-full px-3 py-2 text-sm rounded-xl border border-[var(--border-default)] bg-[var(--bg-card)] resize-none"
        ></textarea>
...
async function simpan() {
  ...
  try {
    ...
    await setOne('keuangan_hutang_piutang', id, {
      id,
      jenis: modalJenis.value,
      pihak: modalPihak.value,
...
async function toggleLunas(h) {
  try {
    await mergeOne('keuangan_hutang_piutang', String(h.id), {
      lunas: !h.lunas,
      _last_update: serverTimestamp()
    })
    toast.success(h.lunas ? 'Set ke Open' : 'Set Lunas')
  } catch (e) {
    toast.error('Gagal: ' + (e?.message || e))
  }
}
...
async function deleteItem(h) {
  ...
  const ok = await confirmDlg({
    title: 'Hapus?',
    message: `Hapus ${h.jenis} dari ${h.pihak}?`,
    confirmText: 'Hapus',
    danger: true
  })
  if (!ok) return
  try {
    await deleteOne('keuangan_hutang_piutang', h.id)
...
onMounted(() => {
  unsub = subscribeColl('keuangan_hutang_piutang', (docs) => {
    items.value = docs
  })
})
