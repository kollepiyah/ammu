...
const ENTITIES = [
  {
    type: 'santri',
    label: 'Santri',
    file: 'DATA SANTRI AUTO NEW.xlsx (sheet DATA UTAMA)',
    cols: [
      ['nama', 'Nama'],
      ['jk', 'JK'],
      ['lembaga', 'Lembaga'],
  ...
  },
  {
    type: 'guru',
    label: 'Guru',
    file: 'DATA GURU AUTO.xlsx (sheet DATA GURU)',
    cols: [
      ['nama', 'Nama'],
      ['jabatan', 'Jabatan'],
      ['lembaga', 'Lembaga'],
  ...
  },
  {
    ...
    file: 'PRESTASI PTPT AUTO.xlsx (sheet Rekap Prestasi)',
    cols: [
      ['_nama', 'Nama'],
      ['lembaga', 'Lembaga'],
      ['kelas', 'Kelas'],
...
// Objek BIASA (bukan reactive): reactive() meng-unwrap ref -> `imp[type].preview.value`
// jadi `null.value` (TypeError render). Ref-nya sendiri sudah reaktif, jadi `.value`
// di template tetap memicu re-render dengan benar.
const imp = {}
ENTITIES.forEach((e) => {
  imp[e.type] = useSupabaseImport()
})
...
async function onFile(ev, type) {
  ...
  if (!file) return
  try {
    await imp[type].loadPreview(file, type)
  } catch (e) {
    /* error sudah di state */
  }
...
<template>
  <div class="p-4 max-w-5xl mx-auto">
    ...
    <p class="text-sm text-slate-500 dark:text-slate-400 mt-1">
      Alat migrasi (super admin). Pilih file Excel dari folder <b>Ammu Data</b>, cek pratinjau, lalu
      impor. Upsert by <code>id</code> — aman diulang.
    </p>
    ...
    >
      <div class="flex flex-wrap items-center justify-between gap-2">
        ...
        <label
          class="h-11 md:h-9 px-3 inline-flex items-center rounded-xl bg-teal-600 hover:bg-teal-700 text-white text-sm font-medium cursor-pointer"
        >
          Pilih File…
      ...
      <div v-if="imp[e.type].error.value" class="mt-3 text-sm text-red-600 dark:text-red-400">
        ⚠️ {{ imp[e.type].error.value }}
      </div>
      <div v-if="imp[e.type].result.value" class="mt-3 text-sm text-green-600 dark:text-green-400">
        ✓ {{ imp[e.type].result.value }}
      </div>

      <template v-if="imp[e.type].preview.value">
        <div class="mt-3 text-sm text-slate-600 dark:text-slate-300">
          Total baris: <b>{{ imp[e.type].preview.value.total }}</b> · Termap:
          <b>{{ imp[e.type].preview.value.rows.length }}</b> · Dilewati (tanpa nama):
          {{ imp[e.type].preview.value.skipped }} · Duplikat-id:
          {{ imp[e.type].preview.value.duplicates }}
        </div>
        <div class="mt-2 overflow-x-auto">
          <table class="w-full text-xs border-collapse">
            ...
            <tbody>
              <tr
                v-for="(row, i) in imp[e.type].preview.value.rows.slice(0, 8)"
                :key="i"
              ...
              >
                <td
                  v-for="c in e.cols"
                  :key="c[0]"
                  class="py-1 pr-3 text-slate-700 dark:text-slate-200"
                >
                  {{ cellOf(row, c[0]) }}
                </td>
              </tr>
