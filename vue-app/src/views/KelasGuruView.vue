...
import { subscribeColl, subscribeDoc, updateOne } from '@/services/db'
...
function matchNgaji(s, lembaga) {
  const sl = String(s.lembaga || '')
    .toLowerCase()
    .trim()
  const target = String(lembaga || '')
    .toLowerCase()
    .trim()
...
const guruForLembaga = computed(() => {
  const target = String(selectedLembaga.value || '')
    .toLowerCase()
    .trim()
  let list = (guruRaw.value || []).filter((g) => {
    ...
    const gl = String(g.lembaga || '')
      .toLowerCase()
      .trim()
    const gls = String(g.lembaga_sekolah || '')
      .toLowerCase()
      .trim()
...
const santriKandidat = computed(() => {
  ...
  if (kategori.value === 'ngaji') {
    list = list.filter(
      (s) => matchNgaji(s, selectedLembaga.value) && String(s.kelas || '') === selectedKelas.value
    )
    return sortSantri(list, { lembagaField: 'lembaga', kelasField: 'kelas' })
  }
  list = list.filter(
    (s) =>
      matchSekolah(s, selectedLembaga.value) &&
      String(s.kelas_sekolah || s.kelas || '') === selectedKelas.value
  )
...
async function tambahKelas() {
  ...
  if (!nama) return
  if (!lembagaObj.value) {
    toast.warning('Lembaga tidak ditemukan di master')
    return
  }
  if ((kelasOptions.value || []).includes(nama)) {
    toast.warning('Kelas sudah ada')
    return
  }
...
async function simpan() {
  if (!selectedKelas.value) {
    toast.warning('Pilih kelas dulu')
    return
  }
  if (!adaGuruDipilih.value) {
    toast.warning('Pilih minimal 1 guru')
    return
  }
  saving.value = true
  let changed = 0
...
async function downloadTemplateAssign() {
  ...
  try {
    ...
    const rows = list.map((s) => ({
      ...
      guru_sekolah_1: Array.isArray(s.guru_sekolah)
        ? s.guru_sekolah[0] || ''
        : s.guru_sekolah || '',
    ...
    toast.success(
      `Template berisi ${rows.length} santri. Isi kolom guru lalu impor. Tanda "-" = kosongkan guru.`
    )
  } catch (e) {
...
async function onImportAssign(e) {
  ...
  try {
    ...
    for (const r of rows) {
      ...
      const nis = String(
        _pickCell(r, 'No. Induk', 'No Induk', 'no_induk', 'NIS', 'nis') || ''
      ).trim() // v.100: terima header baru + template lama
      const nama = String(_pickCell(r, 'Nama Santri', 'Nama', 'nama') || '')
        .trim()
        .toLowerCase()
      ...
      if (g1 !== undefined || g2 !== undefined) {
        const cur = Array.isArray(s.guru_sekolah)
          ? s.guru_sekolah
          : s.guru_sekolah
            ? [s.guru_sekolah]
            : []
    ...
    toast.success(
      `Impor assign selesai: ${updated} santri diperbarui${fuzzyMatched ? ` (${fuzzyMatched} via nama mirip — cek bila ragu)` : ''}, ${skipped} dilewati (kolom guru kosong), ${notFound} tak ditemukan (No. Induk/nama).`
    )
  } catch (e2) {
...
<template>
  <div class="p-3 md:p-5 space-y-4">
    <div class="bg-[var(--bg-card)] rounded-2xl p-4 border border-[var(--border-subtle)] shadow-sm">
      <div class="flex items-center justify-between gap-2 flex-wrap">
        ...
        <div class="flex items-center gap-2 flex-wrap">
          ...
          >
            <i
              :class="['fas mr-1', importingAssign ? 'fa-spinner fa-spin' : 'fa-file-arrow-up']"
            ></i>
            {{
              importingAssign
                ? `Impor… ${importAssignProgress.i}/${importAssignProgress.total}`
                : 'Impor Assign'
            }}
            <input
              type="file"
    ...
    >
      ...
      <div
        v-if="lembagaOptions.length === 0"
        class="text-xs italic text-[var(--text-tertiary)] py-4 text-center"
      >
        Tidak ada lembaga.
      </div>
    ...
    >
      ...
      <div
        ...
        class="text-xs italic text-[var(--text-tertiary)] py-3 text-center"
      >
    ...
    >
      ...
      <div class="grid grid-cols-1 sm:grid-cols-2 gap-3 mb-4">
        <template v-if="kategori === 'ngaji'">
          <div>
            ...
            <select
              v-model="guruPagi"
              class="w-full px-3 py-2 text-sm rounded-lg border border-[var(--border-default)] bg-[var(--bg-input)] text-[var(--text-primary)] cursor-pointer"
            >
              <option value="">— tidak ada —</option>
              <option v-for="g in guruForLembaga" :key="'p-' + g.id" :value="g.nama">
                {{ g.nama }}
              </option>
            </select>
          </div>
          <div>
            ...
            >
              <option value="">— tidak ada —</option>
              <option v-for="g in guruForLembaga" :key="'s-' + g.id" :value="g.nama">
                {{ g.nama }}
              </option>
            </select>
          </div>
        ...
        <template v-else>
          <div>
            <label class="block text-[11px] font-bold text-[var(--text-secondary)] uppercase mb-1"
              >Guru Sekolah 1</label
            >
            <select
              v-model="guruSekolah1"
              class="w-full px-3 py-2 text-sm rounded-lg border border-[var(--border-default)] bg-[var(--bg-input)] text-[var(--text-primary)] cursor-pointer"
            >
              <option value="">— tidak ada —</option>
              <option v-for="g in guruForLembaga" :key="'a-' + g.id" :value="g.nama">
                {{ g.nama }}
              </option>
            </select>
          </div>
          <div>
            <label class="block text-[11px] font-bold text-[var(--text-secondary)] uppercase mb-1"
              >Guru Sekolah 2</label
            >
            <select
              v-model="guruSekolah2"
              class="w-full px-3 py-2 text-sm rounded-lg border border-[var(--border-default)] bg-[var(--bg-input)] text-[var(--text-primary)] cursor-pointer"
            >
              <option value="">— tidak ada —</option>
              <option v-for="g in guruForLembaga" :key="'b-' + g.id" :value="g.nama">
                {{ g.nama }}
              </option>
            </select>
          </div>
      ...
      <p
        v-else-if="santriKandidatTampil.length === 0"
        class="text-xs italic text-[var(--text-tertiary)] py-4 text-center"
      >
        ...
        >
          <i
            :class="[
              'fas',
              checked.has(s.id)
                ? 'fa-square-check text-teal-600'
                : 'fa-square text-[var(--text-tertiary)]'
            ]"
          ></i>
          <span class="flex-1 text-sm font-bold text-[var(--text-primary)] truncate">{{
            s.nama
          }}</span>
      ...
      <p class="text-[10px] text-[var(--text-tertiary)] text-center mt-2">
        Guru kelas disimpan sbg default (dipakai prefill form santri). Penugasan menulis guru ke
        santri tercentang.
      </p>
    </div>
  </div>
