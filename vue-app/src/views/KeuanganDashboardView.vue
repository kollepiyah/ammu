    ...
    <template v-else>
      ...
      >
        <h1 class="text-base md:text-lg font-black text-[var(--text-primary)]">
          ...
          <span v-if="gedungScoped" class="text-teal-600">— {{ myGedung }}</span>
        </h1>
        <p class="text-xs text-[var(--text-secondary)] mt-0.5">
          <template v-if="gedungScoped">Keuangan gedung Anda realtime + tren 12 bulan</template>
          <template v-else>Overview keuangan pondok realtime + tren 12 bulan</template>
        </p>
      </div>
      ...
      <div class="grid grid-cols-4 md:grid-cols-7 gap-2 md:gap-3">
        <router-link
          to="/pos-santri"
          class="group relative overflow-hidden bg-gradient-to-br from-rose-500 dark:from-rose-700 to-rose-700 dark:to-rose-900 rounded-xl p-2.5 md:p-3 text-white shadow-sm hover:shadow-md hover:-translate-y-0.5 transition-all flex flex-col items-center justify-center gap-1 text-center"
        >
        ...
        <router-link
          to="/tabungan"
          class="group relative overflow-hidden bg-gradient-to-br from-emerald-500 dark:from-emerald-700 to-emerald-700 dark:to-emerald-900 rounded-xl p-2.5 md:p-3 text-white shadow-sm hover:shadow-md hover:-translate-y-0.5 transition-all flex flex-col items-center justify-center gap-1 text-center"
        >
        ...
        <router-link
          to="/bisyaroh"
          class="group relative overflow-hidden bg-gradient-to-br from-cyan-500 dark:from-cyan-700 to-cyan-700 dark:to-cyan-900 rounded-xl p-2.5 md:p-3 text-white shadow-sm hover:shadow-md hover:-translate-y-0.5 transition-all flex flex-col items-center justify-center gap-1 text-center"
        >
          <i class="fas fa-hand-holding-usd text-base md:text-lg drop-shadow"></i>
          <p class="text-[10px] md:text-xs font-black leading-tight drop-shadow-sm">
            Bisyaroh Guru
          </p>
        </router-link>
        <router-link
        ...
        <router-link
          to="/laporan-keuangan"
          class="group relative overflow-hidden bg-gradient-to-br from-[var(--color-primary)] to-[var(--color-primary-hover)] rounded-xl p-2.5 md:p-3 text-white shadow-sm hover:shadow-md hover:-translate-y-0.5 transition-all flex flex-col items-center justify-center gap-1 text-center"
        >
        ...
        <router-link
          to="/keu-pengaturan"
          class="group relative overflow-hidden bg-gradient-to-br from-slate-500 to-slate-700 rounded-xl p-2.5 md:p-3 text-white shadow-sm hover:shadow-md hover:-translate-y-0.5 transition-all flex flex-col items-center justify-center gap-1 text-center"
        >
      ...
      <div class="grid grid-cols-2 md:grid-cols-4 gap-3">
        <router-link
          to="/tagihan"
          class="block bg-[var(--bg-card)] p-4 rounded-2xl border border-[var(--border-subtle)] shadow-sm hover:shadow-md transition"
        >
          <p class="text-[10px] uppercase font-bold text-[var(--text-secondary)] tracking-wider">
            Tagihan Aktif
          </p>
          <p class="text-2xl font-black text-rose-600 dark:text-rose-400 mt-1">
            {{ stats.tagihanCount }}
          </p>
          <p class="text-[11px] text-[var(--text-secondary)] mt-1 font-bold">
            {{ fmtRp(stats.totalTagihan) }}
          </p>
        </router-link>
        <router-link
          to="/tabungan"
        ...
        >
          ...
          <p class="text-2xl font-black text-emerald-600 dark:text-emerald-400 mt-1">
            {{ tabunganSantri.length }}
          </p>
          <p class="text-[11px] text-[var(--text-secondary)] mt-1 font-bold">
            {{ fmtRp(stats.totalTabunganSantri) }}
          </p>
        </router-link>
        <router-link
          to="/bisyaroh"
        ...
        >
          ...
          <p class="text-[11px] text-[var(--text-secondary)] mt-1 font-bold">
            {{ fmtRp(stats.totalBisyarohBulan) }}
          </p>
          <!-- v.1.3.6: bisyaroh global (tanpa dimensi gedung) — ditandai supaya admin
               ber-gedung tak mengira ini angka gedungnya. -->
          <p v-if="gedungScoped" class="text-[10px] text-[var(--text-tertiary)] italic mt-0.5">
            semua gedung
          </p>
        </router-link>
        <router-link
        ...
        >
          ...
          <p class="text-2xl font-black text-cyan-600 dark:text-cyan-400 mt-1">
            {{ bukuIndukValid.length }}
          </p>
      ...
      <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
        ...
        >
          <h3
            class="text-sm font-black text-slate-700 dark:text-[var(--text-tertiary)] uppercase tracking-wide mb-2"
          >
            <i class="fas fa-chart-column text-emerald-500 mr-1"></i>Pemasukan vs Pengeluaran (12
            Bulan)
          </h3>
          <div class="flex gap-3 text-[10px] font-bold mb-1">
            <span class="flex items-center gap-1"
              ><span class="w-3 h-3 rounded bg-emerald-500 inline-block"></span>Pemasukan</span
            >
            <span class="flex items-center gap-1"
              ><span class="w-3 h-3 rounded bg-rose-500 inline-block"></span>Pengeluaran</span
            >
          </div>
          <svg
            width="100%"
          ...
          >
            <g v-for="(line, i) in barGrid" :key="`g-${i}`">
              ...
              />
              <text :x="35" :y="line.y + 4" text-anchor="end" font-size="9" fill="#64748b">
                {{ line.label }}
              </text>
            </g>
            <g v-for="(b, i) in bar12" :key="`b-${i}`">
              ...
              <rect
                ...
                rx="2"
              />
              <text :x="b.x + b.w / 2" y="215" text-anchor="middle" font-size="9" fill="#64748b">
                {{ b.label }}
              </text>
            </g>
          </svg>
        ...
        >
          ...
          <p class="text-[10px] text-[var(--text-secondary)] mb-1">
            Saldo: {{ fmtRp(saldoData.last) }}
          </p>
          <svg
            width="100%"
            height="220"
          ...
          >
            <g v-for="(line, i) in saldoGrid" :key="`sg-${i}`">
              <line
                ...
                stroke-dasharray="3,3"
              />
              <text :x="45" :y="line.y + 4" text-anchor="end" font-size="9" fill="#64748b">
                {{ line.label }}
              </text>
            </g>
            <polyline
              :points="saldoData.pathPoints"
              fill="none"
              stroke="#3b82f6"
              stroke-width="2"
            />
            <polyline
              :points="saldoData.fillPoints"
              fill="#3b82f6"
              fill-opacity="0.1"
              stroke="none"
            />
            <g v-for="(p, i) in saldoData.points" :key="`sp-${i}`">
              ...
              <text :x="p.x" y="215" text-anchor="middle" font-size="9" fill="#64748b">
                {{ p.label }}
              </text>
            </g>
          </svg>
      ...
      >
        <h3
          class="text-sm font-black text-slate-700 dark:text-[var(--text-tertiary)] uppercase tracking-wide mb-3"
        >
          <i class="fas fa-chart-pie text-rose-500 mr-1"></i>Breakdown Kategori Pengeluaran (Bulan
          Ini)
        </h3>
        <div
          v-if="kategoriBulanIni.length === 0"
          class="text-center py-6 text-xs text-[var(--text-tertiary)] italic"
        >
        ...
        <div v-else class="grid grid-cols-1 md:grid-cols-2 gap-4 items-center">
          ...
          <div class="space-y-1.5">
            <div v-for="(k, i) in kategoriBulanIni" :key="k.nama" class="flex items-center gap-2">
              ...
              ></span>
              <p class="text-xs font-bold text-[var(--text-primary)] flex-1 truncate">
                {{ k.nama }}
              </p>
