<template>
  ...
  <div class="p-3 md:p-5 max-w-6xl mx-auto space-y-4">
    ...
    <div v-if="usesYear" class="flex items-center gap-2">
      ...
      <select v-model.number="year" @change="reloadActive" class="px-3 py-1.5 rounded-lg border border-[var(--border-default)] bg-[var(--bg-card)] text-sm">
    ...
    <div v-show="!loading && tab === 'santri'" class="grid grid-cols-1 lg:grid-cols-2 gap-4">
      <div :class="cardCls">
        ...
        <div class="relative h-72"><Bar v-if="cSantriLembaga" :data="cSantriLembaga" :options="optBar" /><div v-else :class="emptyCls">Belum ada data</div></div>
      </div>
      <div :class="cardCls">
    ...
    <div v-show="!loading && tab === 'akademik'" class="grid grid-cols-1 lg:grid-cols-2 gap-4">
      <div :class="cardCls">
        <h3 :class="titleCls"><i class="fas fa-clipboard-check text-teal-600 mr-1"></i>Status Tes Kenaikan</h3>
        <div class="relative h-72"><Doughnut v-if="cTesStatus" :data="cTesStatus" :options="optDoughnut" /><div v-else :class="emptyCls">Belum ada data</div></div>
      </div>
      <div :class="cardCls">
        <h3 :class="titleCls"><i class="fas fa-graduation-cap text-teal-600 mr-1"></i>Rapor Terbit per Lembaga</h3>
    ...
    <div v-show="!loading && tab === 'pegawai'" class="space-y-4">
      <div class="grid grid-cols-1 lg:grid-cols-2 gap-4">
        <div :class="cardCls">
          <h3 :class="titleCls"><i class="fas fa-chalkboard-teacher text-teal-600 mr-1"></i>Pegawai per Lembaga</h3>
          <div class="relative h-72"><Bar v-if="cPegLembaga" :data="cPegLembaga" :options="optBar" /><div v-else :class="emptyCls">Belum ada data</div></div>
        </div>
        <div :class="cardCls">
          <h3 :class="titleCls"><i class="fas fa-user-tie text-teal-600 mr-1"></i>Pegawai per Jabatan</h3>
          <div class="relative h-72"><Bar v-if="cPegJabatan" :data="cPegJabatan" :options="optBar" /><div v-else :class="emptyCls">Belum ada data</div></div>
        </div>
      </div>
      <div :class="cardCls">
...
const emptyCls = 'absolute inset-0 flex items-center justify-center text-xs text-[var(--text-tertiary)] italic'
...
// chart refs
const cSantriLembaga = ref(null), cSantriMukim = ref(null)
const ringkas = ref({ masuk: 0, keluar: 0 }), cKeuBulanan = ref(null)
const cTesStatus = ref(null), cRaporLembaga = ref(null)
const cAbsNgaji = ref(null), cAbsSekolah = ref(null)
const cPegLembaga = ref(null), cPegJabatan = ref(null), cGajiBulanan = ref(null)
...
const loadSantri = () => run(async () => {
  const [a, b] = await Promise.all([analyticsQuery('santri_per_lembaga'), analyticsQuery('santri_mukim')])
  cSantriLembaga.value = catChart(a)
  cSantriMukim.value = b.length ? { labels: b.map((r) => r.label), datasets: [{ data: b.map((r) => num(r.value)), backgroundColor: ['#10b981', '#94a3b8'] }] } : null
})
const loadKeuangan = () => run(async () => {
  const [r, m] = await Promise.all([analyticsQuery('keuangan_ringkas'), analyticsQuery('keuangan_bulanan', { year: year.value })])
  const x = r[0] || {}
...
const loadAkademik = () => run(async () => {
  const [t, r] = await Promise.all([analyticsQuery('akademik_tes_status'), analyticsQuery('akademik_rapor_per_lembaga')])
  cTesStatus.value = t.length ? { labels: t.map((x) => x.label), datasets: [{ data: t.map((x) => num(x.value)), backgroundColor: COLORS }] } : null
  cRaporLembaga.value = catChart(r)
})
const loadAbsensi = () => run(async () => {
...
const optCurrency = {
  ...
  scales: { x: { ticks: { font: { size: 9 } } }, y: { beginAtZero: true, ticks: { font: { size: 9 }, callback: (v) => 'Rp ' + new Intl.NumberFormat('id-ID', { notation: 'compact' }).format(v) } } }
}
</script>
