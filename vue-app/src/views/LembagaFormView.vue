    ...
    <div class="bg-white dark:bg-slate-800 rounded-2xl p-4 md:p-5 border border-slate-200 dark:border-slate-700 shadow-sm flex items-center justify-between gap-3">
      <div>
        <h1 class="text-lg md:text-xl font-black text-slate-800 dark:text-white">
          <i :class="editingId ? 'fas fa-edit text-amber-500' : 'fas fa-building text-teal-500'" class="mr-2"></i>
          {{ editingId ? `Edit Lembaga: ${form.lembaga}` : 'Tambah Lembaga Baru' }}
        </h1>
        <p class="text-xs text-slate-500 mt-0.5">Master data lembaga + daftar kelas/jilid</p>
      </div>
      <router-link to="/lembaga" class="px-3 py-2 text-sm bg-slate-100 dark:bg-slate-700 hover:bg-slate-200 text-slate-700 dark:text-slate-300 font-bold rounded-xl transition">
    ...
    <form v-else @submit.prevent="onSubmit" class="space-y-4">
      ...
      <div class="bg-white dark:bg-slate-800 rounded-2xl p-4 md:p-5 border border-slate-200 dark:border-slate-700 shadow-sm">
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-3">
          <div>
            <label class="block text-xs font-bold text-slate-500 mb-1 uppercase">Nama Lembaga *</label>
            <input
              ...
              class="w-full px-3 py-2 text-sm rounded-xl border border-slate-300 bg-slate-50 disabled:opacity-60 focus:ring-2 focus:ring-teal-500 outline-none"
            />
            <p v-if="editingId" class="text-[10px] text-amber-600 mt-1">
              <i class="fas fa-info-circle mr-1"></i>Nama lembaga tidak bisa diubah saat edit (sebagai ID)
            </p>
          </div>
          <div>
            <label class="block text-xs font-bold text-slate-500 mb-1 uppercase">Tipe Lembaga *</label>
            <select v-model="form.tipe" required class="w-full px-3 py-2 text-sm rounded-xl border border-slate-300 bg-slate-50 focus:ring-2 focus:ring-teal-500 outline-none">
      ...
      <div class="bg-teal-50/40 dark:bg-teal-900/20 rounded-2xl p-4 md:p-5 border-2 border-teal-300 dark:border-teal-700 shadow-sm">
        ...
        <div class="flex gap-2 mb-3">
          <input
            ...
            class="flex-1 px-3 py-2 text-sm rounded-xl border border-teal-300 bg-white focus:ring-2 focus:ring-teal-500 outline-none"
          />
          <button
            type="button"
            @click="onAddKelas"
        ...
        <!-- Kelas list -->
        <div v-if="form.kelas.length === 0" class="text-center text-xs text-slate-400 italic py-6 bg-white/40 rounded-xl border border-dashed border-teal-300">
        ...
        <div v-else class="space-y-1.5">
          <div
            ...
            :key="idx"
            class="flex items-center gap-2 bg-white border border-teal-200 rounded-lg p-2"
          >
            <span class="text-[10px] bg-teal-100 text-teal-700 font-black px-2 py-0.5 rounded">#{{ idx + 1 }}</span>
            <span class="flex-1 text-sm font-bold text-slate-800">{{ k }}</span>
            <button
              type="button"
              :disabled="idx === 0"
              @click="moveKelas(idx, idx - 1)"
              class="w-7 h-7 rounded-lg bg-slate-100 hover:bg-slate-200 disabled:opacity-30 text-slate-600 text-xs transition"
              title="Naik"
            >
      ...
      <div class="bg-fuchsia-50/40 dark:bg-fuchsia-900/20 rounded-2xl p-4 md:p-5 border-2 border-fuchsia-300 dark:border-fuchsia-700 shadow-sm">
        <div class="flex items-center justify-between mb-3">
          <h3 class="text-xs font-black text-fuchsia-800 dark:text-fuchsia-300 uppercase tracking-wide">
            <i class="fas fa-user-tie text-fuchsia-600 mr-1"></i>Daftar Jabatan
            <span class="ml-1 text-[10px] bg-fuchsia-100 text-fuchsia-700 px-1.5 py-0.5 rounded">{{ form.jabatan.length }}</span>
          </h3>
        </div>
        <p class="text-[10px] text-slate-500 mb-2"><i class="fas fa-info-circle mr-1"></i>Jabatan yang tersedia di lembaga ini (contoh: Kepala TPQ, Wali Kelas, PJ Administrasi). Pilih <b>tipe</b> untuk filter di Form Guru.</p>
        ...
        <div class="grid grid-cols-[1fr_140px_auto] gap-2 mb-3">
          <input
            ...
            class="px-3 py-2 text-sm rounded-xl border border-fuchsia-300 bg-white focus:ring-2 focus:ring-fuchsia-500 outline-none"
          />
          <select v-model="newJabatanTipe" class="px-2 py-2 text-sm rounded-xl border border-fuchsia-300 bg-white cursor-pointer focus:ring-2 focus:ring-fuchsia-500 outline-none">
          ...
          <button
            type="button"
            @click="onAddJabatan"
            class="px-4 py-2 bg-fuchsia-600 hover:bg-fuchsia-700 text-white font-bold rounded-xl text-sm transition whitespace-nowrap"
          >
            <i class="fas fa-plus mr-1"></i>Tambah
          </button>
        </div>
        ...
        <!-- Jabatan list -->
        <div v-if="form.jabatan.length === 0" class="text-center text-xs text-slate-400 italic py-6 bg-white/40 rounded-xl border border-dashed border-fuchsia-300">
        ...
        <div v-else class="space-y-1.5">
          <div
            ...
            :key="`jab-${idx}`"
            class="flex items-center gap-2 bg-white border border-fuchsia-200 rounded-lg p-2"
          >
            <span class="text-[10px] bg-fuchsia-100 text-fuchsia-700 font-black px-2 py-0.5 rounded">#{{ idx + 1 }}</span>
            <span class="flex-1 text-sm font-bold text-slate-800">{{ typeof j === 'string' ? j : j.nama }}</span>
            <select
              :value="(typeof j === 'object' && j.tipe) || 'guru'"
              @change="updateJabatanTipe(idx, $event.target.value)"
              class="text-[11px] font-bold px-2 py-1 rounded-lg border border-fuchsia-200 bg-fuchsia-50 cursor-pointer focus:ring-2 focus:ring-fuchsia-500 outline-none"
              title="Tipe jabatan"
            >
              <option value="guru">Guru</option>
              <option value="pegawai">Pegawai</option>
      ...
      <div class="bg-white dark:bg-slate-800 rounded-2xl p-3 border border-slate-200 dark:border-slate-700 shadow-sm sticky bottom-3 flex gap-2">
        <router-link to="/lembaga" class="flex-1 text-center px-4 py-3 bg-slate-100 dark:bg-slate-700 hover:bg-slate-200 text-slate-700 dark:text-slate-300 font-bold rounded-xl text-sm shadow-md transition">
