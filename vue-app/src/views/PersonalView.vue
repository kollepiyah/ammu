<template>
  <div class="p-3 md:p-5 max-w-5xl mx-auto space-y-4">
    <div class="bg-white dark:bg-slate-800 rounded-2xl p-4 border border-slate-200 dark:border-slate-700 shadow-sm">
      <h1 class="text-xl md:text-2xl font-black"><i class="fas fa-user-circle text-indigo-500 mr-2"></i>Dashboard Personal</h1>
      <p class="text-xs text-slate-500 mt-0.5">{{ auth.sesiAktif?.nama || '-' }} · {{ auth.sesiAktif?.jabatan || 'Admin' }}</p>
    </div>
    ...
    <div class="bg-white rounded-2xl border border-slate-200 shadow-sm overflow-hidden">
      <div class="px-4 py-3 border-b border-slate-100"><h3 class="text-sm font-black"><i class="fas fa-history text-indigo-500 mr-1"></i>Riwayat Slip Bisyaroh</h3></div>
      <div v-if="slipTahunIni.length === 0" class="p-8 text-center text-sm text-slate-500 italic">Belum ada slip.</div>
      <div v-else class="divide-y divide-slate-100">
        <div v-for="g in slipTahunIni" :key="g.id" class="p-3 flex items-center justify-between">
          <div><p class="text-sm font-bold">{{ g.periode }}</p><p class="text-[10px] text-slate-500">{{ g.lembaga }} · {{ g.jabatan }}</p></div>
