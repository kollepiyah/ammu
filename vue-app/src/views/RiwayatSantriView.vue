  ...
  <div class="p-3 md:p-5 max-w-4xl mx-auto space-y-4">
    ...
    <template v-else>
      ...
      >
        <div>
          <p
            class="text-[10px] uppercase font-black text-[var(--text-secondary)] mb-1.5 tracking-wider"
          >
          ...
          <div class="flex items-center gap-2 mt-2 flex-wrap">
            ...
            <span class="text-[10px] text-[var(--text-tertiary)] font-bold ml-auto">{{
              rangeLabel
            }}</span>
          </div>
        </div>
        ...
        <div class="pt-3 border-t border-[var(--border-subtle)]">
          ...
          <p class="text-[10px] text-[var(--text-tertiary)] font-bold mt-1.5">
          ...
          >
            ...
            <div class="flex-1 min-w-0">
              ...
              <p class="text-[10px] text-[var(--text-secondary)] truncate">
                No. Induk {{ s.nis || s.id }} · {{ s.lembaga || '-' }} · {{ s.kelas || '-' }}
              </p>
            </div>
      ...
      <div
        v-if="lastDebug"
        class="bg-[var(--bg-card-elevated)] rounded-xl p-3 text-[10px] text-[var(--text-secondary)] border border-[var(--border-subtle)]"
      >
        <p class="font-black mb-1"><i class="fas fa-info-circle mr-1"></i>Hasil ekspor terakhir:</p>
        <p>
          Tagihan: <b>{{ lastDebug.tagihan }}</b> · POS: <b>{{ lastDebug.pembayaran }}</b> ·
          Tabungan: <b>{{ lastDebug.tabungan }}</b>
        </p>
        <p
          v-if="lastDebug.tagihan === 0 && lastDebug.pembayaran === 0 && lastDebug.tabungan === 0"
          class="text-rose-600 mt-1"
        >
          Tidak ada data di rentang ini. Cek tanggal record atau coba periode lebih lebar (klik "1
          tahun").
        </p>
      </div>
    </template>
...
const sampaiStr = computed(
  () =>
    `${tahunSampai.value}-${pad2(bulanSampai.value)}-${pad2(lastDayOfMonth(tahunSampai.value, bulanSampai.value))}`
)

const rangeLabel = computed(
  () =>
    `${BULAN[bulanDari.value - 1]} ${tahunDari.value} → ${BULAN[bulanSampai.value - 1]} ${tahunSampai.value}`
)
...
function validateRange() {
  ...
  if (a > b) {
    toast.warning('Rentang tidak valid: "Dari" lebih besar dari "Sampai"')
    return false
  }
  return true
}
...
async function cetakRiwayat(s) {
  ...
  try {
    ...
    if (stats.tagihan + stats.pembayaran + stats.tabungan === 0) {
      ...
      toast.success(
        `PDF ${s.nama} — ${stats.tagihan} tagihan, ${stats.pembayaran} POS, ${stats.tabungan} mutasi`
      )
    }
  } catch (e) {
