<template>
  <div class="p-3 md:p-5 max-w-6xl mx-auto space-y-4">
    ...
    <PageHeader
      ...
      icon="fa-users"
      title="Data Santri"
      :subtitle="isFullAccess ? 'Semua santri pondok' : 'Santri yang Anda ampu'"
    >
      <template #stats>
        <div
      ...
      >
        <div class="p-4 border-b border-[var(--border-subtle)] flex items-center justify-between">
          <h3 class="text-base font-black text-[var(--text-primary)]">
            <i class="fas fa-file-import text-cyan-600 mr-2"></i>Preview Impor —
    ...
    >
      ...
      <div class="relative">
        ...
        <input
          v-model="search"
          type="text"
          placeholder="Cari nama, No. Induk, atau wali..."
      ...
      <div
        ...
        class="mt-2 pt-2 border-t border-[var(--border-subtle)] flex items-center gap-2"
      >
    ...
    <div v-else class="space-y-2">
      <template v-for="s in santriRows" :key="s.id">
        ...
        <div v-if="sectionHeaderFor(s)" class="flex items-center gap-2 pt-2 pb-1 first:pt-0">
          <span class="text-xs font-black text-teal-700 dark:text-teal-300 uppercase tracking-wider"
            ><i class="fas fa-layer-group mr-1.5"></i>{{ sectionHeaderFor(s) }}</span
          >
          ...
          <div class="flex items-start gap-3">
            ...
            <div class="flex-1 min-w-0">
              <div class="flex items-start justify-between gap-2">
                ...
                <span
                  v-if="s.is_mukim"
                  class="text-[9px] bg-teal-100 dark:bg-teal-900/40 text-teal-700 dark:text-teal-300 border border-teal-200 dark:border-teal-700 px-2 py-0.5 rounded font-black uppercase tracking-wider flex-shrink-0"
                  >Ma'had</span
                >
                <span
                  v-else-if="s.is_fullday"
                  class="text-[9px] bg-cyan-100 dark:bg-cyan-900/40 text-cyan-700 dark:text-cyan-300 border border-cyan-200 dark:border-cyan-700 px-2 py-0.5 rounded font-black uppercase tracking-wider flex-shrink-0"
                  >Fullday</span
                >
                <span
                  v-if="s.aktif === false"
...
async function exportSantriExcel() {
  ...
  try {
    ...
    const _g = (s) =>
      Array.isArray(s.guru_sekolah) ? s.guru_sekolah.join(' | ') : s.guru_sekolah || ''
    const rows = (santri.value || []).map((s, i) => ({
      no: i + 1,
      nis: s.nis || '',
      nis_sekolah: s.nis_sekolah || '',
      nisn: s.nisn || '',
      nik: s.nik || '',
      nama: s.nama || '',
      ...
      alamat: s.alamat || '',
      dusun: s.alamat_dusun || '',
      rt: s.alamat_rt || '',
      rw: s.alamat_rw || '',
      desa: s.alamat_desa || '',
      kecamatan: s.alamat_kecamatan || '',
      kabupaten: s.alamat_kabupaten || '',
      provinsi: s.alamat_provinsi || '',
      aktif: s.aktif === false ? 'false' : 'true',
      mukim: s.is_mukim ? 'true' : 'false',
      fullday: s.is_fullday ? 'true' : 'false',
      catatan: s.catatan_riwayat_pribadi || '',
    ...
    for (const r of rows) {
      ...
      allMapped.push({
        ...
        data: {
          ...
          // v.99: status keluar
          tgl_keluar: parseTglDDMMYYYY(
            _pick(r, 'Tgl Keluar (DD/MM/YYYY)', 'Tgl Keluar', 'tgl_keluar')
          ),
