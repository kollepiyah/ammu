  ...
  <div class="p-3 md:p-5 max-w-5xl mx-auto space-y-4">
    ...
    <div class="bg-[var(--bg-card)] rounded-2xl p-4 border border-[var(--border-subtle)] shadow-sm">
      <div class="flex items-start justify-between flex-wrap gap-2">
        ...
        <span class="text-[10px] font-bold bg-cyan-50 text-cyan-700 px-2.5 py-1 rounded-full">
          ...
          >{{ auth.sesiAktif?.nama || auth.sesiAktif?.guru || '—' }}
        </span>
      </div>
    </div>
    ...
    >
      ...
      <p class="text-xs text-[var(--text-secondary)] mt-1">
        Hanya guru/pegawai dengan jabatan Direktur/Supervisor (atau super admin) yang bisa mengakses
        menu ini.
      </p>
    </div>
    ...
    <template v-else>
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-2">
          <div>
            ...
            <select
              ...
              class="w-full px-3 py-2 text-sm rounded-xl border border-[var(--border-default)] bg-[var(--bg-card)] outline-none focus:ring-2 focus:ring-cyan-500"
            >
          ...
          <div class="md:col-span-2">
            ...
            <textarea
              ...
              rows="3"
              class="w-full px-3 py-2 text-sm rounded-xl border border-[var(--border-default)] bg-[var(--bg-card)] outline-none focus:ring-2 focus:ring-cyan-500 resize-none"
            ></textarea>
          </div>
      ...
      >
        ...
        >
          ...
          <span class="text-[10px] text-[var(--text-tertiary)] font-bold"
            >{{ riwayatList.length }} catatan</span
          >
        </div>
        <div
          v-if="riwayatList.length === 0"
          class="p-6 text-center text-xs italic text-[var(--text-tertiary)]"
        >
        ...
        <div v-else class="divide-y divide-slate-100 dark:divide-slate-700">
          <div v-for="r in riwayatList" :key="r.id" class="p-3 md:p-4">
            <div class="flex items-start justify-between gap-2">
              <div class="flex-1 min-w-0">
                ...
                <p class="text-[10px] text-[var(--text-secondary)] mt-0.5">
                  ...
                  {{ r.target_nama }}
                  <span class="ml-1.5 text-[var(--text-tertiary)]"
                    >· {{ fmtTgl(r.createdAt) }}</span
                  >
                </p>
              </div>
              <div class="flex flex-col items-end gap-1">
                <span
                  :class="[
                    ...
                    prioritasClass(r.prioritas)
                  ]"
                  >{{ r.prioritas || 'sedang' }}</span
                >
                <span
                  :class="[
                    'text-[9px] font-black uppercase px-2 py-0.5 rounded',
            ...
            <p class="text-xs text-[var(--text-primary)] mt-2 whitespace-pre-line">
              {{ r.catatan }}
            </p>
            <p
              v-if="r.rekomendasi"
            ...
            >
              <p class="text-[9px] font-bold text-emerald-700 uppercase">
                Respon target ({{ fmtTgl(r.responded_at) }})
              </p>
      ...
      >
        <div
          class="px-4 py-3 border-b border-[var(--border-subtle)] grid grid-cols-1 md:grid-cols-3 gap-2"
        >
          <select
            v-model="filterLembagaRekap"
            class="px-3 py-2 text-sm rounded-xl border border-[var(--border-default)] bg-[var(--bg-card)] outline-none"
          >
            <option value="">Semua lembaga</option>
            <option v-for="l in lembagaList" :key="l.id" :value="l.nama">{{ l.nama }}</option>
          </select>
          <select
            v-model="filterStatusRekap"
            class="px-3 py-2 text-sm rounded-xl border border-[var(--border-default)] bg-[var(--bg-card)] outline-none"
          >
        ...
        <div class="px-4 py-2 bg-[var(--bg-card-elevated)] grid grid-cols-3 gap-2 text-center">
          ...
          <div>
            ...
            <p class="text-base font-black text-rose-700">
              {{ rekapFiltered.filter((x) => x.status === 'open' || !x.status).length }}
            </p>
          </div>
          <div>
            <p class="text-[9px] uppercase text-emerald-700 font-bold">Selesai</p>
            <p class="text-base font-black text-emerald-700">
              {{ rekapFiltered.filter((x) => x.status === 'selesai').length }}
            </p>
          </div>
        </div>
        <div
          v-if="rekapFiltered.length === 0"
          class="p-6 text-center text-xs italic text-[var(--text-tertiary)]"
        >
        ...
        <div
          v-else
          class="divide-y divide-slate-100 dark:divide-slate-700 max-h-[65vh] overflow-y-auto"
        >
            ...
            <div class="flex items-start justify-between gap-2">
              <div class="flex-1 min-w-0">
                <p class="text-sm font-black flex items-center gap-1.5 flex-wrap">
                  {{ r.judul }}
                  <span
                    v-if="r.source === 'naik_kelas'"
                    class="text-[9px] font-black bg-teal-100 text-teal-700 px-1.5 py-0.5 rounded uppercase tracking-wider"
                    >Naik Kelas</span
                  >
                  <span
                    v-else-if="r.source === 'mutasi'"
                    class="text-[9px] font-black bg-amber-100 text-amber-700 px-1.5 py-0.5 rounded uppercase tracking-wider"
                    >Mutasi</span
                  >
                </p>
                <p class="text-[10px] text-[var(--text-secondary)] mt-0.5">
                  <i
                    :class="[
                      ...
                      r.target_type === 'guru'
                        ? 'fa-user'
                        : r.target_type === 'santri'
                          ? 'fa-user-graduate'
                          : 'fa-school'
                    ]"
                  ></i
                  >{{ r.target_nama }}
                  <span v-if="r.target_lembaga" class="text-[var(--text-tertiary)]">
                    · {{ r.target_lembaga }}</span
                  >
                  <span class="text-[var(--text-tertiary)] ml-1.5"
                    >· oleh {{ r.created_by_nama }} · {{ fmtTgl(r.createdAt) }}</span
                  >
                </p>
              </div>
              <div class="flex flex-col items-end gap-1">
...
watch(
  () => _routeSup.query.tab,
  (v) => {
    if (['buat', 'riwayat', 'rekap'].includes(String(v))) activeTab.value = String(v)
  },
  { immediate: true }
)
...
onMounted(() => {
  ...
  unsubCatatan = subscribeColl('supervisi_catatan', (docs) => {
    catatanRaw.value = docs
  })
...
const naikKelasCatatan = computed(() => {
  ...
  for (const s of santriRaw.value) {
    ...
    for (const r of riwayat) {
      ...
      const isMutasi = String(r.keterangan || '')
        .toLowerCase()
        .includes('dipindah')
      arr.push({
...
const targetOptions = computed(() => {
  if (form.value.target_type === 'lembaga') {
    return lembagaRaw.value
      .map((l) => ({ id: l.lembaga || l.nama, label: l.lembaga || l.nama }))
      .filter((x) => x.id)
  }
...
const lembagaList = computed(() =>
  lembagaRaw.value
    .map((l) => ({ id: l.lembaga || l.nama, nama: l.lembaga || l.nama }))
    .filter((x) => x.id)
)
...
async function hapusCatatan(r) {
  ...
  try {
    await deleteOne('supervisi_catatan', String(r.id))
...
const rekapFiltered = computed(() => {
  ...
  if (filterLembagaRekap.value) {
    list = list.filter((c) => {
      ...
      if (c.target_type === 'santri')
        return (
          String(c.target_lembaga || '').toLowerCase() ===
          String(filterLembagaRekap.value).toLowerCase()
        )
      // Untuk catatan guru, cek lembaga guru-nya
      const g = guruRaw.value.find((x) => String(x.id) === String(c.target_id))
      return (
        g &&
        (g.lembaga === filterLembagaRekap.value || g.lembaga_sekolah === filterLembagaRekap.value)
      )
    })
  }
