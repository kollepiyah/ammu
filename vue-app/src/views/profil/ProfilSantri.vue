<template>
  <div class="space-y-4">
    <div class="bg-[var(--bg-card)] rounded-2xl overflow-hidden border border-[var(--border-subtle)] shadow-sm">
      <div class="relative bg-gradient-to-br from-cyan-500 dark:from-cyan-700 via-cyan-600 dark:via-cyan-800 to-cyan-700 dark:to-cyan-900 px-6 py-7">
        <img src="/logo.png" alt="" class="absolute -right-6 -top-6 w-36 h-36 object-contain opacity-10 pointer-events-none" />
        <div class="relative flex flex-col md:flex-row items-center gap-5">
          <div class="w-24 h-24 md:w-28 md:h-28 bg-[var(--bg-card)]/20 border-4 border-white/80 rounded-full flex items-center justify-center overflow-hidden shadow-2xl flex-shrink-0">
          ...
          <div class="flex-1 text-center md:text-left text-white">
            ...
            <p class="text-xs font-bold text-cyan-50/90 uppercase tracking-wider mt-1">{{ santri?.lembaga || '-' }} · Kelas {{ santri?.kelas || '-' }}</p>
            <span class="inline-block mt-2 text-[11px] bg-[var(--bg-card)]/25 border border-white/30 text-white font-black px-3 py-1 rounded-full uppercase tracking-widest">{{ santri?.is_mukim ? "Ma'had" : (santri?.is_fullday ? 'Fullday' : 'Pulang Pergi') }}</span>
          </div>
        </div>
      </div>
      ...
      <div v-if="mode === 'view'" class="p-4 space-y-4">
        ...
        <div class="snap-center shrink-0 w-[88%] md:w-auto bg-[var(--bg-card-elevated)] p-4 rounded-2xl border border-[var(--border-subtle)]">
          ...
          <ul class="space-y-2 text-sm">
            ...
            <li class="flex justify-between"><span class="text-[var(--text-secondary)] font-bold">Tgl Lahir:</span><span class="font-black">{{ santri?.tgl_lahir || '-' }}</span></li>
        ...
        <div class="snap-center shrink-0 w-[88%] md:w-auto bg-emerald-50 dark:bg-emerald-900/20 p-4 rounded-2xl border border-emerald-200 dark:border-emerald-800">
          ...
          <ul class="space-y-2 text-sm">
            ...
            <li class="flex justify-between"><span class="text-emerald-700 font-bold">NIK:</span><span class="font-black">{{ ayahNik || '-' }}</span></li>
            <li class="flex justify-between"><span class="text-emerald-700 font-bold">Pekerjaan:</span><span class="font-black">{{ ayahPekerjaan || '-' }}</span></li>
            <li class="flex justify-between"><span class="text-emerald-700 font-bold">Pendidikan:</span><span class="font-black">{{ ayahPendidikan || '-' }}</span></li>
            <li class="flex justify-between"><span class="text-emerald-700 font-bold">No. HP:</span><span class="font-black">{{ ayahHp || '-' }}</span></li>
          </ul>
        </div>
        ...
        <div class="snap-center shrink-0 w-[88%] md:w-auto bg-rose-50 dark:bg-rose-900/20 p-4 rounded-2xl border border-rose-200 dark:border-rose-800">
          ...
          <ul class="space-y-2 text-sm">
            ...
            <li class="flex justify-between"><span class="text-rose-700 font-bold">NIK:</span><span class="font-black">{{ ibuNik || '-' }}</span></li>
            <li class="flex justify-between"><span class="text-rose-700 font-bold">Pekerjaan:</span><span class="font-black">{{ ibuPekerjaan || '-' }}</span></li>
            <li class="flex justify-between"><span class="text-rose-700 font-bold">Pendidikan:</span><span class="font-black">{{ ibuPendidikan || '-' }}</span></li>
            <li class="flex justify-between"><span class="text-rose-700 font-bold">No. HP:</span><span class="font-black">{{ ibuHp || '-' }}</span></li>
          </ul>
        </div>
        ...
        <div class="bg-amber-50 dark:bg-amber-900/20 p-4 rounded-2xl border border-amber-200 dark:border-amber-800">
          ...
          <ul class="space-y-2 text-sm">
            ...
            <li class="flex justify-between"><span class="text-amber-700 font-bold">No WhatsApp:</span><span class="font-black">{{ santri?.wa || '-' }}</span></li>
          </ul>
        </div>
      </div>
      ...
      <div v-else class="p-4 space-y-4">
        <p class="text-xs text-[var(--text-secondary)] bg-emerald-50/60 dark:bg-emerald-900/20 border border-emerald-200 dark:border-emerald-800 rounded-xl px-3 py-2">
          ...
          Lengkapi data yang masih kosong. Data sekolah, kelas, dan guru hanya bisa diubah oleh admin.
        </p>

        <div class="grid grid-cols-1 md:grid-cols-2 gap-3">
          <div>
            <label class="block text-xs font-bold text-[var(--text-secondary)] mb-1 uppercase">Tempat Lahir</label>
            <input v-model="editForm.tempat_lahir" type="text" class="w-full px-3 py-2 text-sm rounded-xl border border-[var(--border-default)] bg-[var(--bg-card-elevated)] focus:ring-2 focus:ring-cyan-500 outline-none" />
          </div>
          <div>
        ...
        <!-- Ayah -->
        <div class="bg-emerald-50/40 dark:bg-emerald-900/20 rounded-2xl p-4 border border-emerald-200 dark:border-emerald-800">
          <h4 class="text-xs font-black text-emerald-700 dark:text-emerald-300 uppercase mb-3"><i class="fas fa-male mr-1"></i>Data Ayah</h4>
          <div class="grid grid-cols-1 md:grid-cols-2 gap-3">
            <div class="md:col-span-2">
              <label class="block text-xs font-bold text-emerald-700 mb-1 uppercase">Nama Ayah</label>
              <input v-model="editForm.nama_ayah" type="text" class="w-full px-3 py-2 text-sm rounded-xl border border-emerald-300 bg-white dark:bg-[var(--bg-card-elevated)] focus:ring-2 focus:ring-emerald-500 outline-none" />
            </div>
            <div>
              <label class="block text-xs font-bold text-emerald-700 mb-1 uppercase">NIK Ayah</label>
              <input v-model="editForm.nik_ayah" type="text" maxlength="16" class="w-full px-3 py-2 text-sm rounded-xl border border-emerald-300 bg-white dark:bg-[var(--bg-card-elevated)] focus:ring-2 focus:ring-emerald-500 outline-none" />
            </div>
            <div>
              <label class="block text-xs font-bold text-emerald-700 mb-1 uppercase">Pekerjaan</label>
              <input v-model="editForm.pekerjaan_ayah" type="text" class="w-full px-3 py-2 text-sm rounded-xl border border-emerald-300 bg-white dark:bg-[var(--bg-card-elevated)] focus:ring-2 focus:ring-emerald-500 outline-none" />
            </div>
            <div>
              <label class="block text-xs font-bold text-emerald-700 mb-1 uppercase">Pendidikan Terakhir</label>
              <select v-model="editForm.pendidikan_ayah" class="w-full px-3 py-2 text-sm rounded-xl border border-emerald-300 bg-white dark:bg-[var(--bg-card-elevated)] focus:ring-2 focus:ring-emerald-500 outline-none">
                <option value="">-- Pilih --</option>
                <option v-for="p in PENDIDIKAN_OPTS" :key="p" :value="p">{{ p }}</option>
              </select>
            </div>
            <div>
              <label class="block text-xs font-bold text-emerald-700 mb-1 uppercase">No. HP Ayah</label>
              <input v-model="editForm.hp_ayah" type="tel" class="w-full px-3 py-2 text-sm rounded-xl border border-emerald-300 bg-white dark:bg-[var(--bg-card-elevated)] focus:ring-2 focus:ring-emerald-500 outline-none" />
            </div>
          </div>
        </div>

        <!-- Ibu -->
        <div class="bg-rose-50/40 dark:bg-rose-900/20 rounded-2xl p-4 border border-rose-200 dark:border-rose-800">
          <h4 class="text-xs font-black text-rose-700 dark:text-rose-300 uppercase mb-3"><i class="fas fa-female mr-1"></i>Data Ibu</h4>
          <div class="grid grid-cols-1 md:grid-cols-2 gap-3">
            <div class="md:col-span-2">
              <label class="block text-xs font-bold text-rose-700 mb-1 uppercase">Nama Ibu</label>
              <input v-model="editForm.nama_ibu" type="text" class="w-full px-3 py-2 text-sm rounded-xl border border-rose-300 bg-white dark:bg-[var(--bg-card-elevated)] focus:ring-2 focus:ring-rose-500 outline-none" />
            </div>
            <div>
              <label class="block text-xs font-bold text-rose-700 mb-1 uppercase">NIK Ibu</label>
              <input v-model="editForm.nik_ibu" type="text" maxlength="16" class="w-full px-3 py-2 text-sm rounded-xl border border-rose-300 bg-white dark:bg-[var(--bg-card-elevated)] focus:ring-2 focus:ring-rose-500 outline-none" />
            </div>
            <div>
              <label class="block text-xs font-bold text-rose-700 mb-1 uppercase">Pekerjaan</label>
              <input v-model="editForm.pekerjaan_ibu" type="text" class="w-full px-3 py-2 text-sm rounded-xl border border-rose-300 bg-white dark:bg-[var(--bg-card-elevated)] focus:ring-2 focus:ring-rose-500 outline-none" />
            </div>
            <div>
              <label class="block text-xs font-bold text-rose-700 mb-1 uppercase">Pendidikan Terakhir</label>
              <select v-model="editForm.pendidikan_ibu" class="w-full px-3 py-2 text-sm rounded-xl border border-rose-300 bg-white dark:bg-[var(--bg-card-elevated)] focus:ring-2 focus:ring-rose-500 outline-none">
                <option value="">-- Pilih --</option>
                <option v-for="p in PENDIDIKAN_OPTS" :key="p" :value="p">{{ p }}</option>
              </select>
            </div>
            <div>
              ...
              <input v-model="editForm.hp_ibu" type="tel" class="w-full px-3 py-2 text-sm rounded-xl border border-rose-300 bg-white dark:bg-[var(--bg-card-elevated)] focus:ring-2 focus:ring-rose-500 outline-none" />
            </div>
          </div>
        </div>
...
const ayahPendidikan = computed(() => props.santri?.ayah?.pendidikan || props.santri?.pendidikan_ayah)
const ayahHp = computed(() => props.santri?.ayah?.telp || props.santri?.ayah?.hp || props.santri?.hp_ayah || props.santri?.telp_ayah)
...
const ibuHp = computed(() => props.santri?.ibu?.telp || props.santri?.ibu?.hp || props.santri?.hp_ibu || props.santri?.telp_ibu)
...
async function saveProfile() {
  ...
  try {
    ...
    const payload = {
      ...
      // Ayah — flat + nested (PSB compat)
      nama_ayah: ef.nama_ayah,
      nik_ayah: ef.nik_ayah,
      pekerjaan_ayah: ef.pekerjaan_ayah,
      pendidikan_ayah: ef.pendidikan_ayah,
      hp_ayah: ef.hp_ayah,
      'ayah.nama': ef.nama_ayah,
      'ayah.nik': ef.nik_ayah,
      'ayah.pekerjaan': ef.pekerjaan_ayah,
      'ayah.pendidikan': ef.pendidikan_ayah,
      'ayah.telp': ef.hp_ayah,
      // Ibu
      ...
      hp_ibu: ef.hp_ibu,
      'ibu.nama': ef.nama_ibu,
      'ibu.nik': ef.nik_ibu,
      'ibu.pekerjaan': ef.pekerjaan_ibu,
      'ibu.pendidikan': ef.pendidikan_ibu,
      'ibu.telp': ef.hp_ibu,
